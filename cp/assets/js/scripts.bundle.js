...
// Class definition
var KTChat = function () {
	var initChat = function (parentEl, data) {
		...
		jQuery(document).on('click', '.more_comments', function(){
			var element = $(this);
		...
		var handleMessaging = function() {
            ...
            var fileField = KTUtil.find(parentEl, '[type="file"]');
            ...
            formData.append("textvalue", textarea.value);
            ...
            jQuery.ajax({
				...
				success: function(data) {
					...
					if(data.success == true){
						...
						jQuery(node).attr('style', 'margin:1.5rem!important;min-width:50%!important;');
						
						var imgPath = jQuery(KTUtil.find(parentEl,'.kt-chat__input .profile_img')).val();
						if(imgPath){
							imgPath = '<img src="'+imgPath+'" alt="image">'
						}else{
							imgPath = '<i class="flaticon-user"  style="font-size:30px!important;"></i>';
						}
	...
	return {
		...
		init: function() {
			...
			var thisInstance = KTUtil;
			var initChatbox = initChat;
			
			initChatbox( thisInstance.getByID('kt_chat_modal'));
