...
// 			$tableJoinMapping['vtiger_attachmentsfolderfolderid'] = 'INNER JOIN vtiger_attachmentsfolder';
	...
	protected function getConditionValue($value, $operator, $field) {
		...
		$usersCustomPicklists = array("roleid");
		
		$oldDataType = false;
		
		$moduleName = getTabModuleName($field->getTabId());
		
		if(in_array($field->getFieldName(), $usersCustomPicklists) && ($moduleName == 'Users')){ // Felipe : 12-Sep-2018 Users roleid
		    $oldDataType = $field->getFieldDataType();
		    $field->setFieldDataType("picklist");
		}
		...
		if($oldDataType){
		    $field->setFieldDataType($oldDataType);
		}
