								...
								<table class="table table-bordered">
												...
												foreach($monthly_totals as $mnth_info){
													...
													foreach($symbol_values as $symbol_value){
														if($mnth_info['year'] == $symbol_value['year'] && $mnth_info['month'] == $symbol_value['month']){
															echo number_format($symbol_value['amount'],2);
															$found = 1;
														}
													}
													if(!$found)
									...
									<tfoot>
										<tr>
											...
											<?php 
												foreach($monthly_totals as $mnth_info){
													echo "<td class='text-right'>$".number_format($mnth_info['monthly_total'],2)."</td>";
												}
											?>
											<td class="text-right">$<?php echo number_format($data['grand_total'],2) ;?></td>
										</tr>
									</tfoot>
								</table>
							</div>
