...
{strip}
							...
							<div class="pull-right">
								<div class="dropdown">
									<button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown">
										<span title="{vtranslate('LBL_NEW_DOCUMENT', $MODULE_NAME)}"><i class="material-icons">add</i></span>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_NEW_DOCUMENT', 'Documents')}</span>&nbsp; <span class="caret"></span>
									</button>
									<ul class="dropdown-menu animated flipInY pull-right">
										<li class="dropdown-header"><i class="material-icons">file_upload</i> {vtranslate('LBL_FILE_UPLOAD', 'Documents')}</li>
										<li id="VtigerAction">
										...
										<li class="dropdown-header"><i class="material-icons">link</i> {vtranslate('LBL_LINK_EXTERNAL_DOCUMENT', 'Documents')}</li>
										<li id="shareDocument"><a href="javascript:Documents_Index_Js.createDocument('E',{$PARENT_ID},'{$MODULE_NAME}')">&nbsp;<i class="material-icons">link</i>&nbsp;&nbsp; {vtranslate('LBL_FROM_SERVICE', 'Documents', {vtranslate('LBL_FILE_URL', 'Documents')})}</a></li>
										<li role="separator" class="divider"></li>
										<li id="createDocument"><a href="javascript:Documents_Index_Js.createDocument('W',{$PARENT_ID},'{$MODULE_NAME}')"><i class="material-icons">description</i> {vtranslate('LBL_CREATE_NEW', 'Documents', {vtranslate('SINGLE_Documents', 'Documents')})}</a></li>
									</ul>
								</div>
							</div>
						{/if}
	...
	<div class="right-block col-lg-4 col-md-12 col-sm-12 col-xs-12">
				...
				<div class="widgetContainer_troubleTickets" data-url="{$HELPDESK_WIDGET_MODEL->getUrl()}" data-name="{$HELPDESK_WIDGET_MODEL->getLabel()}">
					<div class="widget_header clearfix">
						...
						{if $HELPDESK_WIDGET_MODEL->get('action')}
							<div class="pull-right">
								<button class="btn addButton btn-primary btn-sm createRecord" type="button" data-url="{$HELPDESK_WIDGET_MODEL->get('actionURL')}">
									<i class="material-icons">add</i>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_ADD',$MODULE_NAME)}</span>
								</button>
							</div>
						{/if}
				...
				<div class="widgetContainer_mileStone" data-url="{$MILESTONE_WIDGET_MODEL->getUrl()}" data-name="{$MILESTONE_WIDGET_MODEL->getLabel()}">
					<div class="widget_header clearfix">
						...
						{if $MILESTONE_WIDGET_MODEL->get('action')}
							<div class="pull-right">
								<button class="btn addButton btn-sm btn-primary createRecord"  id="createProjectMileStone" type="button" data-url="{$MILESTONE_WIDGET_MODEL->get('actionURL')}">
									<i class="material-icons">add</i>&nbsp;<span class="hidden-sm hidden-xs">{vtranslate('LBL_ADD',$MODULE_NAME)}</span>
								</button>
							</div>
						{/if}
