...
{strip}
	...
	<div class="row" style="padding:1% 0">
		<div class="col-sm-6">
			<button class="btn btn-primary addButton addCustomBlock" type="button">
				<i class="ti-plus"></i>&nbsp;
						...
						<div class="col-sm-12">
							<div class="layoutBlockHeader row">
								<div class="blockLabel col-sm-3 padding10 marginLeftZero" style="word-break: break-all;">
									...
									{/if}
									<h4 class="display-inline-block translatedBlockLabel">{vtranslate($BLOCK_LABEL_KEY, $SELECTED_MODULE_NAME)}</h4>
								</div>
								<div class="col-sm-9 padding10 marginLeftZero">
									<div class="blockActions" style="float:right !important;">
										<span>
											<i class="ti-info-alt" title="{vtranslate('LBL_COLLAPSE_BLOCK_DETAIL_VIEW', $QUALIFIED_MODULE)}"></i>&nbsp; {vtranslate('LBL_COLLAPSE_BLOCK', $QUALIFIED_MODULE)}&nbsp;
										...
										{if $BLOCK_MODEL->isAddCustomFieldEnabled()}
											<button class="btn btn-primary addButton btn-sm addCustomField" type="button">
												<i class="ti-plus"></i>&nbsp;{vtranslate('LBL_ADD_CUSTOM_FIELD', $QUALIFIED_MODULE)}
											</button>&nbsp;&nbsp;
										{/if}
										{if $BLOCK_MODEL->isActionsAllowed()}
											<button class="inActiveFields addButton btn btn-primary btn-sm">{vtranslate('LBL_SHOW_HIDDEN_FIELDS', $QUALIFIED_MODULE)}</button>&nbsp;&nbsp;
											{if $BLOCK_MODEL->isCustomized()}
												<button class="deleteCustomBlock addButton btn btn-primary btn-sm">{vtranslate('LBL_DELETE_CUSTOM_BLOCK', $QUALIFIED_MODULE)}</button>
											{/if}
										{/if}
									</div>
								</div>
													...
													<div class="row " style="padding: 10px 0px;">
														...
														<div class="fieldProperties col-sm-10" data-field-id="{$FIELD_MODEL->get('id')}">
															<span class="mandatory switch text-capitalize {if (!$IS_MANDATORY)}disabled{/if} {if $FIELD_MODEL->isMandatoryOptionDisabled()} cursorPointerNotAllowed {else} cursorPointer {/if}"
																...
																<i class="ti-info-alt" data-name="mandatory" 
															...
															<span class="quickCreate switch {if (!$IS_QUICK_EDIT_ENABLED)}disabled{/if} 
																...
																<i class="ti-plus" data-name="quickcreate" 
															...
															<span class="header switch {if (!$IS_HEADER_FIELD)} disabled {/if} 
																...
																<i class="ti-flag-alt" data-name="headerfield" 
															...
															<span class="summary switch {if (!$IS_SUMMARY_VIEW_ENABLED)} disabled {/if} 
																...
																<i class="ti-key" data-name="summaryfield" 
																	data-enable-value="1" data-disable-value="0"
														...
														<span class="col-sm-2 actions">
															{if $FIELD_MODEL->isEditable()}
																<a href="javascript:void(0)" class="editFieldDetails">
																	<i class="ti-pencil text-info" title="{vtranslate('LBL_EDIT', $QUALIFIED_MODULE)}"></i>
																</a>
															{/if}
															{if $FIELD_MODEL->isCustomField() eq 'true'}
																<a href="javascript:void(0)" class="deleteCustomField pull-right" data-field-id="{$FIELD_MODEL->get('id')}"
																	...
																	{/if} >
																	<i class="ti-trash" title="{vtranslate('LBL_DELETE', $QUALIFIED_MODULE)}"></i>
																</a>
															{/if}
														</span>
													</div>
									...
									{if $BLOCK_MODEL->isAddCustomFieldEnabled()}
										<li class="row dummyRow">
											...
											<span class="col-sm-4" style="margin-top: 7%;margin-left: -15%;">
												<button class="btn btn-primary btn-sm addButton" style="padding: 2px 15px;"><i class="ti-plus"></i>&nbsp;{vtranslate('LBL_ADD',$QUALIFIED_MODULE)}</button>
											</span>
										</li>
									{/if}
								{/if}
								...
								{if count($FIELDS_LIST)%2 neq 0 }
									{if $BLOCK_MODEL->isAddCustomFieldEnabled()}
										<li class="row dummyRow">
											...
											<span class="col-sm-4" style="margin-top: 7%;margin-left: -15%;">
												<button class="btn btn-primary btn-sm addButton" style="padding: 2px 15px;"><i class="ti-plus"></i>&nbsp{vtranslate('LBL_ADD',$QUALIFIED_MODULE)}</button>
											</span>
										</li>
									{/if}
								{/if}
		...
		<div class="layoutBlockHeader">
			...
			<div class="col-sm-9 padding10 marginLeftZero">
				<div class="blockActions" style="float: right !important;">
					...
					<button class="btn btn-primary addButton addCustomField" type="button">
						<i class="ti-plus"></i>&nbsp;{vtranslate('LBL_ADD_CUSTOM_FIELD', $QUALIFIED_MODULE)}
					</button>&nbsp;&nbsp;
					<button class="inActiveFields addButton btn btn-primary btn-sm">{vtranslate('LBL_SHOW_HIDDEN_FIELDS', $QUALIFIED_MODULE)}</button>&nbsp;&nbsp;
					<button class="deleteCustomBlock addButton btn btn-primary btn-sm">{vtranslate('LBL_DELETE_CUSTOM_BLOCK', $QUALIFIED_MODULE)}</button>
