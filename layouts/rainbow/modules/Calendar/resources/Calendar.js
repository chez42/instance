...
//handling eventtype condition element display
	...
	registerCollapseEvents: function (widget) {
		...
		widget.on('show.bs.collapse hide.bs.collapse', function (e) {
			...
			if (e.type === 'hide') {
				widgetStateIndicator.removeClass('ti-angle-down').addClass('ti-angle-right');
				thisInstance.changeWidgetDisplayState(widget, 'hide');
			} else {
				widgetStateIndicator.removeClass('ti-angle-right').addClass('ti-angle-down');
		...
		agendaListView = view.extend({
			...
			registerToggleMoreDetailsEvent: function (container) {
				...
				container.on('click', '.agenda-more-details', function () {
					...
					if (indicator.hasClass('ti-plus')) {
						...
						indicator.removeClass('ti-plus').
								addClass('ti-minus');
					} else {
						details.addClass('hide');
						indicator.removeClass('ti-minus').
								addClass('ti-plus');
					}
				});
			},
			...
			generateEventHTML: function (event) {
				var html = '' +
						...
						'<i class="ti-plus" title=' + app.vtranslate('JS_DETAILS') + '></i>' +
						'</div>' +
				...
				if (event.recurringcheck) {
					html += '<i class="ti-reload" style="font-size:10px;" title="' + app.vtranslate('JS_RECURRING_EVENT') + '"></i>&nbsp;';
				}
