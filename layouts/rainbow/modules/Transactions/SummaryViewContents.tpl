            ...
            {if $FIELD_MODEL->get('name') neq 'modifiedtime' && $FIELD_MODEL->get('name') neq 'createdtime'}
                <tr class="summaryViewEntries">
                    ...
                    <td class="fieldValue" style="width:65%">
                        <div class="row-fluid">
                        {*15.6.20 added check for uitype 1*}
                        <span class="value" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'or $FIELD_MODEL->get('uitype') eq '1'}style="word-wrap: break-word;white-space:pre-wrap;"{/if}>
                            {if ($FIELD_MODEL->get('uitype') eq "1" OR $FIELD_MODEL->get('uitype') eq "2" OR $FIELD_MODEL->get('uitype') eq "1003") AND $FIELD_MODEL->get('name') eq 'security_symbol'}
                                <span class="omniSecuritySymbol" style="cursor:pointer" data-security_symbol="{$FIELD_MODEL->get('fieldvalue')}">{$FIELD_MODEL->get('fieldvalue')}</span>
                            {else}
