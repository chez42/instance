...
Vtiger.Class('Vtiger_Index_Js', {
	...
	registerEvents: function() {
		...
		if($(window).width()<451) var minHeight = $(window).height() - 175;
		else var minHeight = $(window).height() - 105;
	...
	addBodyScroll: function () {
		...
		$(".main-container .content-area").css("min-height",minHeight+"px !important");
	},
	...
	registerQuickCreateSubMenus : function() {
		jQuery("#quickCreateModules, #quickCreateModules_mobile").on("click",".quickCreateModuleSubmenu",function(e){
			e.preventDefault();
	...
	registerQuickCreateEvent : function (){
		var thisInstance = this;
		jQuery("#quickCreateModules, #quickCreateModules_mobile").on("click",".quickCreateModule",function(e,params){
	...
	registerAppTriggerEvent : function() {
		...
		jQuery('.navbar, .main-container').on('click', function() {
			toggleAppMenu('hide');
		});
		...
		jQuery('.app-modules-dropdown-container').hover(function(e) {
			if(window.innerWidth>=500 && ! isTouch){
...
//					
	...
	*/
	...
	showRelatedQuickPreviewForId: function(recordId, moduleName, appName, templateId, isReference, mode) {
		var self = this;
		if(self.isInventoryModule(moduleName)) {
			self._showRelatedInventoryQuickPreviewForId(recordId, moduleName, templateId, isReference, mode);
		} else {
			self._showRelatedQuickPreviewForId(recordId, moduleName, appName, isReference);
		}
	},
	
	_showRelatedInventoryQuickPreviewForId: function (recordId, moduleName, templateId, isReference, mode) {
		var thisInstance = this;
		var params = {};
		if(typeof moduleName === 'undefined') {
			moduleName = app.module();
		...
		params['parent_recordId'] = app.getRecordId();
		params['parent_module'] = app.module();
		params['module'] = moduleName;
		params['record'] = recordId;
		params['view'] = 'RelatedRecordQuickPreview';
	...
	_showRelatedQuickPreviewForId: function (recordId, moduleName, appName, isReference) {
		var self = this;
		var params = {};
		if (typeof moduleName === 'undefined') {
			moduleName = app.module();
	...
	registerRelatedNextRecordClickEvent: function(container){
		var self = this;
		container.find('#quickPreviewNextRelatedRecordButton').on('click',function(e){
			var element = jQuery(e.currentTarget);
			var nextRecordId = element.data('record') || element.data('id');
			var moduleName = container.find('#sourceModuleName').val();
			var appName = element.data('app');
			...
			self.showRelatedQuickPreviewForId(nextRecordId, moduleName, appName, templateId, false, 'navigation');
		});
	},
	
	registerRelatedPreviousRecordClickEvent: function(container){
		var self = this;
		container.find('#quickPreviewPreviousRelatedRecordButton').on('click', function (e) {
			var element = jQuery(e.currentTarget);
			var prevRecordId = element.data('record') || element.data('id');
			var moduleName = container.find('#sourceModuleName').val();
			var appName = element.data('app');
			...
			self.showRelatedQuickPreviewForId(prevRecordId, moduleName, appName, templateId, false, 'navigation');
		});
	},
	
	/**
	 * Function request for documents related reminder popups
	 */
	requestDocumentsReminder : function() {
		var self =this;
		var url = 'module=Documents&action=DocumentsReminder&mode=getReminders';
		AppConnector.request(url).then(function(data){
			if(data.success && data.result) {
				var response = data.result;
				var ContactList = response.Contacts;
				var ContactDocList = response.Documents;
				for(var contactid in ContactList){
					var contactName = ContactList[contactid];
					for(var docid in ContactDocList[contactid]) {
						var DocTitle  = ContactDocList[contactid][docid];
						self.showDocumentsReminderPopup(contactName, DocTitle, docid);
					}
				}
			}
		});
	...
	/**
	 * Function display the Documents Reminder popup
	 */
	showDocumentsReminderPopup : function(contactName, record, recordid) {
		...
		var notifyParams = {
			...
			'message' : '<div class="col-sm-12">'+
						...
						'<div class="row" style="margin-top:10px;">'+
							'<div class="col-md-6">'+
								'<a id="snooze_'+recordid+'" class="btn btn-success" href=# data-id="'+recordid+'">'+
								app.vtranslate('Remind me later')+'</a>'+
							'</div>'+
							'<div class="col-md-6">'+
								'<a id="acknowledge_'+recordid+'" class="btn btn-info pull-right" href=# data-id="'+recordid+'">'+
								app.vtranslate('Acknowledge')+'</a>'+
							'</div>'+
						'</div>'+
					'</div>'
			};
		...
		jQuery('#snooze_'+recordid).on('click', function(e) {
			jQuery(e.currentTarget).closest('.vt-notification').find('[data-notify="dismiss"]').trigger('click');
			app.request.post({
				'data' : {
					'module' : 'Documents',
					'action' : 'DocumentsReminder',
					'mode' : 'Snooze',
					'record' : recordid
				}
			}).then(function(e,res) {});
		...
		jQuery('#acknowledge_'+recordid).on('click', function(e) {
			jQuery(e.currentTarget).closest('.vt-notification').find('[data-notify="dismiss"]').trigger('click');
			app.request.post({
				'data' : {
					'module' : 'Documents',
...
function checkInactive(){
	var lastactivity = getCookie('lastactivity');
	if(lastactivity && lastactivity>0){
		var currenttime = Math.floor(Date.now() / 1000);
		if(currenttime - lastactivity > 600){ //Set auto logout after 2mins inactive
			currenttime = Math.floor(Date.now() / 1000);
			setCookie('lastactivity',currenttime);
			location.href = 'index.php?module=Users&action=Logout';
		}
	}
	else{
		var currenttime = Math.floor(Date.now() / 1000);
...
function mycOpenLeftSidebar(){
	...
	if(window.innerWidth<=900){
		...
		$("#appnavigator > .app-navigator").css("left","240px");		
		//$(".left-sidebar").animate({ "width": "220px" }, "fast" );
	}
	else{
		$(".left-sidebar").css("width","240px");
		$("#appnavigator > .app-navigator").css("left","260px");
		//$(".left-sidebar").animate({ "width": "240px" }, "fast" );
	}
...
function mycCloseLeftSidebar(){
	...
	if(window.innerWidth<=900){
		$(".left-sidebar").css("width","0px");
		//$(".left-sidebar").animate({ "width": "0px" }, "fast" );
	}
	else $(".left-sidebar").css("width","0px");
	
	$(".quickTopButtons").removeAttr("style");
	//$("#page, .app-fixed-navbar").css("margin-left","80px");
	$(".with-childs").find("ul").css("padding-left","5px");
