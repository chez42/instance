...
Vtiger.Class("Vtiger_Detail_Js",{
	...
	triggerSendSms: function(detailActionUrl, module) {
	 ...
	 deleteRelatedActivity : function(e) {
		...
		if(recurringEnabled) {
			...
			app.helper.showPromptBox({'message' : message}).then(function(data) {
				thisInstance.deleteActivityRelation(postData);
			},
			function(error,err) {
			});	
	...
	deleteRelatedTasks : function(e) {
		...
		if(id){
			var message = app.vtranslate('JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE');
			app.helper.showPromptBox({'message' : message}).then(function(data) {
				thisInstance.deleteTaskRelation(postData);
			},
			function(error,err) {
			});	
		}
	...
	deleteTaskRelation : function(customParams) {
		var params = {
			'module'			: app.getModuleName(),
			'related_module'	: 'Task',
			'action'			: 'RelationAjax',
		...
		app.request.post({data: params}).then(function(err, data) {
			if(data) {
				params = {
					...
					'view'		: 'Detail',
					'module'	: app.getModuleName(),
					'mode'		: 'showRecentTasks'
				};
				app.request.get({data: params}).then(function(err, result) {
					jQuery('#relatedTasks').html(result);
					Vtiger_Detail_Js.getInstance().registerEventForTaskWidget();
				});
			}
		});
	},
	...
	triggerAddCommentForm : function(ActionUrl){
		
		var thisInstance = this;
		
		var actionParams = ActionUrl+"&selected_id="+jQuery('#recordId').val();
		...
		app.request.post({data:actionParams}).then(
			function(error, data) {
				...
				if(isFormExists){
					...
					thisInstance.saveCommentForDetail(form);
				}
			}
		);
	},
	...
	detailViewRecentTaskTabLabel : 'Tasks',
	...
	registerEventsForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'a.relationDelete', function(e){
			...
			if(relatedModuleName == 'Calendar' && row.data('recurringEnabled')) {
				...
				app.helper.showPromptBox({'message' : message}).then(
					function(e) {
	...
	*/
	...
	registerAjaxEditEvent : function(){
		...
		detailContentsHolder.on('click','.detailview-table .fieldValue .editAction', function(e) {
			var editedLength = jQuery('.detailview-table .fieldValue .ajaxEdited').length;
			if(editedLength === 0) { 
	...
	ajaxEditHandling : function(currentTdElement){
		...
		var customHandlingFields = ['owner','ownergroup','picklist','multipicklist','reference','currencyList','text', 'DocFolderPicklist'];
	...
	getInlineWrapper : function(element) {
		var wrapperElement = element.closest('.fieldValue');
...
//				ajaxEditNewValue = fobj;
			...
			if(previousValue == ajaxEditNewValue) {
				...
				var params = {
					...
					submitHandler : function(form){
						...
						jQuery(form).attr("modulename", thisInstance.getModuleName());
	...
	registerBlockAnimationEvent : function(){
		...
		detailContentsHolder.on('click','.blockToggle',function(e){ 
			...
			if(data.mode == 'show'){
				hideHandler();
				currentTarget.hide();
				closestBlock.addClass("closedBlock");
				closestBlock.find("[data-mode='hide']").removeClass('hide').show();
			}else{
				showHandler();
				currentTarget.hide();
				closestBlock.removeClass("closedBlock");
				closestBlock.find("[data-mode='show']").removeClass('hide').show();
				
				closestBlock.find(".fieldLabel").each(function() {
	...
	registerSummaryViewContainerEvents : function(summaryViewContainer){
		...
		this.registerEventForTaskWidget();
		this.loadWidgets();
		/**
	...
	loadWidget : function(widgetContainer) {
		...
		widgetContainer.waitMe({effect : 'bounce',text : 'Please wait...',textPos : 'horizontal'});
		...
		self.getRelatedRecordsCount(recordId, moduleName).then(function(data){
			jQuery.each(data, function(key, value){
				...
				if(parseInt(value) > 0){
					...
					numberEle.removeClass('disabled');
				} else{
	...
	toggleCommentContent: function (e) {
		var currentTarget = jQuery(e.currentTarget);
		var commentContentBlock = currentTarget.closest('.comment');
		...
		var fullComment = commentContentInfo.data('fullcomment');
		...
		if (currentTarget.hasClass('showMore')) {
			...
			commentContentInfo.html(commentContentInfo.data('shortcomment')+toggleElement.clone().html());
		}
	},
...
//                        Vtiger_Index_Js.getInstance().registerMentionListener(jQuery('.widgetContainer_comments').find('.mention_listener'));
	...
	showAllTags : function(container) {
		...
		app.helper.showModal(showTagModal.find('.modal-dialog'),{'cb' : function(modalContainer){
				...
				var registerShowAllTagEvents = function(modalContainer) {
					...
					if(typeof $.fn.instaFilta !== 'undefined'){
	...
	registerRelatedRowClickEvent: function() {
		...
		detailContentsHolder.on('click','.listViewEntries',function(e){
			...
			if(selection.length == 0) { 
				...
				if(targetElement.hasClass('connectionContact')) return;
			...
			detailContentsHolder.on('click','.connectionContact',function(e){
				var selection = window.getSelection().toString();
				if(selection.length == 0) { 
					var targetElement = jQuery(e.target, jQuery(e.currentTarget));
					if(targetElement.hasClass('js-reference-display-value')) return;
	...
	registerEventToShowQuickPreview: function () {
		var self = this;
		var detailContentsHolder = this.getContentHolder();
		detailContentsHolder.on('click', '.quickView', function (e) {
			e.stopImmediatePropagation();
			
			var recordId = jQuery('#recordId').val();
			var element = detailContentsHolder.find(e.currentTarget);
	...
	showQuickPreviewForId: function (recordId, appName) {
		...
		var relatedModuleName = self.getRelatedModuleName();
		var vtigerInstance = Vtiger_Index_Js.getInstance();
		vtigerInstance.showRelatedQuickPreviewForId(recordId, relatedModuleName, appName);
	},
	...
	registerEvents : function() {
		...
		detailContentsHolder.on('click','.moreRecentActivityTasks', function(){
	...
	registerEventForTaskWidget : function(){
		var thisInstance = this;
		/*
		 * Register click event for add button in Related Activities widget
		 */
		jQuery('.toDotask').on('click', function(e){
			var currentTarget = jQuery(e.currentTarget);
			var referenceModuleName;
			...
			app.event.on('post.QuickCreateForm.save',function(event,data){
				...
				params['view'] = 'Detail';
				params['module'] = module;
				params['mode'] = 'showRecentTasks';
				

				app.request.post({"data":params}).then(
					function(err,data) {
						var activitiesWidget = jQuery('#relatedTasks');
						activitiesWidget.html(data);
						vtUtils.applyFieldElementsView(activitiesWidget);
						thisInstance.registerEventForTaskWidget();
					}
				);
			});
