...
Vtiger.Class("Vtiger_List_Js", {
	...
	triggerCalculateBilling: function(){
		var listInstance = window.app.controller();
		listInstance.performCalculateBilling();
	},
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
	...
	registerInlineEdit: function (currentTrElement) {
		...
		for (var i = 0; i < tdElements.length; i++) {
			...
			var customHandlingFields = ['owner', 'ownergroup', 'picklist', 'multipicklist', 'reference', 'string', 'url', 'currency', 'text', 'DocFolderPicklist'];
	...
	registerStarToggle: function () {
		...
		listViewContainer.on('click', '.markStar', function (e) {
			...
			if (element.hasClass('active')) {
				params.value = 0;
				element.removeClass('text-danger').addClass('ti-star-o');
			} else {
				params.value = 1;
				element.removeClass('ti-star-o').addClass('text-danger');
			}
...
//      
		...
		//END
		this.registerCollapsedFunction();
	...
	registerDropdownPosition: function (container) {
		...
		jQuery('.table-actions').on('click', '.dropdown', function (e) {
			...
			if($(".fixedListDropDown").length>0){
				
				if($(".fixedListDropDown").attr("parentrowid")==dropdown.closest("tr").data("id")){
					$(".fixedListDropDown").remove();
					return;
				}
				else $(".fixedListDropDown").remove();
			}
			...
			fixed_dropdown_menu.addClass("fixedListDropDown");
			fixed_dropdown_menu.attr("parentrowid",dropdown.closest("tr").data("id"));
			...
			$('#page').scroll(function () {
				var rowTop = $("tr[data-id='"+$(".fixedListDropDown").attr("parentrowid")+"'] .table-actions .dropdown").offset().top+20;
				console.log(rowTop+"tr[data-id='"+$(".fixedListDropDown").attr("parentrowid")+"']");
				$(".fixedListDropDown").css("top",rowTop+"px");
			});	
	...
	registerCollapsedFunction: function(){
		...
		$(document).on('click','.col',function(){
	        ...
	        if(filters.find('#sharedList').length > 0){
...
$(function(){
        ...
        filters.on('click','.deleteFilter',function(e){
            var element = jQuery(e.currentTarget);
            if(typeof element.data('url') == "undefined") return;
            element.trigger('post.DeleteFilter.click',{'url':element.data('url')});
        });
        
        filters.on('click','.toggleDefault',function(e){
            var element = jQuery(e.currentTarget);
