...
{strip}
		...
		<select class="{if empty($NOCHOSEN)}select2{/if} col-lg-12" name="columnname">
			...
			{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE}
				...
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
					...
					{/if}
					{if  $FIELD_MODEL->getFieldDataType() eq 'userRole'}
						{assign var=PICKLIST_VALUES value=$FIELD_MODEL->getAllRoles()}
						{$FIELD_INFO['picklistvalues'] = array_flip($PICKLIST_VALUES)}
	...
	<span class="hide">
	 ...
	 <div class="col-lg-1 col-md-1 col-sm-1">
		<i class="deleteCondition ti-trash text-danger cursorPointer" title="{vtranslate('LBL_DELETE', $MODULE)}"></i>
	</div>
</div>
{/strip}