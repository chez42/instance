        ...
        {if $ACCESSIBLE_USERS|@count gt 1}
            <select class="select2 widgetFilter col-lg-3 reloadOnChange" name="type">
                ...
                {foreach key=USER_ID from=$ACCESSIBLE_USERS item=USER_NAME}
                    <option value="{$USER_ID}" {if $TYPE }{if $USER_ID eq $TYPE}  selected {/if}
                    {else} {if $USER_ID eq $CURRENT_USER_ID}  selected {/if}{/if}>
                ...
                <div class="col-lg-7">
                        {if $COMMENTS_MODULE_MODEL->isPermitted('DetailView')}
                            <label class="radio-group cursorPointer">
                                <input type="radio" name="historyType" class="widgetFilter reloadOnChange cursorPointer" value="comments"{if $HISTORY_TYPE eq 'comments'}checked=""{/if} /> {vtranslate('LBL_COMMENTS', $MODULE_NAME)}
                            </label><br>
                        {/if}
                        <label class="radio-group cursorPointer">
                            <input type="radio" name="historyType" class="widgetFilter reloadOnChange cursorPointer" value="updates"{if $HISTORY_TYPE eq 'updates'}checked=""{/if} /> 
                            <span>{vtranslate('LBL_UPDATES', $MODULE_NAME)}</span>
                        </label><br>
                        <label class="radio-group cursorPointer">
                            <input type="radio" name="historyType" class="widgetFilter reloadOnChange cursorPointer" value="all" {if $HISTORY_TYPE eq 'all'}checked=""{/if} /> {vtranslate('LBL_BOTH', $MODULE_NAME)}
                        </label>
                </div>
            </div>
        </div>
            ...
            <div class="col-sm-12">
                ...
                <span class="col-lg-7">
                    <div class="input-daterange input-group dateRange widgetFilter" id="datepicker" name="modifiedtime">
                        <input type="text" class="input-sm form-control" name="start" value='{$START}' style="height:30px;"/>
                        <span class="input-group-addon">to</span>
                        <input type="text" class="input-sm form-control" name="end" value='{$END}'style="height:30px;"/>
                    </div>
                </span>
            </div>
        </div>
