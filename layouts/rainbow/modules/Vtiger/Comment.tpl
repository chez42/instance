						...
						<div class="media">
							<div class="media-left title" id="{$COMMENT->getId()}">
								...
								<div class="col-lg-2 recordImage commentInfoHeader" data-commentid="{$COMMENT->getId()}" data-parentcommentid="{$COMMENT->get('parent_comments')}" data-relatedto = "{$COMMENT->get('related_to')}">
									...
									{if !empty($IMAGE_PATH)}
										<img src="{$IMAGE_PATH}" width="50px" height="50px" align="left">
									{else}
							...
							<div class="media-body">
								<div class="comment" style="line-height:1;">
									<span class="creatorName" style="color:#398bf7">
										{$CREATOR_NAME}
									</span>&nbsp;&nbsp;
									...
									<div class="editedStatus" name="editStatus">
										...
										{if $COMMENT->getCommentedTime() neq $COMMENT->getModifiedTime()}
											<p class="text-muted cursorDefault"><small><em>{vtranslate('LBL_MODIFIED',$MODULE_NAME)}</em></small>&nbsp;<small title="{Vtiger_Util_Helper::formatDateTimeIntoDayString($COMMENT->getModifiedTime())}" class="commentModifiedTime">{Vtiger_Util_Helper::convertDateTimeIntoUsersDisplayFormat($COMMENT->getModifiedTime())}</small></p>
										{/if}
									</div>
