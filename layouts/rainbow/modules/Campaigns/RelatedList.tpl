							...
							<div class="btn-group">
								...
								{else}
									...
									<button type="button" module="{$RELATED_MODULE_NAME}"  class="btn addButton btn-primary
										{if $IS_SELECT_BUTTON eq true} selectRelation {/if} {if $IS_SEND_EMAIL_BUTTON eq true} btn-warning sendEmail {/if}"
										...
										{if $IS_SEND_EMAIL_BUTTON eq true} disabled="disabled" {/if}>{if ($IS_SELECT_BUTTON neq true) && ($IS_SEND_EMAIL_BUTTON neq true)}<i class="ti-plus"></i>{/if}&nbsp;{$RELATED_LINK->getLabel()}</button>
								{/if}
							</div>
						{/foreach}&nbsp;
					</div>
							...
							<tr class="listViewHeaders">
								...
								{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
									<th class="nowrap">
										...
										{if $COLUMN_NAME eq $HEADER_FIELD->get('column')}
											<a href="#" class="removeSorting"><i class="ti-trash"></i></a>
										{/if}
									</th>
								{/foreach}
								<th class="nowrap">
							...
							<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'>
								...
								<td style="width:100px">
									<span class="actionImages">
										<a name="relationEdit" data-url="{$RELATED_RECORD->getEditViewUrl()}" href="javascript:void(0)"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="ti-pencil"></i></a> &nbsp;&nbsp;
										{if $IS_DELETABLE}
