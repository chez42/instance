		                                ...
		                                {foreach item="CUSTOM_VIEW" from=$GROUP_CUSTOM_VIEWS name="customView"}
											...
											<li style="font-size:13px;" class='listViewFilter {if $VIEWID eq $CUSTOM_VIEW->getId() && ($CURRENT_TAG eq '')} active {/if}{*if $smarty.foreach.customView.iteration gt 10} {assign var=count value=1} {/if} {else if $smarty.foreach.customView.iteration gt 10} filterHidden hide{/if*} '> 
												...
												{append var="CUSTOM_VIEW_NAMES" value=$VIEWNAME}
		                                         <a class="filterName listViewFilterElipsis" href="{$LISTVIEW_URL|cat:'&viewname='|cat:$CUSTOM_VIEW->getId()|cat:'&app='|cat:$SELECTED_MENU_CATEGORY}" oncontextmenu="return false;" data-filter-id="{$CUSTOM_VIEW->getId()}" title="{$VIEWNAME|@escape:'html'}">{$VIEWNAME|@escape:'html'}{if $CUSTOM_VIEW->get('status') eq '0'}  {$MODULE}{/if}</a> 
		                                            <div class="pull-right">
		                                                ...
		                                                {if $CUSTOM_VIEW->isMine() and $CUSTOM_VIEW->get('viewname') neq 'All' || $CURRENT_USER_MODEL->isAdminUser()}
		                                                    ...
		                                                    {/if} data-editable="{if $CUSTOM_VIEW->isEditable()|| $CURRENT_USER_MODEL->isAdminUser()}true{else}false{/if}" 
		                                                            {if $CUSTOM_VIEW->isEditable() || $CURRENT_USER_MODEL->isAdminUser()} data-editurl="{$CUSTOM_VIEW->getEditUrl()}{/if}" {if $CUSTOM_VIEW->isDeletable()} {if $SHARED_MEMBER_COUNT eq 1 or $LIST_STATUS eq 3} data-shared="1"{/if} data-deleteurl="{$CUSTOM_VIEW->getDeleteUrl()}"{/if}
		                                                           {/if}
		                                                          toggleClass="fa {if $IS_DEFAULT}ti-check-alt{else}ti-check{/if}" data-filter-id="{$CUSTOM_VIEW->getId()}" 
		                                                          data-is-default="{$IS_DEFAULT}" data-defaulttoggle="{$CUSTOM_VIEW->getToggleDefaultUrl()}" data-default="{$CUSTOM_VIEW->getDuplicateUrl()}" data-isMine="{if $CUSTOM_VIEW->isMine() || $CURRENT_USER_MODEL->isAdminUser()}true{else}false{/if}">
		                                                    </span>
		                                                     </span>
		                                                </div>
		                                            </li>
