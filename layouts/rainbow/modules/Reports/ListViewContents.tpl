...
{strip}
	<div class="col-sm-12 col-xs-12 ">
		...
		<div class="essentials-toggle" title="{vtranslate('LBL_LEFT_PANEL_SHOW_HIDE', 'Vtiger')}">
			<span class="essentials-toggle-marker {if $LEFTPANELHIDE eq '1'}ti-angle-right{else}ti-angle-left{/if} cursorPointer"></span>
		</div>
												...
												<select class="select2 listSearchContributor report-type-select" name="{$LISTVIEW_HEADER_KEY}" multiple data-fieldinfo='{$FIELD_INFO|escape}'>
													{foreach item=PICKLIST_LABEL key=PICKLIST_KEY from=$PICKLIST_VALUES}
														{if $PICKLIST_LABEL eq 'Chart'}
															{assign var="ICON_CLASS" value='ti-pie-chart'}
														{elseif $PICKLIST_LABEL eq 'Detail'}
															{assign var="ICON_CLASS" value='ti-bar-chart'}
														{/if}
											...
											<span class="value textOverflowEllipsis">
												{if $LISTVIEW_HEADERNAME eq 'reporttype'}
													{if $LISTVIEW_ENTRY_VALUE eq 'summary' || $LISTVIEW_ENTRY_VALUE eq 'tabular'}
														<center title="{vtranslate('LBL_DETAIL_REPORT', $MODULE)}"><span class='ti-bar-chart' style="font-size:17px;"></span></center>
													{elseif $LISTVIEW_ENTRY_VALUE eq 'chart'}
														<center title="{vtranslate('LBL_CHART_REPORT', $MODULE)}"><span class='ti-pie-chart' style="font-size:1.7em;"></span></center>
													{/if}
