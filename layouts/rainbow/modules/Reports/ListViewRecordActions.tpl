...
{strip}
    ...
    <div class="table-actions reportListActions">
        ...
        {if $REPORT_TYPE eq 'chart'}
            <span>
                <a class="quickView ti-zoom-in icon action" title="{vtranslate('LBL_QUICK_VIEW', $MODULE)}"></a>
            </span>
        {/if}
        ...
        {if $LISTVIEW_ENTRY->isEditableBySharing()}
            <span class="more dropdown action">
                <span href="javascript:;" class="dropdown-toggle" data-toggle="dropdown">
                    <i class="ti-info-alt icon"></i></span>
        ...
        <div class="btn-group inline-save hide">
            <button class="button btn-success save" name="save"><i class="ti-check"></i></button>
            <button class="button btn-danger cancel" name="Cancel"><i class="ti-close"></i></button>
        </div>
    </div>
{/strip}