					...
					<thead>
						<tr class="listViewHeaders">
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								{else}
									<th class="nowrap">
									{if $HEADER_FIELD->get('column') eq "access_count" or $HEADER_FIELD->get('column') eq "idlists" or  $HEADER_FIELD->get('name') eq "related_to" or $HEADER_FIELD->get('name') eq "to_number" }
						...
						<tr class="searchRow">
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								<th>
									{if $HEADER_FIELD->get('column') eq 'time_start' or $HEADER_FIELD->get('column') eq 'time_end' or $HEADER_FIELD->getFieldDataType() eq 'reference' or $HEADER_FIELD->getName() eq 'related_to' or $HEADER_FIELD->getName() eq 'to_number'}
									{else}
								...
								<td class="relatedListEntryValues" title="{strip_tags($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME))}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
									<span class="value {if $HEADER_FIELD->getFieldDataType() neq 'email' && $HEADER_FIELD->getFieldDataType() neq 'phone'} textOverflowEllipsis{/if}">
										{if $RELATED_MODULE_NAME eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
											...
											{else if $HEADER_FIELD->getName() eq 'related_to'}	
												{assign var=relatedRecords value=RingCentral_Module_Model::getRelatedRecords($RELATED_RECORD->getId())}
												{$relatedRecords}
											{else if $HEADER_FIELD->getName() eq 'to_number'}	
												{assign var=relatedNo value=RingCentral_Module_Model::getRelatedNos($RELATED_RECORD->getId())}
												{$relatedNo}
											{else}
