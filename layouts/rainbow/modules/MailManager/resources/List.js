...
Vtiger_List_Js("MailManager_List_Js", {}, {
	...
	loadFolders : function(folder, accountid) {
		...
		app.request.post({"data" : params}).then(function(error, responseData) {
			...
			if(error === null) {
				...
				// Don't Hide loader here so user won't confuse
	...
	registerFolderClickEvent : function() {
		var self = this;
		var container = self.getContainer();
		container.find('.mm_folder:not(i)').click(function(e) {
			...
			if(jQuery(document).width()<768)
				$('#modnavigator > div, .modules-menu').addClass('hidden-xs hidden-sm').css('width','100%');
			}else{
		...
		if (typeof mailids !== 'undefined') {
			...
			app.request.post({"data" : params}).then(function(error, responseData) {
				for(var k in responseData){
					...
					var messageEle = jQuery('[data-messageid="'+k+'"]');
					...
					app.request.post({data : params}).then(function(err,data) {
						...
						if(data.status) {
							...
							for(var i = 0; i < msgNos.length; i++) {
								container.find('[data-messageid="'+msgNos[i]+'"]').remove();
							}
	...
	markMessageUnread : function(msgNos) {
		...
		if(typeof msgNos == "object") {
			for(var i = 0; i < msgNos.length; i++) {
				var msgNo = msgNos[i];
				var msgEle = container.find('[data-messageid="'+msgNo+'"]');
	...
	getUnreadCountByMsgNos : function(msgNos) {
		...
		var container = self.getContainer();
		for(var i = 0; i < msgNos.length; i++) {
			var isRead = parseInt(container.find('[data-messageid="'+msgNos[i]+'"]').data('read'));
			if(isRead == 0) {
				count++;
			}
		}
	...
	loadContentsInIframe : function(element) {
		...
		jQuery("#mailPreviewContainer, #mails_container").css("width","100%");
		jQuery("#mails_container").hide();		
		jQuery("#mailPreviewContainer").show();
		jQuery("#mobileMailSwitch").remove();
		
		jQuery("#mailManagerActions").prepend('<div style="width:100%; text-align: center; padding:10px" id="mobileMailSwitch"><div class="btn-group listViewActionsContainer" role="group" aria-label="..."><button type="button" class="btn btn-warning" href="javascript:void(0);" onclick="jQuery(\'#mails_container\').show(); jQuery(\'#mailPreviewContainer\').hide();" title="Mail List">Mail List</button></div></div>');
	...
	highLightMail : function(msgNo) {
		...
		var selectedMailEle = container.find('[data-messageid="'+msgNo+'"]');
	...
	registerMailPaginationEvent : function() {
		...
		container.find('.mailPagination').click(function(e) {
			...
			app.request.post({data : params}).then(function(err, data) {
				app.helper.hideProgress();
				var uiContent = data.ui;
				container.find('[data-messageid="'+msgNo+'"]').addClass('mmReadEmail');
				container.find('[data-messageid="'+msgNo+'"]').data('read', "1");
				var nameSubject = container.find('[data-messageid="'+msgNo+'"]').find('.nameSubjectHolder').find('strong').html();
				container.find('[data-messageid="'+msgNo+'"]').find('.nameSubjectHolder').html(nameSubject);
				container.find('#mailPreviewContainer').html(uiContent);
				self.registerMailDeleteEvent();
				self.registerForwardEvent();
				self.registerReplyEvent();
		...
		container.find('#mmDelete').click(function() {
			...
			app.helper.showPromptBox({'message' : app.vtranslate('LBL_DELETE_CONFIRMATION')}).then(function() {
				...
				app.request.post({data : params}).then(function(err,data) {
					app.helper.hideProgress();
					if(data.status) {
						container.find('[data-messageid="'+msgNo+'"]').remove();
				...
				app.helper.showPromptBox({'message' : app.vtranslate('LBL_DELETE_CONFIRMATION')}).then(function() {
					...
					app.request.post({data : params}).then(function(err,data) {
						...
						if(data.status) {
							...
							for(var i = 0; i < msgNos.length; i++) {
								var nextDiv = container.find('[data-messageid="'+msgNos[i]+'"]').next("div");
								container.find('[data-messageid="'+msgNos[i]+'"]').remove();
							}
							var openedMsgNo = container.find('#mmMsgNo').val();
							if(jQuery.inArray(openedMsgNo, msgNos) !== -1) {
		...
		moveFolder.on('click', 'a', function(e){
			...
			if(msgNos.length <= 0) {
				...
				app.request.post({data : params}).then(function(err,data) {
					...
					if(data.status) {
						app.helper.showSuccessNotification({'message': app.vtranslate('JSLBL_MAIL_MOVED')});
						var unreadCount = thisInstance.getUnreadCountByMsgNos(msgNos);
