	...
	registerLeavePageWithoutSubmit: function(form) {
		...
		if(app.getTurnOfConfirmation() != 1){
			window.onbeforeunload = function(e) {
				if (initialFormData != form.serialize() && form.data('submit') != "true" && $("#notifyLogout").length<1) {
	...
	registerModalDismissWithoutSubmit: function(form) {
		...
		jQuery('.modal .close,.modal .cancelLink').click(function(e) {
			if (initialFormData != form.serialize() && form.data('submit') != "true") {
				app.helper.showPromptBox({'message' : app.vtranslate("JS_CHANGES_WILL_BE_LOST") +' '+ app.vtranslate('JS_WISH_TO_PROCEED')}).then(function(){
					window.onbeforeunload = null;
					...
					if (result) {
						if(result.length){
							var userParams = {
								module: "Users",
								action: "SaveAjax",
								mode: "updateTurnOfConfirmation",
								'turn_of_confirmation' : 1,
								'userid' : app.getUserId()
							};
		        	  		app.request.post({'data': userParams}).then(function(){
		        	  			_USERMETA.turnOfConfirmation = "1";
		        	  		});
		        	  	}
						aDeferred.resolve();
					} else {
