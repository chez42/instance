...
/****************************** 09-Aug-2018 Changes *************************************/
		...
		if(!result['isRelateListView']) {
			form.find('.relatedList').addClass('disabled');
			meTitle = app.vtranslate('JS_SHOW_THIS_FIELD_IN', app.vtranslate('JS_RELATED_LIST'));
			
			//form.find('[name="related_list_view"]').filter(':checkbox').attr('checked', true);
		} else {
			form.find('.relatedList').removeClass('disabled');
			meTitle = app.vtranslate('JS_HIDE_THIS_FIELD_IN', app.vtranslate('JS_MASS_EDIT'));
		}
		
		if(result['isRelateListViewDisabled']) {
			form.find('.relatedList').addClass('cursorPointerNotAllowed');
			form.find('[data-name="related_list_view"]').attr('readonly', 'readonly');
			//form.find('[name="related_list_view"]').filter(':checkbox').attr('readonly', 'readonly').addClass('optionDisabled');
		} else {
			form.find('.relatedList').addClass('cursorPointer');
			form.find('[data-name="related_list_view"]').removeAttr('readonly');
		}
