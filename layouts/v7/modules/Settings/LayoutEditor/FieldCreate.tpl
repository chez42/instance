	...
	<div class="modal-dialog createFieldModal modelContainer {if !$IS_FIELD_EDIT_MODE}hide{/if}">
		...
		<div class="modal-content">
			...
			{*Change For 09-Aug-2018 *}				
								...
								<div class="form-group col-sm-6">
									<label class="control-label fieldLabel col-sm-7">
										<img src="{vimage_path('MassEdit.png')}" height=14 width=14/> &nbsp; {vtranslate('LBL_ENABLE_RELATED_LIST_FIELD',$QUALIFIED_MODULE)}
									</label>
									<div class="controls col-sm-5">
										<input type="hidden" name="related_list_view" value="0"/>
										<label class="checkbox" style="margin-left: 9%;">
											<input type="checkbox" class="{if $FIELD_MODEL->isRelatedTabViewOptionDisabled()} cursorPointerNotAllowed {else} cursorPointer{/if}" name="related_list_view" value="1" {if $FIELD_MODEL->get('relatedlistview') eq '1'}checked="checked"{/if}
												{if $FIELD_MODEL->isRelatedTabViewOptionDisabled() || $IS_NAME_FIELD}readonly="readonly"{/if} />
										</label>
									</div>
								</div>
