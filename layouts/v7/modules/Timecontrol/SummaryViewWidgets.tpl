...
{strip}
	{foreach item=DETAIL_VIEW_WIDGET from=$DETAILVIEW_LINKS['DETAILVIEWWIDGET']}
		...
		{elseif ($DETAIL_VIEW_WIDGET->getLabel() eq 'Call Record Details')}
			{assign var=CALL_WIDGET_MODEL value=$DETAIL_VIEW_WIDGET}
		{elseif ($DETAIL_VIEW_WIDGET->getLabel() eq 'Red Flags')}
			{assign var=REDFLAG_WIDGET_MODEL value=$DETAIL_VIEW_WIDGET}
		{/if}
	{/foreach}
