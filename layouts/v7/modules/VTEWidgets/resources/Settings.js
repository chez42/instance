            ...
            app.request.get({'url':url}).then(function(err,resp) {
                ...
                if(err === null) {
                    ...
                    if(type == 'RelatedModule'){
                        ...
                        form.find("select[name='relatedmodule']").change(
                            function(e){
                                thisInstance.loadFilters(form);
...
jQuery(document).ready(function() {
    ...
    $(".connectedSortable").each(function (index) {
        ...
        if(uiWdiget<countWidget ||(uiWdiget==countWidget && hasWidget>1)){
            ...
            if(temp.length==0) {
                var widgetBlock = $(this).find('.blocksSortable').append("<div class='blockSortable ui-sortable-handle dummyRow'><span class='dragUiText'>Drap & Drop Widget</span></div> ");
            }
        }
    });
});
