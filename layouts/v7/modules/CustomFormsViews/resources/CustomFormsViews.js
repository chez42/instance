...
Vtiger.Class("CustomFormsViews_Js",{
    ...
    getConfig: function() {
        ...
        if(sPageURL.indexOf('&relatedModule=') != -1) {
            ...
            if (btnGroup.length > 0 && btnGroup.find("ul").length == 0){
                app.helper.showProgress();
                app.request.post(actionParams).then(
                    function(err,data){
                        if(err === null) {
                            ...
                            if (!jQuery.isEmptyObject(formsList) && formsList.hasOwnProperty('result') != true) {
                                ...
                                var customButtons = '<button id="'+btnID+'" type="button" class="btn btn-default module-buttons dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><span class="caret"></span></div>' + addText + '</button><ul class="dropdown-menu">';
                                jQuery.each(formsList, function (i, v) {
