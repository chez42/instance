    ...
    <table class="table table-bordered table-condensed listViewEntriesTable">
        ...
        {foreach item=LISTVIEW_ENTRY key=RECORDID from=$LISTVIEW_ENTRIES}
            <tr class="listViewEntries" data-url="index.php?module=CustomFormsViews&view=EditView&parent=Settings&record={$RECORDID}">
                <td width="30%" style="vertical-align:top !important;cursor: pointer;" nowrap class="medium">{vtranslate($LISTVIEW_ENTRY['module'], $LISTVIEW_ENTRY['module'])}</td>
                ...
                <td nowrap class="{$WIDTHTYPE}">
                    <div class="actions pull-right">
                            <span class="actionImages">
                                    <a href='javascript: void(0);' class="editRecordButton" data-url="index.php?module=CustomFormsViews&view=EditView&parent=Settings&record={$RECORDID}"><i title="{vtranslate('LBL_EDIT')}" class="glyphicon glyphicon-pencil alignMiddle"></i></a>&nbsp;
                                    <a class="deleteRecordButton" data-id="{$RECORDID}"><i title="{vtranslate('LBL_DELETE')}" class="glyphicon glyphicon-trash alignMiddle"></i></a>
                            </span>
                    </div>
                </td>
            </tr>
