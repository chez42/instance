							...
							<span class="fieldValue">
								<span class="value">
									{if ($LISTVIEW_HEADER->isNameField() eq true or $LISTVIEW_HEADER->get('uitype') eq '4') and $MODULE_MODEL->isListViewNameFieldNavigationEnabled() eq true }
										{assign var="MODSECURITYID" value="{ModSecurities_Module_Model::GetCrmidFromSymbol($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))}"}
										{if $MODSECURITYID neq 0}
											{assign var="MODSECURITY_RECORD_MODEL" value=Vtiger_Record_Model::getInstanceById($MODSECURITYID)}
											<a target = '_blank' href="{$MODSECURITY_RECORD_MODEL->getDetailViewUrl()}&app={$SELECTED_MENU_CATEGORY}">{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}</a>
										{else}
									...
									{else}
										...
										{else if $LISTVIEW_HEADER->getFieldDataType() eq 'multipicklist'}
											...
											{/foreach}
										{else if $LISTVIEW_HEADER->getFieldName() eq 'account_number'}	
										
											{assign var="PORTFOLIOID" value="{PortfolioInformation_Module_Model::GetCrmidFromAccountNumber($LISTVIEW_ENTRY_VALUE)}"}
											{if $PORTFOLIOID neq 0}
												{assign var="PORTFOLIO_RECORD_MODEL" value=Vtiger_Record_Model::getInstanceById($PORTFOLIOID)}
												<a target = '_blank' href="{$PORTFOLIO_RECORD_MODEL->getDetailViewUrl()}&app={$SELECTED_MENU_CATEGORY}">{$LISTVIEW_ENTRY_VALUE}</a>	
											{else}
