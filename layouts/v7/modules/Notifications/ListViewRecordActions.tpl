...
<div class="table-actions">
    ...
    <span class="more dropdown action">
        ...
        <ul class="dropdown-menu">
			...
			{assign var="ACCEPTED" value=$moduleModel->getAcceptedValue($LISTVIEW_ENTRY->getRaw('related_to'))}
			{if $CURRENT_USER_MODEL->id eq $LISTVIEW_ENTRY->getRaw('smownerid') && getSalesEntityType($LISTVIEW_ENTRY->getRaw('related_to')) eq 'Calendar' && !$ACCEPTED}
				<li>
        			<a data-event="accept" data-eventid="{$LISTVIEW_ENTRY->getRaw('related_to')}" data-notifyId="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" class="eventAction noificationlist">
        				<i title="Accept" class="fa fa-check-circle pull-right eventAction" style="color:green;"></i> Accept
    				</a>
        		</li>
        		<li>
        			<a data-event="reject" data-eventid="{$LISTVIEW_ENTRY->getRaw('related_to')}" data-notifyId="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" class="eventAction noificationlist">
        				<i title="Reject" class="fa fa-times-circle pull-right" style="color:red;"></i> Reject
    				</a>
        		</li>
			{/if}
        </ul>
