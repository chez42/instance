...
{strip}
<select class="{if empty($BASE)}select2{/if}"   {if $IS_DEFAULT eq 1} disabled="disabled" {/if}
        name="{if $SOURCE_MODULE == 'Products'}productField{else}serviceField{/if}{$INDEX}"
        data-rule-required="true" aria-required="true" style="width: 100%;">
    <option value="">{vtranslate('LBL_SELECT_FIELD')}</option>
    {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE}
        {vtranslate($BLOCK_LABEL, $SOURCE_MODULE)}{$SOURCE_MODULE}
        <optgroup label='{vtranslate($BLOCK_LABEL, $SOURCE_MODULE)}'>
            {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
                {if $FIELD_MODEL->getFieldDataType() neq 'reference' || $FIELD_MODEL->get('uitype') == 10}
                    ...
                    <option value="{$FIELD_MODEL->$columnNameApi()}" data-fieldtype="{$FIELD_MODEL->getFieldType()}" data-field-name="{$FIELD_NAME}"
                            {if $CURRENT_FIELD AND $CURRENT_FIELD == $FIELD_MODEL->getName()}
