...
Vtiger_Edit_Js("Quoter_Js", {
        ...
        if (record) {
            ...
            AppConnector.request(params).then(
                function (data) {
                    ...
                    if (record && level < 3) {
                        var newGroupButton = '<div class="btn-group">' +
                            '<button type="button" class="btn btn-default addButton selectRelationNew" data-modulename="Products">&nbsp;Select Products</button>' +
                            '</div>' +
                            '<div class="btn-group">' +
                            '<button type="button" class="btn btn-default addButton " data-url="index.php?module=Products&amp;view=Edit&amp;sourceModule=Products&amp;sourceRecord=' + record + '&amp;relationOperation=true" name="addButton">' +
                            '<i class="fa fa-plus"></i>&nbsp;Add Product' +
                            '</button>' +
                            '</div>';
                        jQuery('.relatedHeader > div > div:first').html(newGroupButton);
    ...
    getTaxDiv: function (taxObj, parentRow) {
        ...
        if (!jQuery.isEmptyObject(taxObj)) {
            ...
            for (var taxName in taxObj) {
                ...
                contentDiv += '<tr>' +
                    '<td>' +
                    '<input type="text" name="' + taxName + '_percentage' + rowNumber + '" data-rule-positive=true data-rule-inventory_percentage=true  id="' + taxName + '_percentage' + rowNumber + '" value="' + taxInfo.taxpercentage + '" class="smallInputBox taxPercentage">&nbsp;%' +
                    '</td>' +
     ...
     */
    qcalculateTaxForLineItem: function (lineItemRow) {
        var numberOfDecimal = parseInt(jQuery('.numberOfCurrencyDecimal').val());
        ...
        lineItemTable.on('click', '.individualTax1', function (e) {
            ...
            var callBackFunction = function(element, data) {
                data.find('.popoverButton').on('click', function(e){
                    ...
                    if (validate) {
                        element.popover('destroy');
                        self.qcalculateTaxForLineItem(lineItemRow);
...
jQuery(document).ready(function () {
    ...
    } else if (targetModule == 'Products' && targetView == 'Detail') {
        var currentRelatedTab = jQuery('.related-tabs .more-tab.moreTabElement').filter('.active');
        if (currentRelatedTab.data('label-key') == 'Product Bundles') {
