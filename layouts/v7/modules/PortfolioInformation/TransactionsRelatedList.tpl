				...
				<table class="table table-bordered listViewEntriesTable">
					<thead>
					 ...
					 <tr>
						...
						{/foreach}
						<td>
							<button class="btn listSearch" data-trigger="listSearch">{vtranslate('LBL_SEARCH', $MODULE )}</button>
						</td>
				  </tr>
						...
						<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								<td class="{$WIDTHTYPE}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
									...
									{else if $HEADER_FIELD->get('uitype') eq '72' or $HEADER_FIELD->get('uitype') eq '71'}
									 
										{assign var=FieldValue value=$RELATED_RECORD->get($RELATED_HEADERNAME)}
										{assign var=currencySymbol value=$RELATED_RECORD->get('currencySymbol')}
										{if $FieldValue neq ''}   	   
											{assign var=CURRENCY_SYMBOL_PLACEMENT value={$USER_MODEL->get('currency_symbol_placement')}}
											{if $CURRENCY_SYMBOL_PLACEMENT eq '1.0$'}
												{$FieldValue}{$currencySymbol}
											{else}
												{$currencySymbol}{$FieldValue}	
											{/if}
										{/if}
									{else}
						...
						<tbody>
							<tr>
								<td>
									...
									{vtranslate('LBL_EQ_ZERO')} {vtranslate($RELATED_MODULE_NAME, $RELATED_MODULE_NAME)} {vtranslate('LBL_FOUND')}.
								</td>
							</tr>
						</tbody>
					</table>
