...
{else if $FIELD_NAME eq 'template_id'}
	...
	{foreach item=TEMPLATE_NAME key=TEMPLATE_VALUE from=$CALENDAR_TEMPLATES}
		<option value="{$TEMPLATE_VALUE}" {if $FIELD_MODEL->get('fieldvalue') eq $TEMPLATE_VALUE} selected {/if}>{$TEMPLATE_NAME}</option>
	{/foreach}
	</select>
{/if}
{/strip}