...
Vtiger_List_Js("Settings_Users_List_Js",{
	...
	*/
	...
	massDeleteRecords : function(deleteRecordActionUrl) {
    	...
    	if(listSelectParams){
    		...
    		if(postData.filter_inactive_status == '1'){
    			 var message = app.vtranslate('LBL_DELETE_USER_PERMANENT_CONFIRMATION');
    			 var mode = "permanent";
    		} else	
    			var message = app.vtranslate('LBL_DELETE_USER_CONFIRMATION');
    		...
    		app.helper.showConfirmationBox({'message' : message}).then(function(data) {
    			...
    			var data = app.convertUrlToDataParams(deleteRecordActionUrl);
    			postData = jQuery.extend(postData, data);
    			postData = jQuery.extend(postData, listSelectParams);
               
	    		var cvId = listInstance.getCurrentCvId();
				
	            if(postData.viewname === 'undefined' || typeof postData.viewname === 'undefined'){
            		...
            		app.request.post({'url':deleteRecordActionUrl}).then(
    					function(err, data){
    						...
    						if(err === null){
    							...
    							var form = jQuery("#massDeleteUser");
    							form.on('submit', function(e){
    								e.preventDefault();
    								var transferUserId = form.find('[name="tranfer_owner_id"]').val();
									var cvId = listInstance.getCurrentCvId();
									...
									params.permanent = jQuery('[name="deleteUserPermanent"]:checked', form).val();
    ...
    triggerMassAction: function (massActionUrl) {
		...
		if (listSelectParams) {
			...
			app.request.post({'data': postData}).then(
					function (err, data) {
						if (data == true) {
							app.helper.showSuccessNotification({'message': "Status Change Successfully!"});
							var listInstance = new Settings_Users_List_Js;
	    					var listParams = listInstance.getListViewParams();
	    					listInstance.loadListViewRecords(listParams);
	    					jQuery('#deSelectAllMsgDiv').trigger('click');
						}else{
							app.helper.showErrorNotification({'message': "Try Again"});
						}
						app.helper.hideProgress();
					}
			);
...
//			'search_value' : self.getListViewContainer().find('.userFilter').find('button.btn-primary').data('searchvalue')
	...
	loadListViewRecords : function(urlParams) {
		...
		app.request.get({data:usersListParams}).then(function(err, res){
			...
			app.helper.hideProgress();
			thisInstance.markSelectedIdsCheckboxes();
	...
	getListSearchParams : function(includeStarFilters) {
		...
		if(searchParams.length > 0) {
			...
			var listSearchParams = listSearchParams;
		}
	...
	registerEvents : function() {
		...
		this.changeButtonTitle(); //16-Aug-2018 
        ...
        jQuery.each(scrollContainers,function(key,scroll){
            ...
            var listcontentHeight = 250;//scroll.find(".list-menu-content").height();
...
/*************************  16-Aug-2018 Change for filter  **************************/	
	changeButtonTitle : function() {
		var self = this;
		var name = $(document).find('.listViewFilter.active');
		if(name.length){
			$('#selected').text(name.text());
		}
