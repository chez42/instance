...
Vtiger_Detail_Js("Users_Detail_Js",{
	...
	triggerExchangeSubscription : function(requestUrl, triggerElement){
		
		var currentElement = jQuery(triggerElement);
		
		var url = currentElement.attr("onClick");
		
		var buttonText = currentElement.html();
		
		app.helper.showProgress(app.vtranslate('JS_PLEASE_WAIT'));
		
		app.request.post({'url': requestUrl}).then(function (err, response) {
				...
				if(response.success){
					...
					var currentMode = decodeURIComponent(matches[2].replace(/\+/g, " "));
					
					var newMode = "";
					
					if(currentMode == 'Subscribe')
						newMode = "UnSubscribe";
					else if(currentMode == 'UnSubscribe')
						newMode = "Subscribe";
						
					var newUrl = url.replace('mode='+currentMode, 'mode='+newMode);
					
					buttonText = buttonText.replace(currentMode,newMode);
					
					currentElement.attr("onClick", newUrl);
