...
(function ($) {
    ...
    $.Class("GoogleRadiusMaps_Js", {
		...
		autoDrawByColor: function(latLng, pinColor, focusInstance, addressInfo, infoWindow, i) {
			...
			$('#colorpickerFieldcolorpickerField' + i).css("border", "1px solid #" + pinColor);
		},
		...
		autoSaveColor: function(i, pinColor) {
			...
			app.request.get(Params_1).then(function(err,result) {
                if(err === null) {
                    var checkPin = result.color_picker_fields.fields.color_picker_field;
                    var Params_2 = "";
                    if (typeof checkPin !== 'undefined' && checkPin.length == 6) {
                        var pId = result.color_picker_fields.fields.id;
                        Params_2 = {
                            "type": "GET",
                            "url": 'index.php?module=GoogleRadiusMaps&action=ActionAjax',
                            "dataType": "json",
                ...
                focusInstance.getLatLngByAddress(focusInstance.props.loadedAddresses, function (addressInfo, theAddressInfo) {
                    ...
                    if (focusInstance.props.currentRadius.getBounds().contains(latLng)) {
                        ...
                        var popupInfo = 
							...
							if(typeof focusInstance.props.loadedAddressesOfRecord[i] == 'object' 
                                && focusInstance.props.loadedAddressesOfRecord[i].hasOwnProperty('street') 
                                && isEmpty(focusInstance.props.loadedAddressesOfRecord[i].street)) {
                        ...
                        app.request.get(Params).then(function(err,result) {
							
							//if(typeof result !== 'undefined' && result != '') {
                            if(err === null){
                                if (typeof result.color_picker_fields == 'object' && result.color_picker_fields.hasOwnProperty('color_picker_field')){
                                    binColor = result.color_picker_fields.fields.color_picker_field;
        ...
        registerEvents: function() {
            ...
            this.checkEnable(function(result){
                ...
                $('.listViewActionsContainer')
                    ...
                    '<img src="layouts/v7/modules/GoogleRadiusMaps/resources/images/google_maps.png" style="width:30px; margin-left:5px"/></a></div>');
                ...
                thisInstance.props.currentUserAddress = result.current_user_address;
                thisInstance.props.currentModuleFields = result.module_fields;
                thisInstance.props.currentCompanyAddress = result.company_address;

                // Add custom style
                $('head').append('<link href="layouts/v7/modules/GoogleRadiusMaps/resources/css/main.css" rel="stylesheet" />');
                $('head').append('<link href="layouts/v7/modules/GoogleRadiusMaps/resources/css/colorpicker.css" rel="stylesheet" />');
                $('head').append('<link href="layouts/v7/modules/GoogleRadiusMaps/resources/css/layout.css" rel="stylesheet" />');
				
				$('head').append('<script type="text/javascript" ' +
                        'src="layouts/v7/modules/GoogleRadiusMaps/resources/js/colorpicker.js"></script>');
						
				$('head').append('<script type="text/javascript" ' +
					'src="layouts/v7/modules/GoogleRadiusMaps/resources/js/eye.js"></script>');
					
				$('head').append('<script type="text/javascript" ' +
					'src="layouts/v7/modules/GoogleRadiusMaps/resources/js/utils.js"></script>');
					
				$('head').append('<script type="text/javascript" ' +
					'src="layouts/v7/modules/GoogleRadiusMaps/resources/js/layout.js?ver=1.0.2"></script>');

                window.initializeMap = function() {
                    $('head').append('<script type="text/javascript" ' +
                        'src="layouts/v7/modules/GoogleRadiusMaps/resources/js/markerwithlabel.js"></script>');
                ...
                $(document).on('change', '#slbAddresses, #slbAddressesEvents', function(){
                    ...
                    thisInstance.getAddressesOfRecord(recordId, function(result){
                        if(result.success) {
                            $.each(result.addresses, function(i, o){
                ...
                $('body').on('change', '.listViewEntriesMainCheckBox', function(){
                ...
                $('body').on('change', '.listViewEntriesCheckBox', function() {
                    ...
                    if($(this).prop('checked')) {
                        thisInstance.getAddressesOfRecord(recordId, function(result) {
                            if(result) {
								var street = result.addresses[0].street;
								if(typeof street === 'undefined' || isEmpty(street)) {
									var lat = parseFloat(result.addresses[0].lat);
									var lng = parseFloat(result.addresses[0].long);
									var latLng = new google.maps.LatLng(lat, lng);
									...
									street = st.join('+');
									var city = result.addresses[0].city;
									var state = result.addresses[0].state;
									var postal_code = result.addresses[0].postal_code;
                                ...
                                thisInstance.props.loadedAddressesOfRecord[recordId] = result.addresses[0];
                ...
                $(document).on('click', '.btnRoute', function(e) {
                    ...
                    if(type == 'print') {
                        ...
                        function showRoutingModal(element) {
                            app.helper.showModal(element, {'cb' : function(wizardContainer) {
                                var recordsHtml = "";
                        ...
                        if(typeof window.routingModalHtml == 'undefined') {
                            $.get('layouts/v7/modules/GoogleRadiusMaps/resources/views/routing-modal.html', function(html){
                                //^Load language
                                html = $.tmpl(html, {
            ...
            this.getAddresses(function(result){
                    thisInstance.props.loadedAddresses = result.addresses;
            });
        },
        ...
        showMapWindow: function() {
            ...
            if(!this.props.viewLoaded) {
                ...
                $.get(url, function(data) {
                    ...
                    var listViewContentContainer = $('#listViewContent');
                    var resizeElementContainer = listViewContentContainer.find('.col-sm-12');

                    resizeElementContainer.addClass('halfpart');
                    resizeElementContainer.addClass('col-sm-6');
                    resizeElementContainer.addClass('col-xs-6');
                    resizeElementContainer.removeClass('col-sm-12');
                    resizeElementContainer.removeClass('col-xs-12');
                    listViewContentContainer.append('<div class="col-sm-6 col-xs-6">'+html+'</span>');
                    // var floatTheadWrapper = $('.floatThead-wrapper');
                    var floatTheadContainer = $('.floatThead-floatContainer.floatThead-container');
                    // var tableContentElement = $('#table-content');
                    floatTheadContainer.css('width','100%');
                    $('#table-content').perfectScrollbar('update');
                    // tableContentElement.css('width','50%');
                    //
                    // floatTheadWrapper.append(html);
                ...
                window.adjustLayout();
                var listViewContentContainer = $('#listViewContent');
                var resizeElementContainer = listViewContentContainer.find('.halfpart');
        ...
        hideMapWindow: function() {
            ...
            var listViewContentContainer = $('#listViewContent');
            var resizeElementContainer = listViewContentContainer.find('.halfpart');
            resizeElementContainer.addClass('col-sm-12');
            resizeElementContainer.addClass('col-xs-12');
            resizeElementContainer.removeClass('col-xs-6');
            resizeElementContainer.removeClass('col-sm-6');
            $('#table-content').perfectScrollbar('update');
            window.adjustLayout();
        },
        ...
        getAddressesOfRecord: function(recordId, callback) {
            ...
            //     }
            // );
            app.request.get(actionParams).then(
    ...
    jQuery(document).ready(function() {
        setTimeout(function () {
            initData_GoogleRadiusMaps();
        }, 4000);
    });
    function initData_GoogleRadiusMaps() {
        // Only load when loadHeaderScript=1 BEGIN #241208
        if (typeof VTECheckLoadHeaderScript == 'function') {
            if (!VTECheckLoadHeaderScript('GoogleRadiusMaps')) {
                return;
            }
        }
        // Only load when loadHeaderScript=1 END #241208

        var currViewMode = app.getViewName();
    ...
    //#639234
    /*
    * for vt710. when change list view on vt710,
    * page loaded by ajax,
    * so function load GoogleMap button NOT trigger
    * script bellow to reload page follow filter.
    * */
    $('a.filterName.listViewFilterElipsis').on('click',function(){
        window.location.href = this.href;
    });
})(jQuery);

function loadScriptForGGRadiusMap() {
    ...
    app.request.post({data: params}).then(function (err, data) {
        ...
        if (typeof google == 'undefined' || typeof google.maps == 'undefined'){
            $('head').append('<script type="text/javascript" ' +
                            'src="//maps.googleapis.com/maps/api/js?v=3&key=' +
                            googleApiKey +
                            '&signed_in=true&callback=window.initializeMap"></script>');
        }
    });
}
