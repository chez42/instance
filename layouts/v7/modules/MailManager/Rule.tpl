	...
	<div class="mailConverterRuleBlock">
		<div class="details border1px">
			<div class="ruleHead modal-header" style="cursor: move; min-height: 30px; padding: 10px 0px;">
				<strong>
					<img class="alignMiddle" src="{vimage_path('white-drag.png')}" style="margin-left: 10px;" />&nbsp;&nbsp;{vtranslate('LBL_RULE', $QUALIFIED_MODULE_NAME)}&nbsp;<span class="sequenceNumber">{$RULE_COUNT}</span>&nbsp;:&nbsp;{vtranslate($RULE_MODEL->get('action'), $QUALIFIED_MODULE_NAME)}
			...
			<fieldset class="marginTop10px">
				<strong class="marginLeft10px">{vtranslate('LBL_CONDITIONS', $QUALIFIED_MODULE_NAME)}</strong>
				<hr>
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$FIELDS}
					<div class="col-lg-12 padding10">
						<div class="col-lg-1"></div>
						<div class="col-lg-3 fieldLabel"><label>{vtranslate($FIELD_MODEL->get('label'), $QUALIFIED_MODULE_NAME)}</label></div>
						<div class="col-lg-7 fieldValue">
							{if $FIELD_NAME neq 'action' && $FIELD_NAME neq 'assigned_to'}
								...
								{if $FIELD_NAME eq 'matchusing'}
									{assign var=FIELD_VALUE value=vtranslate('LBL_ANY_CONDITIONS', $QUALIFIED_MODULE_NAME)}
									{if $RULE_MODEL->get('matchusing') eq 'AND'}
										{assign var=FIELD_VALUE value=vtranslate('LBL_ALL_CONDITIONS', $QUALIFIED_MODULE_NAME)}
									{/if}
			...
			<hr>
			<fieldset class="marginTop10px">
				<strong class="marginLeft10px">{vtranslate('LBL_ACTIONS', $QUALIFIED_MODULE_NAME)}</strong>
				<hr>
