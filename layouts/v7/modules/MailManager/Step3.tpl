...
{strip}
	...
	<div class="block fieldBlockContainer">
		<form class="form-horizontal" id="ruleSave" method="post" name="step3">
			<input type="hidden" name="module" value="MailConverter" />
						...
						<table class="table editview-table no-border">
							<tbody>
								...
								{foreach key=FIELD_NAME item=FIELD_MODEL from=$FIELDS}
									<tr>
										<td class="fieldLabel control-label" style="width:25%; padding-right:20px;"><label>{vtranslate($FIELD_MODEL->get('label'), $QUALIFIED_MODULE_NAME)}</label>
											...
											{if $FIELD_DATA_TYPE eq 'picklist'}
												...
												{if $FIELD_NAME eq 'subject'}
													<select name="subjectop" class="select2 fieldValue inputElement">
														<option value="">{vtranslate('LBL_SELECT_OPTION', $QUALIFIED_MODULE_NAME)}</option>
												...
												{elseif $FIELD_NAME eq 'body'}
													<select name="bodyop" class="select2 fieldValue inputElement">
														<option value="" {if $RECORD_MODEL->get('bodyop') eq ""}selected{/if}>{vtranslate('LBL_SELECT_OPTION', $QUALIFIED_MODULE_NAME)}</option>
														{foreach key=PICKLIST_KEY item=PICKLIST_VALUE from=$PICKLIST_VALUES}
			...
			<div class="border1px modal-overlay-footer clearfix">
				<div class="row clearfix">
					<div class="textAlignCenter col-lg-12 col-md-12 col-lg-12 ">
						{*<button class="btn btn-danger backStep" type="button" onclick="javascript:window.history.back();"><strong>{vtranslate('LBL_BACK', $QUALIFIED_MODULE_NAME)}</strong></button>&nbsp;&nbsp;*}
						<button class="btn btn-success"><strong>{vtranslate('LBL_FINISH', $QUALIFIED_MODULE_NAME)}</strong></button>
