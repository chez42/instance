...
Vtiger.Class("VTEEmailMarketing_Create_Js", {
    ...
    registerShowHideFilter: function(){
        ...
        jQuery(document).on('click','.btn-on-off-filter',function () {
            ...
            if(check == 'true'){
            	...
            	if(table.find('tr[module="'+module+'"]').length > 0){
		        		...
		        		function (data) {
		        			if(data.success){
		        				...
		        				$.each(result,function(index, val){
		        					html += '<tr module="'+module+'" ><td>';
	                                
	                                if(module == 'Contacts'){
	                                    html+='<h5> <i class="vicon-contacts module-icon"></i>'+val.module+'</h5>';
	                                }else if(module == 'Leads'){
	                                    html+='<h5> <i class="vicon-leads module-icon"></i>'+val.module+'</h5>';
	                                }else if(module == 'Potentials'){
	                                	html+='<h5> <i class="vicon-potentials module-icon"></i>'+val.module+'</h5>';
	                                }else{
	                                    html+='<h5> <i class="vicon-accounts module-icon"></i>'+val.module+'</h5>';
		        					}
		                            html +='</td>'+
		                            	'<td>'+
			                                '<h4>'+val.name_filter+'</h4>'+
			                                '<i><span class="marketing-list-user">( '+val.name+' )</span></i>'+
		                                '</td>'+
		                                '<td>'+
		                                	'<h5 class="text-center">'+val.count+'</h5>'+
		                                	'<h5 class="text-center">Records</h5>'+
		                                '</td>'+
		                                '<td>'+
			                                '<button class="btn addButton btn-default btn-success btn-load-filter"'+
			                                        'value="'+val.cvid+'" action="0">Load'+
			                                '</button>'+
			                            '</td>'+
			                            '<td>'+
            ...
            if(action == 1){
            	var params = {
    	            ...
    	            AppConnector.request(params).then(function (data) {
    	                if(data.success == true){
    	                    app.helper.hideProgress();
    	                    thisBtn.attr('action',0);
    	                    thisBtn.removeClass('btn-primary');
    	                    thisBtn.addClass('btn-success');
    	                    thisBtn.html('Load');
    	                    thisInstance.getTotalRelated();
    	                    thisBtn.closest('tr').css("background", "");
    	                    var result = data.result;
    	                    thisBtn.closest('tr').find('td .filter-loaded-record').html('');
	                        thisBtn.closest('tr').find('td .show-info-load-0-record').html('');
    	                }
    	            });
    ...
    registerShowPopupSelectingRelatedRecord : function() {
        ...
        step2.on('click','.selectRelation',function (e) {
            ...
            else if(moduleRel == 'Potentials'){
                var relationId = step2.find('input[name="relationIdPotentials"]').val();
                step2.find('input[name="module_related"]').val('Potentials');
        ...
        AppConnector.request(params).then(
            function (data) {
                ...
                if (data.success == true) {
                    ...
                    if (campaign.countPotentials != 0) {
                        tableList += '<td>Opportunities</td>' +
                            '<td>' + campaign.countPotentials + '</td>' +
                            '</tr>';
                    }
        ...
        AppConnector.request(params).then(function (data) {
            if(data.success){
                ...
                for(var i =0; i < result.length ; i++){
                    ...
                    }else if(result[i].module == 'Potentials'){
                    	 html += '<td><h5>'+result[i].label+'</h5></td>';
                         html += '<td><h5>'+result[i].potentials_email+'</h5></td>';
                    }else{
                        html += '<td><h5>'+result[i].label+'</h5></td>';
