...
<div class="VTEEmailMarketing-content-area" style="padding: 15px;">
    <div class="editContainer" style="padding-left: 2%;padding-right: 2%">
        <div class="row">
            ...
            {assign var=LABELS value = ["step1" => vtranslate("LBL_CAMPAIGN_DETAILS","VTEEmailMarketing") , "step2" => vtranslate("LBL_MARKETING_LIST","VTEEmailMarketing"), "step3" => vtranslate("LBL_REVIEW_AND_SEND","VTEEmailMarketing")]}
        ...
        <form class="form-horizontal recordEditView" id="EditView" name="edit" method="post" action="index.php" enctype="multipart/form-data">
            ...
            <input type="hidden" name="idEmailMarketing" value="{$RECORDMODEL->getId()}">
            <input type="hidden" name="campagin_name" value="{$RECORDMODEL->get('vtecampaigns')}">
            {assign var=FROMEMAILNAME value=explode('(',$RECORDMODEL->get('sender'))}
            <input type="hidden" name="from_name" value="{$FROMEMAILNAME[0]}">
            <input type="hidden" name="from_email" value="{str_replace(')','',$FROMEMAILNAME[1])}">
            <input type="hidden" name="templateEmail" value="{$TEMPLATEID}">
            <input type="hidden" name="step" value="{if $RECORD}step2{else}step1{/if}">
