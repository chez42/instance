	...
	<select class="select2 listSearchContributor {$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}"  name="{$ASSIGNED_USER_ID}" multiple>
		<optgroup label="{vtranslate('LBL_USERS')}">
			{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
				<option {if $OWNER_NAME|in_array:$TASK_FILTERS['assigned_user_id']}selected {else if $OWNER_NAME eq getUserFullName($USER_MODEL->id)} selected {/if} value="{$OWNER_NAME}" data-picklistvalue= '{$OWNER_NAME}'
