...
Vtiger_Index_Js("Vtiger_TaskManagement_Js",{},{
	...
	registerStatusCheckboxEvent : function(){
		...
		overlay.on('click','.statusCheckbox',function(e){
			...
			if (element.is(':checked')) {
				...
				fieldNameValueMap['field'] = 'task_status';
	...
	registerTaskManagementSearch : function(){
		...
		overlay.find('.filter-menu a').click(function(){
			overlay.find('.taskManagementFilter.active').removeClass('active');
			$(this).closest('.taskManagementFilter').addClass('active');
			overlay.find('#selected').text($(this).text());
			overlay.find('input[name="taskManagementallCvId"]').val($(this).data('filterId'));
			thisInstance.clearExistingCustomScroll();
			thisInstance.loadContents();
		});
	...
	registerQuickEditTaskEvent : function(){
		...
		jQuery('#taskManagementContainer').on('click', ".quickTask",function(e){
			...
			var quickCreateNode = jQuery('#quickCreateModules').find('[data-name="Task"]');
			...
			app.event.one("post.QuickCreateForm.show",function(event,form){
				...
				if(typeof basicInfo != "undefined"){
					...
					var taskStatus = form.find('select[name="task_status"]');
					taskStatus.val('Not Started');
					vtUtils.showSelect2ElementView(taskStatus);
				}

				var taskPriority = form.find('select[name="task_priority"]');
				if(taskPriority.length > 0){
					taskPriority.val(priority);
					vtUtils.showSelect2ElementView(taskPriority);
				}

				form.append("<input type=hidden name='task_priority' value='"+priority+"'>");
			});
	...
	registerPostQuickCreateSaveEvent : function() {
		...
		app.event.on('post.QuickCreateForm.save',function(event,data){
			if(typeof data == 'object'){
				priority = data['task_priority']['value'];
			}
	...
	registerSubjectFieldEnterEvent : function(){
		...
		overlay.on("keypress",".taskSubject",function(e){
			...
			if(keycode == '13'){
				...
				fieldNameValueMap["task_priority"] = priority;
				fieldNameValueMap["subject"] = subject;
				fieldNameValueMap["task_status"] = 'In Progress';
	...
	getAllFilterParams : function(){
		...
		var filterMode = jQuery('.otherFilters input[name="taskManagementallCvId"]').val();
		
		filterParams["viewname"] = filterMode;
		...
		overlay.find(".ui-droppable").droppable({
			...
			drop: function( event, ui ) {
				...
				if(draggedElementPriority != priority){
					...
					var fieldNameValueMap = {"task_priority":priority};
					app.helper.showProgress();
...
//							app.helper.showVerticalScroll(currentBlock.find("."+priority.toLowerCase()+"-entries"),params);
	...
	initializeTaskStatus : function(){
		...
		var taskStatus = container.find('input[name="taskManagementallCvId"]');
...
//		this.loadContents();
		...
		var filters = jQuery('.taskManagement-filters');
        ...
        jQuery.each(scrollContainers,function(key,scroll){
            ...
            var listcontentHeight = 200;
            
            if(scroll.find(".taskManagement-menu-content").height() < 190 && scroll.find(".taskManagement-menu-content").height() > 0){
            	listcontentHeight = scroll.find(".taskManagement-menu-content").height();
            }
            //scroll.find(".taskManagement-menu-content").height();
            scroll.css("height",listcontentHeight);
            var params = {
				setHeight: listcontentHeight,
				autoHideScrollbar: false
			 };
			 app.helper.showVerticalScroll(scroll,params);
            //scroll.perfectScrollbar();
        });
        
        thisInstance.changeButtonTitle();
	},
	
	changeButtonTitle : function() {
		var self = this;
		var overlay = self.getOverlayContainer();
		var name = overlay.find('.taskManagementFilter.active');
		if(name.length){
			overlay.find('#selected').text(name.text());
		}
