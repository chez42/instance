				...
				<div id="appnav" class="navbar-right">
					<ul class="nav navbar-nav">
						{foreach item=BASIC_ACTION from=$MODULE_BASIC_ACTIONS}
                            {if $BASIC_ACTION->getLabel() eq 'LBL_CONFIGURE_ACTIONS'}
								<li>
									<button style="background-color: #49afcd ; color: #FFFFFF;" id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}"
											class="btn addButton btn-info module-buttons"
                            ...
                            {elseif $BASIC_ACTION->getLabel() eq 'LBL_CRON_EXECUTE'}
								<li>
									<button style="background-color: #5bb75b ; color: #FFFFFF;" id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}"
											data-url="{$BASIC_ACTION->getUrl()}"
											class="btn addButton btn-success module-buttons"
											onclick="javascript:VTEMailConverter_List_Js.RunCron(this);">
										<div class="fa {$BASIC_ACTION->getIcon()}" aria-hidden="true"></div>&nbsp;&nbsp;
										{vtranslate($BASIC_ACTION->getLabel(), $MODULE)}
									</button>
								</li>
                            {elseif $BASIC_ACTION->getLabel() eq 'LBL_MAIL_CONVERTER'}
								<li>
									<button id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($BASIC_ACTION->getLabel())}"
											data-url="{$BASIC_ACTION->getUrl()}"
											class="btn addButton btn-success module-buttons"
											onclick="javascript:window.open('{$BASIC_ACTION->getUrl()}', '_blank');"
											type="button">
										<div class="fa {$BASIC_ACTION->getIcon()}" aria-hidden="true"></div>&nbsp;&nbsp;
										{vtranslate($BASIC_ACTION->getLabel(), $MODULE)}
									</button>
