	...
	<div class="container-fluid">
		...
		<div class="app-list row">
			...
			{foreach item=APP_NAME from=$APP_LIST}
				...
				{if count($APP_GROUPED_MENU.$APP_NAME) gt 0}
					<div class="dropdown app-modules-dropdown-container">
						...
						{/foreach}
						<style>
							.app-item-color-{$APP_NAME}{
								border-left: 4px solid {$APP_COLOR[$APP_NAME]};
							}
						</style>
						<div class="menu-item app-item dropdown-toggle app-item-color-{$APP_NAME}" data-app-name="{$APP_NAME}" id="{$APP_NAME}_modules_dropdownMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" data-default-url="{$FIRST_MENU_MODEL->getDefaultUrl()}&app={$APP_NAME}">
							<div class="menu-items-wrapper app-menu-items-wrapper">
								...
								<span class="app-name textOverflowEllipsis"> {vtranslate("$APP_NAME")}</span>
			...
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($DOCUMENTS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="index.php?module=Documents&view={if $USER_PRIVILEGES_MODEL->documents_default_view}{$USER_PRIVILEGES_MODEL->documents_default_view}{else}List{/if}">
					<div class="menu-items-wrapper">
			...
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($TASK_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="index.php?module=Task&view=List">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa">{$TASK_MODULE_MODEL->getModuleIcon()}</span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('Task')}</span>
					</div>
				</div>
			{/if}
			...
			{if $USER_PRIVILEGES_MODEL->hasModulePermission($COMMENTS_MODULE_MODEL->getId())}
				<div class="menu-item app-item app-item-misc" data-default-url="index.php?module=ModComments&view=List">
					<div class="menu-items-wrapper">
						<span class="app-icon-list fa" style="font-size: 1.3em !important;"><i class="fa fa-comment"></i></span>
						<span class="app-name textOverflowEllipsis"> {vtranslate('ModComments')}</span>
					</div>
				</div>
			{/if}
