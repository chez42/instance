...
Vtiger_Widget_Js('Vtiger_Notebook_Widget_Js', {
	...
	saveNotebookContent: function() {
		...
		app.request.post({"url":params}).then(function(err,data) {
			...
			app.helper.showVerticalScroll(widgetContent,{'setHeight' : adjustedHeight});
			jQuery('.dashboard_notebookWidget_text', self.container).css("display", "none");;
...
Vtiger_Widget_Js('Vtiger_USHeatMap_Widget_Js',{},{
	...
	loadChart : function() {
		...
		var chartData = this.generateChartData();
		var container = this.getContainer();
		var tabid = container.closest('div.tab-pane').data('tabid');
		
		container.find('#us_heat_map_chart').attr('id','us_heat_map_chart_'+tabid);
		...
		am4core.ready(function() {
			...
			// Create map instance
			var chart = am4core.create("us_heat_map_chart_"+tabid, am4maps.MapChart);
			...
			function createSeries(heatfield) {
			  ...
			  template.tooltipText = "{name} : [bold]{stores} [/]";
			...
			function setupStores(data) {
			  ...
			  am4core.array.each(data, function(store) {
				...
				var stateLatLong = USStateLatLon[store.id];
				
				// Get store data
				if(jQuery.inArray(store.id, validUSStates) > -1){
					
					var store = {
					  state: store.id,
					  long: am4core.type.toNumber(stateLatLong['longitude']),
					  lat: am4core.type.toNumber(stateLatLong['latitude']),
					  location: store.co_loc_n,
					  city: store.mail_city_n,
					  count: am4core.type.toNumber(store.value),
					  url: store.url
					};
	
					// Process state-level data
					if (regionalSeries[store.state] == undefined) {
					  var statePolygon = polygonSeries.getPolygonById(store.state);
					  if (statePolygon) {
...
Vtiger_Widget_Js('Vtiger_PopulationPyramid_Widget_Js',{},{
	...
	generateChartData : function() {
		...
		var container = this.getContainer();
		var jsonData = container.find('#chart_data').val();
	...
	loadChart : function() {
		...
		var tabcontainer = this.getContainer();
		var tabid = tabcontainer.closest('div.tab-pane').data('tabid');
		
		tabcontainer.find('#chartdiv').attr('id','chartdiv_'+tabid);
		
		if(!chartData) return false;
	
		//am4core.useTheme(am4themes_animated);
		am4core.options.commercialLicense = true;
		var mainContainer = am4core.create("chartdiv_"+tabid, am4core.Container);
		...
		maleChart.data = JSON.parse(JSON.stringify(chartData));
		...
		femaleChart.data = JSON.parse(JSON.stringify(chartData));
