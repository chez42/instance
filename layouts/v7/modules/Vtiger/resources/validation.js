...
jQuery.validator.addMethod("check_file_type", function(value, element, params) {
	
		var result;
		var fileInput = element.files[0];
		
		var MimeFilter = new RegExp(params,'i');	
		if (fileInput) {
			result = MimeFilter.test(fileInput.type);
			if(!result){
				result =false;
			}
		} else { 
		...
		return result;
	}, jQuery.validator.format(app.vtranslate('File must be a valid JPEG, JPG or PNG'))
);
