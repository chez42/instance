...
jQuery.Class("Vtiger_AdvanceFilter_Js",{
	...
	getFieldSpecificUi : function(fieldSelectElement) {
		...
		if(fieldModel.getType().toLowerCase() == "boolean") {
			...
			if(fieldModel.getName() == "is_admin" && app.getModuleName() == 'Users')
				html += '<option value="off"';
			else		
				html += '<option value="0"';
			
			if(selectedValue == '0' || selectedValue == 'off') {
	...
	getUi : function(){
		...
		if(comparatorSelectedOptionVal == 'e' || comparatorSelectedOptionVal =='n'){
			...
			for(var option in pickListValues) {
				if(this.getName() == 'roleid' && app.getModuleName() == 'Users')
					html += '<option value="'+pickListValues[option]+'" ';
				else	
					html += '<option value="'+option+'" ';
				
				if(this.getName() == 'roleid' && app.getModuleName() == 'Users'){
					if(jQuery.inArray(app.htmlDecode(pickListValues[option]),selectedOptionsArray) != -1){
						html += ' selected ';
					}
				}else{
			...
			for(var key in pickListValues){
				...
				if(this.getName() == 'roleid' && app.getModuleName() == 'Users')
					pickListValuesArrayFlip[pickListValue] = pickListValue;
