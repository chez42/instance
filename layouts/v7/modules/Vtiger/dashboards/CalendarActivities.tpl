    ...
    {if $usersList}
        <div class="userList">
            <select class="select2 widgetFilter" name="type">
                <option value="{$CURRENTUSER->getId()}" {if $SELECTED eq $CURRENTUSER->getId()}selected{/if}>{vtranslate('LBL_MINE',$MODULE_NAME)}</option>
                ...
                {foreach key=GROUP_ID from=$SHARED_GROUPS item=GROUP_NAME}
                    <option value="{$GROUP_ID}" {if $SELECTED eq $GROUP_ID}selected{/if}>{$GROUP_NAME}</option>
                {/foreach}
                <option value="all" {if $SELECTED eq "all"}selected{/if}>{vtranslate('All', $MODULE_NAME)}</option>
            </select>
        </div>
    {/if}
</div>
