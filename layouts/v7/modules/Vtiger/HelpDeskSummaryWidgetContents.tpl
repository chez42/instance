				...
				<li>
					<div>
						<div class="textOverflowEllipsis width27em">
                            ...
                            <span>
                                <a href="{$RELATED_RECORD->getDetailViewUrl()}" target="_blank" title="{$RELATED_RECORD->getDisplayValue('ticket_title')}" id="{$MODULE}_{$RELATED_MODULE}_Related_Record_{$RELATED_RECORD->get('id')}">
                                    {$RELATED_RECORD->getDisplayValue('ticket_title')}
                                </a>
                            </span>
						</div>
						<div>{vtranslate('Status',$MODULE)} : <strong> {$RELATED_RECORD->getDisplayValue('ticketstatus')}</strong> </div>
