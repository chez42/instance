						...
						<div class="btn-toolbar">
							<div class="btn-group">
								{foreach item=DETAILVIEW_LINK from=$DETAILVIEW_LINKS}
									...
									<button class="btn btn-default reportActions" name="{$LINKNAME}" data-href="{$DETAILVIEW_LINK->getUrl()}{if $LINKNAME neq 'Send Email'}&source={$REPORT_MODEL->getReportType()}{/if}">
										{$LINKNAME}
									</button>
								{/foreach}
							</div>
