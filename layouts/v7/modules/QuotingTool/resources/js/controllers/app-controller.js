...
(function ($) {
    ...
    controllers.controller('CtrlApp',
        function ($rootScope, $scope, $templateCache, $compile, $timeout, $translate, PageTitle, AppUtils, AppToolbar, AppConstants,
                  Template, TemplateSetting,TemplateProposal, GlobalConfig, ProgressIndicator) {
            // Reset page title
            PageTitle.reset();
                ...
                switch (focus) {
                    ...
                    case $rootScope.app.data.blocks.pricing_table:
                        AppUtils.loadTemplate($scope, focus.setting_template, false, function (html) {
                            ...
                            //// Show setting popup when click to product image setting icon
                            //html.on('click', '.quoting_tool-btn-options-product-image', function (e) {
                            //    alert('click');
                            //    $('img.quoting_tool_product_image').dblclick();
                            //});
                            // Open modal
                            AppHelper.showModalWindow(html, '#', function () {
                    ...
                    case $rootScope.app.data.blocks.related_module:
                        AppUtils.loadTemplate($scope, focus.setting_template, false, function (html) {
                            ...
                            $scope.settings.related_module.item_fields_group = [];
                            $scope.settings.related_module.item_fields_group.length = 0;
                            $scope.settings.related_module.link_module= [];
                            ...
                            if (info['settings']) {
                                ...
                                if (info['settings']['item_fields']) {
                                    ...
                                    for (var sf = 0; sf < selected.length; sf++) {
                                        ...
                                        if ($scope.settings.related_module.link_module.fields){
                                ...
                                // item fields group by
                                if (info['settings']['item_fields_group']) {
                                    selected = info['settings']['item_fields_group'];
                                        ...
                                        if ($scope.settings.related_module.link_module.fields){
                                            for (var inv = 0; inv < $scope.settings.related_module.link_module.fields.length; inv++) {
                                                ...
                                                if ((linkModuleFieldItem.block.id == selectedItem.block.id)
                                                    && (linkModuleFieldItem.id == selectedItem.id)
                                                    && (linkModuleFieldItem.name == selectedItem.name)) {
                                                    $scope.settings.related_module.item_fields_group.push($scope.settings.related_module.link_module.fields[inv]);
                                                }
                                            }
                                        }
                                    }
                                ...
                                // sort field
                                if(info['settings']['sort_field']){
                                    selected = info['settings']['sort_field'];
                                    for (var sf = 0; sf < selected.length; sf++) {
                                        selectedItem = selected[sf];
                                        ...
                                        if ($scope.settings.related_module.link_module.fields){
                                            for (var inv = 0; inv < $scope.settings.related_module.link_module.fields.length; inv++) {
                                                ...
                                                if ((linkModuleFieldItem.block.id == selectedItem.block.id)
                                                    && (linkModuleFieldItem.id == selectedItem.id)
                                                    && (linkModuleFieldItem.name == selectedItem.name)) {
                                                    $scope.settings.related_module.sort_field.push($scope.settings.related_module.link_module.fields[inv]);
                                                }
                                            }
                                        }
                                    }
                                ...
                                // sort field
                                if (info['settings']['sort_fields_order']) {
                                    $scope.settings.related_module.sort_fields_order = info['settings']['sort_fields_order'];
                                }
                                if (info['settings']['sort_group_fields_order']) {
                                    $scope.settings.related_module.sort_group_fields_order = info['settings']['sort_group_fields_order'];
                                }
                                if (info['settings']['sort_type']) {
                                    $scope.settings.related_module.sort_type = info['settings']['sort_type'];
                                }
                                // conditions
                                if (info['settings']['conditions']) {
                                    $scope.settings.related_module.conditions = info['settings']['conditions'];
                                }
                            ...
                            // Open modal
                            AppHelper.showModalWindow(html, '#', function () {
                                $("#popup_settings_related_module").css("width", "auto");
                                var settingsRelatedModuleFields = html.find('#settings_related_module_fields');
                                var settingsRelatedModuleFieldsGroup = html.find('#settings_related_module_group_by');
                                var settingsRelatedModuleSortField=html.find('#settings_related_module_sort_field');
                                var settingsLinkModule = html.find('#settings_link_modules');

                                $scope.changeRelatedModule(false);
                                ...
                                if(!info['settings']){
                                 ...
                                 var myVar = setInterval(function(){
                                    ...
                                    AppHelper.arrangeSelectChoicesInOrder(settingsRelatedModuleFieldsGroup, $scope.settings.related_module.sort_group_fields_order);
                                     //TASKID:1148851 - DEV: tiennguyen - DATE: 2018-12-03 - START
                                     //NOTE : get value sort field
                                     settingsRelatedModuleSortField.val($scope.settings.related_module.sort_fields_order);
                                     settingsRelatedModuleSortField.select2();
                                     html.find('[name="sort_type"]').each(function () {
                                         if($(this).val() == $scope.settings.related_module.sort_type){
                                             $(this).prop('checked',true);
                                         }
                                     });
                                     //TASKID:1148851 - DEV: tiennguyen - DATE: 2018-12-03 - END
                                    clearInterval(myVar);
                                }, 100);
                                ...
                                html.on('click', '.btn-submit', function () {
                                    ...
                                    // fields group by
                                    settingsRelatedModuleFieldsGroup.find('option[selected="selected"]').each(function () {
                                        $(this).prop('selected',true);
                                    });
                                    settingsRelatedModuleSortField.find('option:selected').each(function () {
                                        $(this).prop('selected','selected');
                                    });
                                    $scope.settings.related_module.sort_field=settingsRelatedModuleSortField.find('option:selected').data('info');
                                    $scope.settings.related_module.sort_type=html.find('[name="sort_type"]:checked').val();
                                    var itemFieldIds = AppHelper.getSelectedColumns(settingsRelatedModuleFields);
                                    var itemFieldIdsGroup = AppHelper.getSelectedColumns(settingsRelatedModuleFieldsGroup);
                                    var sortFieldIds = settingsRelatedModuleSortField.find('option:selected').val();
                                    var itemFields = [];
                                    var itemFieldsGroup = [];
                                    var itemFieldSortedOption=null;
                                    var itemFieldSortedInfo = {};

                                    for (var ii = 0; ii < itemFieldIds.length; ii++){
                                        itemFieldSortedOption = settingsRelatedModuleFields.find('option[value="' + itemFieldIds[ii] + '"]:selected');
                                        itemFieldSortedInfo = itemFieldSortedOption.data('info');
                                        itemFields.push(itemFieldSortedInfo);
                                    }
                                    for (var xx = 0; xx < itemFieldIdsGroup.length; xx++){
                                        itemFieldSortedOption = settingsRelatedModuleFieldsGroup.find('option[value="' + itemFieldIdsGroup[xx] + '"]:selected');
                                        itemFieldSortedInfo = itemFieldSortedOption.data('info');
                                        itemFieldsGroup.push(itemFieldSortedInfo);
                                    }
                                    $scope.settings.related_module.item_fields = itemFields;
                                    $scope.settings.related_module.item_fields_group = itemFieldsGroup;
                                    $scope.settings.related_module.item_fields_order = itemFieldIds;
                                    $scope.settings.related_module.sort_fields_order = sortFieldIds;
                                    $scope.settings.related_module.sort_group_fields_order = itemFieldIdsGroup;
                                    ...
                                    info['settings']['conditions'] = {};
                                    info['settings']['conditions']['all'] = [];
                                    ...
                                    for(var icr = 0; icr < totalRows; icr++){
                                        ...
                                        if (rowItem['fieldname'] != 'none'){
                                            ...
                                            info['settings']['conditions']['all'].push(rowItem);
                                        }
                                    }
                                    info['settings']['conditions']['any'] = [];
                                    ...
                                    for(var icr = 0; icr < totalRows; icr++){
                                        ...
                                        if (rowItem['fieldname'] != 'none'){
                                            ...
                                            rowItem['value'] = valueItem;
                                            rowItem['type'] = conditionRow.find("[name='columnname']").find("option:selected").data("fieldinfo").type;
                                            info['settings']['conditions']['any'].push(rowItem);
                                        }
                                    }
                                    objTable.attr('data-info', angular.toJson(info));
                                    $scope.settings.related_module.conditions = info['settings']['conditions'];
                                    ...
                                    var tbody = objTable.find('tbody');
                                    var arrTr = tbody.find('tr');
                                    if(itemFieldsGroup.length > 0) {
                                        if(tbody.find('.row_group').length == 0){
                                            var rowGroupBy = '<tr  class="row_group"><td id="cell_group" style="border:'+cellStyle.border+'" colspan ="'+colHeader.length+'">$Group_by</td></tr>';
                                            tbody.append(rowGroupBy);
                                        }else{
                                            objTable.find('#cell_group').attr('colspan',colHeader.length);
                                        }
                    ...
                    AppUtils.loadTemplate($scope, focus.setting_template, false, function (html) {
                        ...
                        // Reset settings
                        $scope.settings.create_related_record.size = $scope.tableBlockSize[0];
                        $scope.settings.create_related_record.theme = $scope.tableBlockTheme[1];    // Default is first theme
                        $scope.settings.create_related_record.item_fields = [];
                        ...
                        AppHelper.showModalWindow(html, '#', function () {
                            ...
                            $scope.changeRelatedModuleRecord(false);
                            if(!info['settings']){
                    ...
                    case $rootScope.app.data.widgets.field_image:
                        ...
                        break;
                    case $rootScope.app.data.blocks.barcode:
                        AppUtils.loadTemplate($scope, focus.setting_template, true, function (html) {
                            // Before open modal
                            var objImgField = container.find('img');
                            var info = objImgField.data('info');
                            ...
                            AppHelper.showModalWindow(html, '#', function () {
                                // Trigger click
                                html.on('click', '.btn-submit-barcode', function () {
                                    var selected_field = $rootScope.app.data.selectedModuleField['token'];
                                    jQuery('.first-load-barcode-text').remove();
                                    var selected_barcode = html.find('#settings_barcode_type').val();
                                    //[BARCODE|EAN2=$xxxxxx$|BARCODE]
                                    var text_input = "[BARCODE|"+selected_barcode+"="+selected_field + "|BARCODE]";
                                    var info = {
                                    ...
                                    AppUtils.pasteHtmlAtCaret(text_input, info);
                                    AppHelper.hideModalWindow();
                                });
                            });
                        });
                            ...
                            if (!jQuery.isEmptyObject(info)) {
                                ...
                                for(var i= 0; i< info.length ; i++) {
                                    (function (info) {
                                        ...
                                        AppUtils.loadTemplate($scope, 'layouts/v7/modules/QuotingTool/resources/js/views/popups/custom_table/field_table.html', true, function (html) {
                                ...
                                for(var i = 0; i < numRow; i++) {
                                    ...
                                    if (i < sort1) {
                                        ...
                                        (function (labelField, text, required, editablefield,id1 ,name1, uitype1, datatype1, module1) {
                                            ...
                                            AppUtils.loadTemplate($scope, 'layouts/v7/modules/QuotingTool/resources/js/views/popups/custom_table/field_table.html', false, function (html1) {
                                                var infofield = {};
                                                if (labelField != 'PLACEHOLDER(BLANK)') {
                                                    infofield = {
                                                        editable: editablefield,
                                    ...
                                    if (i < sort2) {
                                        ...
                                        (function (labelField2, text2, required2, editablefield2, id2 ,name2, uitype2, datatype2, module2) {
                                            AppUtils.loadTemplate($scope, 'layouts/v7/modules/QuotingTool/resources/js/views/popups/custom_table/field_table.html', false, function (html2) {
                                                var infofield2 = {};
                            ...
                            AppHelper.showModalWindow(html, '#', function () {
                                ...
                                html.on('click', '.btn-submit-teammember', function () {
                                    ...
                                    if (numberMember > info && numberMember<=3){
                                        if( jQuery(htmlbody).find('tr').length==0){
            ...
            $scope.changeRelatedModule = function (isChangeRelatedModuleFields) {
                $scope.settings.related_module.item_fields = [];
                $scope.settings.related_module.item_fields.length = 0;
                ...
                if (isChangeRelatedModuleFields){
                    $('#settings_related_module_fields').select2("val", "");
                    $scope.settings.related_module.conditions = '';
                }

                $("#settings_related_module_fillter").html('');
                var relatedModule = $scope.settings.related_module.link_module.name;
            ...
            $scope.changeRelatedModuleRecord = function (isChangeRelatedModuleFields) {
                $scope.settings.create_related_record.item_fields = [];
                $scope.settings.create_related_record.item_fields.length = 0;
                if (isChangeRelatedModuleFields){
                    $('#settings_create_related_record_fields').select2("val", "");
                    $scope.settings.create_related_record.conditions = '';
                }

                $("#settings_create_related_record_fillter").html('');
                var relatedModule = $scope.settings.create_related_record.link_module.name;
                var conditions = angular.toJson($scope.settings.create_related_record.conditions);
                var params = {};
                params['module'] = 'QuotingTool';
                params['view'] = 'RelatedModulesFilter';
                params['relatedModule'] = relatedModule;
                ...
                app.request.post({data:params}).then(function (error, data) {
                    if (data) {
                        $("#settings_create_related_record_fillter").html(data);
                        Vtiger_AdvanceFilter_Js.getInstance();
                    }
                });
            };
            ...
            $rootScope.changeSetting = function ($event) {
                ...
                $rootScope.app.last_focus_item_setting.each(function () {
                    ...
                    // fix bug text field not change size when change style of input.
                    var width = parseInt(thisFocus.css('width'));
                    var height = parseInt(thisFocus.css('height'));
                    container.css({
                        'width': ((width >= 26) ? width + 19 : 26) // Fix checkbox block on PDF
                        // 'height': height + 8
                    });
                    container.find('input:not([type="checkbox"])').css({
                        'height': 18
                    });
                    container.find('textarea').css({
                        'height': 48,
                        'width': ((width >= 26) ? width + 14 : 26)
                    });
                });
            ...
            //Check change for Pricing Table Field, if is Product Image has selected => process it
            $rootScope.changePricingTableField = function (event) {
                event.preventDefault();
                var product_image_input = $('input[name="product_image_width"]');
                var selectedOptions = $('#settings_pricing_table_fields').find('option:selected');
                var have_img_field = false;
                $.each(selectedOptions, function (key,opt){
                    //console.log(opt);
                    var field_info = $(opt).data('info');
                    if(field_info.datatype ==  "image"){
                        have_img_field = true;
                        if(product_image_input.length == 0){
                            var html_images_size = '<tr>'+
                                '<th>Images size</th>' +
                                '<td>' +
                                '<input name="product_image_width" value="70" size="5" style="text-align: center;" /> X <input size="5" name="product_image_height" value="70"  style="text-align: center;" /> '+
                                '</td>'+
                                '</tr>';
                            $('#settings_pricing_table_fields').closest('tr').after(html_images_size);
                            return false;
                ...
                if(!have_img_field) {
                    if(product_image_input.length > 0){
                        product_image_input.closest('tr').remove();
                    }
                }
            };
            /**
            ...
            $rootScope.loadTemplate = function (templateId) {
                ...
                var container = content.find('.content-container').each(function () {
                            ...
                            stop: function (event, ui) {
                                ...
                                for (var k in $rootScope.app.data.blocksOrder) {
                                    ...
                                    if ($rootScope.app.data.blocksOrder[k].layout.id == id) {
                                        var args = {
                                            id: $rootScope.app.data.blocksOrder[k].layout.id
                                            // type: 'after',
                                            // focus: prev
                                        };
                    ...
                    //Register click to product-image settings icon
                    if(html.find('.quoting_tool-btn-options-product-image').length > 0 ){
                        html.find('div.quoting_tool-btn-options-product-image').html('<div class="p-image"></div>');
                        html.on('click', '.quoting_tool-btn-options-product-image', function () {
                            var target_table = $('table[data-table-type="pricing_table"]');
                            target_table.parents('div.content-container').find('.btn--block-control-settings').click();
                        });
                    }
            ...
            $scope.showPopupSettingField = function ($event) {
                ...
                AppUtils.loadTemplate($scope, 'layouts/v7/modules/QuotingTool/resources/js/views/popups/custom_table/popup_settings_field.html', true, function (html) {
                    html.modal('show');
                    //editable
            ...
            //only get item details fields for primary module, relatedmodule
            ...
            $rootScope.getWidgetFields = function (param) {
                return function (item) {
                    ...
                    if(jQuery.inArray(item.uitype, ['10', '53', '69','57','51','4', '70', '52']) >= 0 ||
                        jQuery.inArray(item.name, ['crmid',  'source', 'cf_team', 'cf_teammembers', 'isconvertedfromlead']) >= 0|| (item.name).indexOf('cf_acf') > -1){
                        return false;
            ...
            var init = function () {
                ...
                // Anblock
                $rootScope.app.model.anblock = $rootScope.app.form_item.anblock.val() == 1;
                // Configuration
                ...
                if(view == 'Edit' && recordId == false && moduleName == 'QuotingTool'){
                    AppUtils.loadTemplate($scope, 'layouts/v7/modules/QuotingTool/resources/js/views/popups/settings_create_new_record.html', true, function (html) {
                        AppHelper.showModalWindow(html, '#', function () {
