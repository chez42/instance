...
jQuery.Class("Emails_MassEdit_Js",{
	 ...
	 */
	triggerSendEmailList: function (massActionUrl, module, params) {
		
		var data = app.convertUrlToDataParams(massActionUrl);
		if (params) {
			jQuery.extend(data, params);
		}
		Vtiger_Index_Js.showComposeEmailPopup(data);
	...
	registerEvents : function(){
		...
		this.registerInsertLinkEvent();
		
		app.event.on("post.DocumentsSelect.click",function(event, data){
			app.helper.showProgress();
			var responseData = JSON.parse(data);
			var documentIds = [];
			...
			var existingIds = jQuery(document).find('[name="insertlinkids"]').val();
			...
			jQuery.grep(documentIds, function(el) {
		        if (jQuery.inArray(el, existingIds) == -1) difference.push(el);
			});
			jQuery(document).find('[name="insertlinkids"]').val(JSON.stringify(documentIds));
			...
			if(difference.length){
				...
				app.request.post({"data":params}).then(function(err,data){
					app.helper.hideProgress();
					if(!err){
						CKEDITOR.instances['description'].insertHtml(data.html);
					}
				});
			}else{
				app.helper.hideProgress();
	...
	registerInsertLinkEvent : function(){
		var thisInstance = this;
		jQuery('#insertLink').on('click',function(e){
			
			app.helper.showProgress();
			var url = jQuery(e.currentTarget).data('url')+'&multi_select=true';
			...
			app.request.post({"data":postParams}).then(function(err,data){
				...
				var ele = jQuery('<div class="modal popupModal"></div>');
				ele.append(data);
				var field = '<input type="hidden" id="mode" value="pdffiles"/>';
				ele.find('#popupPageContainer').append(field);
				jQuery('body').append(ele);
				thisInstance.showpopupModal();
				app.event.trigger("post.Popup.Load",{"eventToTrigger":"post.DocumentsSelect.click"});
			});
		});
	},
