		...
		<form name='list' id='listedit' action='' onsubmit="return false;">
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					...
					{/if}
					{if $LISTVIEW_HEADER->getName() eq  'parent_id'}
						{assign var=NO_SORTING value=1}
					{/if}
				...
				{if $MODULE_MODEL->isQuickSearchEnabled() && !$SEARCH_MODE_RESULTS}
					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						{if $LISTVIEW_HEADER->getName() neq  'parent_id'}
							<th>
				...
				<tbody class="overflow-y">
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						{assign var=DATA_URL value=$LISTVIEW_ENTRY->getFullDetailViewUrl()}
					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						|| $LISTVIEW_HEADER->get('uitype') eq '9' || $LISTVIEW_HEADER->get('uitype') eq '25' || $LISTVIEW_HEADER->get('typeofdata') eq 'N~O' }text-right{/if}" 
							...
							<span class="fieldValue">
								<span class="value">
									{if ($LISTVIEW_HEADER->isNameField() eq true or $LISTVIEW_HEADER->get('uitype') eq '4') and $MODULE_MODEL->isListViewNameFieldNavigationEnabled() eq true }
										<a href="{$LISTVIEW_ENTRY->getFullDetailViewUrl()}">{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}</a>
									...
									{else}
										...
										{else if $LISTVIEW_HEADER->getFieldDataType() eq 'multipicklist'}
											...
											{/foreach}
										{else if $LISTVIEW_HEADER->getName() eq 'parent_id'}
											{$MODULE_MODEL->getParentRecordsValus($LISTVIEW_ENTRY_RAWVALUE, $LISTVIEW_HEADER->getUITypeModel())}
										{else if $LISTVIEW_HEADER->getName() eq 'access_count' || $LISTVIEW_HEADER->getName() eq 'click_count'}
											{$LISTVIEW_ENTRY_RAWVALUE}
										{else if $LISTVIEW_HEADER->getName() eq 'saved_toid'}
											{if strlen($LISTVIEW_ENTRY_VALUE) > 30}
												{substr($LISTVIEW_ENTRY_VALUE,0,30)}...
											{else}
