...
Vtiger.Class('Documents_Index_Js', {
	...
	showDocumentsSettings: function () {
		var instance = Documents_Index_Js.getInstance();
		instance.showDocumentsSettings();
	},
		...
		app.request.post({'data': params}).then(function (e, data) {
			...
			if (!e) {
				app.helper.showModal(data, {
					'cb': function (modalContainer) {
						thisInstance.registerDocumentsSettingsShownEvents(modalContainer);
					}
				});
			} else {
	...
	registerDocumentsSettingsShownEvents: function (modalContainer) {
		
		app.helper.showVerticalScroll(jQuery('.modal-body'));
		vtUtils.enableTooltips();
