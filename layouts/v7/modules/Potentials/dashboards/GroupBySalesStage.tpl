        ...
        <div class="userList col-lg-5">
            <div>
                <select class="widgetFilter select2" id="owner" name="owner" style='width:30%;margin-bottom:0px'>
                    <option value="{$CURRENTUSER->getId()}"{if $SELECTED eq $CURRENTUSER->getId()}selected{/if} >{vtranslate('LBL_MINE')}</option>
                    <option value="all" {if $SELECTED eq "all"}selected{/if}>{vtranslate('LBL_ALL')}</option>
                        ...
                        <optgroup label="{vtranslate('LBL_USERS')}">
                            {foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
                                {if $OWNER_ID neq {$CURRENTUSER->getId()}}
                                    <option value="{$OWNER_ID}" {if $SELECTED eq $OWNER_ID}selected{/if}>{$OWNER_NAME}</option>
                                {/if}
                            {/foreach}
                        </optgroup>
                    {/if}
		...
		<div class="row">
			...
			<span class="col-lg-7">
                <div class="input-daterange input-group dateRange widgetFilter" id="datepicker" name="expectedclosedate">
                    <input type="text" class="input-sm form-control" name="start" style="height:30px;" value="{$START}" />
                    <span class="input-group-addon">to</span>
                    <input type="text" class="input-sm form-control" name="end" style="height:30px;" value="{$END}" />
                </div>
			</span>
		</div>
	</div>
