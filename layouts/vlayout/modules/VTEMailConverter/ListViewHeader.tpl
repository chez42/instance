				...
				<span class="btn-toolbar span4">
					...
					{foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
						<span class="btn-group">
							{if $LISTVIEW_BASICACTION->getLabel() eq 'LBL_CONFIGURE_ACTIONS'}
								<button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn btn-info"
                                        onclick="javascript:VTEMailConverter_List_Js.configureActionRule();">
									<i class="icon-cog icon-white"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong>
								</button>
							{elseif $LISTVIEW_BASICACTION->getLabel() eq 'LBL_CRON_EXECUTE'}
								<button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}"
										data-url="{$LISTVIEW_BASICACTION->getUrl()}"
										class="btn btn-success"
										onclick="javascript:VTEMailConverter_List_Js.RunCron(this);">
									<i class="icon-refresh icon-white"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong>
								</button>
							{elseif $LISTVIEW_BASICACTION->getLabel() eq 'LBL_MAIL_CONVERTER'}
								<button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}"
										data-url="{$LISTVIEW_BASICACTION->getUrl()}"
										class="btn btn-default"
										onclick="javascript:window.open('{$LISTVIEW_BASICACTION->getUrl()}', '_blank');">
									<i class="icon-filter"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong>
								</button>
							{else}
								<button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn addButton"
									...
									<i class="icon-plus"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong>
								</button>
							{/if}
						</span>
					{/foreach}
