...
<style type="text/css">
	...
	.vte-email-converter-configure-action table tbody td input{width: 95%;}
	.vte-email-converter-configure-action .chzn-container{min-width: 300px;}
	.vte-email-converter-configure-action .chzn-container .chzn-drop{min-width: 298px;}
	.vte-email-converter-configure-action .chzn-container .chzn-search input{min-width: 260px;}
	.vte-email-converter-configure-action .match-field .chzn-container{min-width: 20px; max-width: 100px;}
	.blockUI{left: 0 !important;}
	.vte-email-converter-configure-action .create-if-not-existed .chzn-container{min-width: 10px; max-width: 75px;}
	.vte-email-converter-configure-action .create-if-not-existed{display: inline-block;}
	.modal-open .tooltip{z-index: 20700; !important;}
	...
	<div class="modal-dialog modal-lg  vte-email-converter-configure-action" style="padding: 10px;">
		<form name="vte-email-converter-configure-action-form" action="" method="post" >
			...
			<div class="vte-email-converter-action" style="display: block; margin-bottom: 15px; text-align: center;">
				<select class="chzn-select" name="current_action_module">
				...
				<a style="background-color: #35aa47 ; color: #FFFFFF;" data-url="index.php?module=VTEMailConverter&view=CreateAction" class="btn btn-success pull-right create-new-action-form" href="javascript:void(0);">
					<i class="icon-plus icon-white"></i>&nbsp;&nbsp;{vtranslate('LBL_NEW_ACTION_BTN', $MODULE)}
				</a>
			</div>
			<div class="vte-email-converter-mapping-fields" style="height: 450px; overflow-y: auto;">
				<table class="table table-bordered ">
					...
					<tr class="listViewHeaders" style="background-color: #c9cccf;">
						...
						<td>
							...
							&nbsp;<i class="icon icon-info-sign alignMiddle ep-tooltip" data-toggle="tooltip" title="{Vtiger_Util_Helper::toSafeHTML(vtranslate('LBL_END_WITH_DESC', $MODULE))}"></i>
						</td>
						<td colspan="3" class="advanced-options">
							{vtranslate('LBL_PARSE_EXTRA', $MODULE)}
							&nbsp;<i class="icon icon-info-sign alignMiddle ep-tooltip" data-toggle="tooltip" title="{Vtiger_Util_Helper::toSafeHTML(vtranslate('LBL_PARSE_EXTRA_DESC', $MODULE))}"></i>
						</td>
						<td></td>
						<td>{vtranslate('CRM_FIELDS', $MODULE)}</td>
						<td class="match-field">
							{vtranslate('LBL_MATCH_FIELD', $MODULE)}
							&nbsp;<i class="icon icon-info-sign alignMiddle ep-tooltip" data-toggle="tooltip" title="{Vtiger_Util_Helper::toSafeHTML(vtranslate('LBL_MATCH_FIELD_DESC', $MODULE))}"></i>
						</td>
				...
				<div class="pull-left create-if-not-existed" style="display: {if $ACTION_TYPE eq 'UPDATE'}inline-block {else}none {/if};">
					<select class="chzn-select pull-left" name="rules[create_if_not_existed]">
					...
					<label style="margin-left: 5px; margin-top: 5px; color: #ff0000; display: inline-block;">{vtranslate('LBL_CREATE_IF_RECORD_IS_NOT_FOUND_PLACE_HOLDER', $MODULE)}</label>
				</div>
				<button class="pull-right btn addButton"><i title="{vtranslate('LBL_ADD_FIELD_BTN', $MODULE)}" class="icon-plus"></i>&nbsp;{vtranslate('LBL_ADD_FIELD_BTN', $MODULE)}</button>
			</div>
