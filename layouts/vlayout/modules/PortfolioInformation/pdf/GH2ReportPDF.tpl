...
<div id="GHReport_wrapper">
    ...
    <div class="GHReport_section">
        <h2 style="width:100%; background-color:lightgrey; text-align:center;"><span style="font-size:14px;">{$HEADING} PERFORMANCE ({$YTDPERFORMANCE->GetStartDate()|date_format:'%B, %Y'} to {$YTDPERFORMANCE->GetEndDate()|date_format:'%B, %Y'})</span></h2>
        <table class="table table-bordered" style="display:block; width:100%;">
            ...
            {foreach from=$ytd_individual_performance_summed key=account_number item=v}
                <tr {if $ytd_individual_performance_summed[$account_number]['Flow']->disable_performance eq 1} style="{*background-color:#FFFFE0;*}" {/if}>
                    ...
                    <td style="text-align:right; width:10%">${$ytd_begin_values[$account_number]->value|number_format:2:".":","}</td>
                    <td style="text-align:right; width:10%">${$ytd_individual_performance_summed[$account_number]['Flow']->amount|number_format:2:".":","}</td>
                    <td style="text-align:right; width:10%">${$ytd_individual_performance_summed[$account_number]['income_div_interest']->amount|number_format:2:".":","}</td>
                    {*<td style="text-align:right; width:10%">${$ytd_individual_performance_summed[$account_number]['Income']->amount|number_format:2:".":","}</td>*}
                    {*<td style="text-align:right;">${$ytd_individual_performance_summed[$account_number]['Expense']->amount|number_format:2:".":","}</td>*}
                    <td style="text-align:right; width:10%">${$ytd_end_values[$account_number]->value|number_format:2:".":","}</td>
                    <td style="text-align:right; width:10%">${$ytd_appreciation[$account_number]|number_format:2:".":","}</td>
                    {*                    <td style="text-align:right;">{$ytd_appreciation_percent[$account_number]|number_format:2:".":","}%</td>*}
                    <td style="text-align:right; width:10%">{$ytd_twr[$account_number]|number_format:2:".":","}%</td>
                </tr>
            {/foreach}
            <tr>
