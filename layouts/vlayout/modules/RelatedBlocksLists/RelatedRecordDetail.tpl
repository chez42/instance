        ...
        <table class="table table-bordered equalSplit detailview-table">
            ...
            <tr>
                ...
                <td class="fieldLabel {$WIDTHTYPE}">
                    <label class='muted pull-right marginRight10px'>{vtranslate($tax.taxlabel, $RELMODULE_NAME)}(%)</label>
                </td>
                ...
                {/if}
                {/if}
                <td class="fieldLabel {$WIDTHTYPE}"><label class="muted pull-right marginRight10px">{vtranslate({$FIELD_MODEL->get('label')},{$RELMODULE_NAME})}</label></td>
                <td class="fieldValue {$WIDTHTYPE}">
                ...
                <td class="fieldValue {$WIDTHTYPE}" id="{$RELMODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                    ...
                    {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $FIELD_MODEL->getName() != 'assigned_user_id'}
    ...
    {else}
        {foreach item=FIELD_MODEL from=$FIELDS_LIST name=fields_list_data}
            ...
            {if $FIELD_MODEL->isEditable() eq 'true'}
                <td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {/if} style="white-space:nowrap;">
                    <div class="row-fluid">
                         <span class="value" data-field-type="{$FIELD_MODEL->getFieldDataType()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'} style="white-space:normal;" {/if}>
                             {if ($RELMODULE_NAME eq 'Calendar' OR $RELMODULE_NAME eq 'Events')
                                    AND ($FIELD_MODEL->getName() == 'date_start' OR $FIELD_MODEL->getName() == 'due_date')}
        ...
        <td class="fieldValue">
            <div class="actions pull-right">
                <span class="actionImages">
                    ...
                    {/if}
                    {if $IS_MODULE_DELETABLE}
                        <a class="relatedBtnDelete" data-rel-module="{$RELMODULE_NAME}" data-record-id="{$RELATED_RECORD_MODEL->getId()}"><i class="icon-trash alignMiddle" title="Delete"></i></a>
                    {/if}
                </span>
            </div>
        </td>
