...
{foreach from=$RECORD->getChildren() item=CHILD_RECORD}
	<li data-role="{$CHILD_RECORD->getParentString()}" data-roleid="{$CHILD_RECORD->getId()}">
			...
			{*{if $VIEW_NAME != 'Popup'}*}
			<div class="toolbar">
				&nbsp;<a class='btnDelete' href="{$CHILD_RECORD->getCreateChildUrl()}" data-url="{$CHILD_RECORD->getCreateChildUrl()}" title="{vtranslate('LBL_ADD_RECORD', 'VTEForecast')}"><span class="icon-plus-sign"></span></a>
				&nbsp;<a class='btnRemove' data-id="{$CHILD_RECORD->getId()}" href="javascript:;" data-url="{$CHILD_RECORD->getDeleteActionUrl()}" data-action="modal" title="{vtranslate('LBL_DELETE', 'VTEForecast')}"><span class="icon-trash"></span></a>
			</div>
			{*{/if}*}
		</div>
