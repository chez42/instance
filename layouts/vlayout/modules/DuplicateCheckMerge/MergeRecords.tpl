...
<div style='background: white;'>
	...
	<form class="form-horizontal contentsBackground" name="massMerge" method="post" action="index.php">
		...
		<input type="hidden" name="pmodule" value="{$MODULE}" />
		...
		<table class='table table-bordered table-condensed'>
			...
			{foreach item=RECORD from=$RECORDMODELS name=recordList}
				<th style="vertical-align: middle;">
                    ...
                    <br>
                    {vtranslate($FIELDS['createdtime']->get('label'), $MODULE)}:&nbsp;{$RECORD->getDisplayValue('createdtime')}
                    <br>
                    {vtranslate($FIELDS['modifiedtime']->get('label'), $MODULE)}:&nbsp;{$RECORD->getDisplayValue('modifiedtime')}
				</th>
			{/foreach}
			</thead>
