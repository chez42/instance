...
Vtiger_Edit_Js("Quoter_Js", {
    ...
    addActionForProductBundle: function () {
        ...
        if(record){
            ...
            AppConnector.request(params).then(
                function (data) {
                    ...
                    if(record && level < 3){
                        var newGroupButton = '<div class="btn-group"><button type="button" class="btn addButton selectRelationNew" data-modulename="Products">&nbsp;<strong>Select Products</strong></button></div><div class="btn-group"><button type="button" class="btn addButton " data-url="index.php?module=Products&amp;view=Edit&amp;sourceModule=Products&amp;sourceRecord=3&amp;relationOperation=true" name="addButton"><i class="icon-plus icon-white"></i>&nbsp;<strong>Add Product</strong></button></div>';
                        jQuery('.relatedHeader .btn-toolbar div.span6:first').html(newGroupButton);
                        thisInstance.registerEventForSelectSubProducts();
                    }
                }
            );
    ...
    calculateShippingAndHandlingTaxCharges : function() {
        ...
        jQuery('#s_h_percent').val(currentTaxTotal);
    ...
    updateLineItemsElementWithSequenceNumber : function(lineItemRow,expectedSequenceNumber , currentSequenceNumber){
        ...
        jQuery.each(this.columnSetting, function (index,value) {
            var regex = /cf_/gi;
    ...
    fixFieldName:function(){
        var setting = this.columnSetting;
        if(setting){
    ...
    registerEventForSectionDropDown: function () {
        ...
        sectionContainer.find('.section_item').on('click', function () {
            ...
            var trElement = '<tr class="section">' +
                '<td class="fieldLabel" colspan="'+(parseInt(columnCount)-1)+'" style = "font-size: 14px;">' +
                ...
                '<input type = "hidden" class ="section_value" name="section0" value="'+sectionValue+'" />' +
                '</td>' +tdSection+
                '</tr>';
    ...
    registerEventForRunningSubTotalDropDown: function () {
        ...
        runningSubTotalItemContainer.find('.runningSubTotalItem').on('click', function () {
            ...
            var trElement = '<tr class="running_item" data-running-item-name = "'+itemName+'" data-running-item-rowno = "'+rowNo+'">' +
                ...
                '</span>&nbsp;&nbsp;' +
                '</td>'+tdLast+'<td colspan="'+(parseInt(columnCount)-2)+'" style="border-left:0;">'+
...
jQuery(document).ready(function(){
    ...
    }else if(targetModule == 'Products' && targetView == 'Detail' && targetMode == 'showRelatedList'){
        var currentRelateButton = jQuery('.related ul li.active');
        if(currentRelateButton.data('label-key')== 'Product Bundles'){
            Quoter_Js.addActionForProductBundle();
        }
...
jQuery(document).ajaxComplete(function( event, xhr, settings) {
    var currentRelateButton = jQuery('.related ul li.active');
    if(currentRelateButton.length > 0 && currentRelateButton.data('label-key') == 'Product Bundles' && xhr.responseText.indexOf("relatedContainer") != -1 ){
        if(xhr.responseText.indexOf("relatedContainer") != -1){
            Quoter_Js.addActionForProductBundle();
        }

    }

    var url = decodeURIComponent(settings.data);
