...
class Premailer
{
    ...
    protected static function convert($html = "", $url = "", $fetchresult = true, $adaptor = "hpricot", $base_url = "", $line_length = 65, $link_query_string = "", $preserve_styles = true, $remove_ids = false, $remove_classes = false, $remove_comments = false)
    {
        ...
        if ($adaptor == "hpricot" || $adaptor == "nokigiri") {
        ...
        $options = array("timeout" => 15, "connecttimeout" => 15, "useragent" => "PHP Premailer", "ssl" => array("verifypeer" => false, "verifyhost" => false));
        $conf = array("url" => self::ENDPOINT, "timeout" => 30, "useragent" => "PHP Premailer", "ssl_verifyhost" => 0, "SSL_VERIFYPEER" => 0, "post" => 1, "postfields" => $params, "returntransfer" => true, "httpheader" => array("Expect:"));
        ...
        if ($_res_info->http_code != 201) {
            ...
            if ($fetchresult) {
                $html = curl_init();
                curl_setopt_array($html, array(CURLOPT_URL => $_res->documents->html, CURLOPT_TIMEOUT => 15, CURLOPT_USERAGENT => "PHP Premailer", CURLOPT_SSL_VERIFYHOST => 0, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_HTTPHEADER => array("Expect:"), CURLOPT_RETURNTRANSFER => true));
                $return["html"] = curl_exec($html);
                curl_close($html);
                $plain = curl_init();
                curl_setopt_array($plain, array(CURLOPT_URL => $_res->documents->txt, CURLOPT_TIMEOUT => 15, CURLOPT_USERAGENT => "PHP Premailer", CURLOPT_SSL_VERIFYHOST => 0, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_HTTPHEADER => array("Expect:"), CURLOPT_RETURNTRANSFER => true));
