...
class HelpDesk extends CRMEntity {
	...
	function saveViewPermissionIds(){
		...
		$view_permission = $this->column_fields['view_permission'];
		
		$view_permission = explode(" |##| ", $view_permission);
		
		if(!in_array($this->column_fields["assigned_user_id"], $view_permission))
			$view_permission[] = $this->column_fields["assigned_user_id"];
		
		if(!empty($view_permission)){
			
			$permission_result = $adb->pquery("select * from vtiger_ticket_view_permission where ticketid = ?",array($this->id));
			
			if($adb->num_rows($permission_result) > 0)
				$adb->pquery("delete from vtiger_ticket_view_permission where ticketid = ?",array($this->id));
				
			$permission_sql = "insert into vtiger_ticket_view_permission(ticketid, view_permission_id) values";
			
			foreach($view_permission as $v_pid){
				$v_pid = str_replace(" ", "", $v_pid);
				if($v_pid == '' ) continue;
				$values .= " (". $this->id.",".$v_pid."),";
			}
...
case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name
	...
	// Changes for record Permission 28-Aug-2018
	            ...
	            if (!empty($sharingRuleInfo) && (count($sharingRuleInfo['ROLE']) > 0 ||
	                ...
	                if($scope == ''){
	                    ...
	                    if($module == 'Documents'){
        					...
        					($tableName$scope.id = vtiger_crmentity$scope.smownerid AND vtiger_crmentity$scope.smcreatorid != $user->id) OR
        					($tableName$scope.id = vtiger_crmentity$scope.smcreatorid AND $tableName$scope.id = $user->id)";
	...
	function get_tasks($id, $cur_tab_id, $rel_tab_id, $actions=false) {
	    ...
	    if($singlepane_view == 'true')
    		...
    		inner join vtiger_troubletickets on vtiger_troubletickets.ticketid = vtiger_task.parent_id
    		left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
    		left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
