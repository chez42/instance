...
class Emails_Module_Model extends Vtiger_Module_Model{
	...
	public function getModuleBasicLinks(){
	    ...
	    if($createPermission) {
            $basicLinks[] = array(
                ...
                'linkurl' => 'javascript:Emails_MassEdit_Js.triggerSendEmailList("index.php?module='.$moduleName.'&view=MassActionAjax&mode=showComposeEmailForm&step=step1","Emails");',
                'linkicon' => 'fa-plus'
            );
	    }
	    return $basicLinks;
	...
	public function getParentRecordsValus($values ,$uitypeModel){
	    $module = CRMEntity::getInstance($this->getName());
	    
	    $records = $module->getCRMIdsFromStringPattern($values);
	    
	    $finalValue = array();
	    foreach ($records as $record){
	       $finalValue[] = $uitypeModel->getDisplayValue($record);
	    }
