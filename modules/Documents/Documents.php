...
class Documents extends CRMEntity {
	...
	function save_module($module)
	{
		...
		if($this->mode == '' && isset($_REQUEST['sourceRecord']) && isset($_REQUEST['sourceModule'])){
...
//	Function to get fieldname for uitype 28 assuming that doc has only one file upload type
	...
	function updateOwner() {
	    ...
	    $owner = $adb->pquery("SELECT smownerid FROM vtiger_crmentity
            WHERE crmid = ?",array($_REQUEST['sourceRecord']));
	    
	    $ownerId = $adb->query_result($owner,0,'smownerid');
	    
	    $adb->pquery("UPDATE vtiger_crmentity SET smownerid = ?
            WHERE crmid = ?",array($ownerId,$this->id));
