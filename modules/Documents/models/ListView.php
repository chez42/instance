...
class Documents_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewMassActions($linkParams) {
		...
		$documentFolderModel = Vtiger_Module_Model::getInstance('DocumentFolder');
		if($documentFolderModel->isPermitted('CreateView')) {
		    $massActionLinks[] = array(
		        'linktype' => 'LISTVIEWMASSACTION',
		        'linklabel' => 'LBL_ADD_FOLDER',
		        'linkurl' => 'javascript:Documents_List_Js.addFolder("index.php?module=' . $moduleModel->getName() . '&action=MassDelete");',
		        'linkicon' => ''
		    );
		    $links['LISTVIEWMASSACTION'][] = Vtiger_Link_Model::getInstanceFromValues($massActionLink);
		}
		...
		$massActionLinks[] = array(
		    'linktype' => 'LISTVIEWMASSACTION',
		    'linklabel' => 'Export as Zip file',
		    'linkurl' => 'javascript:Documents_List_Js.exportZip("index.php?module=' . $moduleModel->getName() . '&view=ExportZip");',
		    'linkicon' => ''
		);
	...
	public function getListViewEntries($pagingModel) {
        ...
        if(!empty($folderValue)&&$folderValue!='xxx') {
	...
	function getQuery() {
	    ...
	    if($this->get('listmode') == 'pdffiles'){
	        
	        /*$listQuerycom = explode("WHERE",$query);
	        
	        $listQuerycom[0] = str_replace("INNER JOIN vtiger_attachmentsfolder vtiger_attachmentsfolderfolderid  ON vtiger_notes.folderid = vtiger_attachmentsfolderfolderid.folderid",
	            " ",$listQuerycom[0]);
	        $listQuerycom[0].= " INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.crmid = vtiger_notes.notesid
            INNER JOIN vtiger_attachments ON vtiger_attachments.attachmentsid = vtiger_seattachmentsrel.attachmentsid ";
	        
	        $query = implode(" WHERE ",$listQuerycom);
	        */
	        $query .= ' AND vtiger_notes.filelocationtype = "I"';
