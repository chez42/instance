...
class OmniMail_Lead_Model extends Leads_Module_Model{
    var $lead_id, $module, $action, $method, $subject, $fromname, $from, $toname, $to, $body, $fname, $lname, $office, $mobile, $homephone, $email;
     ...
     * @return type
     */
    public function GetLeadRecordModel($record_id){
        if($contact_id)
            $recordModel = Vtiger_Record_Model::getInstanceById($record_id, 'Leads');
        else
    ...
    public function LoadInfo(Vtiger_Request $request){
        ...
        $this->lead_id =    $request->get('record_id');
