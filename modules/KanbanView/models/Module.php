...
class KanbanView_Module_Model extends Vtiger_Module_Model
{
    ...
    {
        $settingsLinks[] = array("linktype" => "MODULESETTING", "linklabel" => "Settings", "linkurl" => "index.php?module=KanbanView&parent=Settings&view=Settings", "linkicon" => "");
        $settingsLinks[] = array("linktype" => "MODULESETTING", "linklabel" => "Uninstall", "linkurl" => "index.php?module=KanbanView&parent=Settings&view=Uninstall", "linkicon" => "");
    ...
    public function getRecordIdSequence($listRecordId, $module)
    {
        ...
        $resultMaxId = $adb->pquery("SELECT MAX(sequence) as max_id FROM kanban_sequence WHERE module =?", array($module));
        $maxId = 1;
        if (0 < $adb->num_rows($resultMaxId)) {
        ...
        foreach ($listRecordId as $recordId) {
            ...
            if ($adb->num_rows($rs) == 0) {
                $maxId++;
                $adb->pquery("INSERT INTO kanban_sequence(crmid,module,sequence) VALUES(?,?,?);", array($recordId, $module, $maxId));
            }
        }
        $recordCondition = implode(",", $listRecordId);
        $rsSequence = $adb->pquery("SELECT crmid FROM kanban_sequence WHERE module = ? AND crmid IN (" . $recordCondition . ") ORDER BY sequence ASC", array($module));
        if (0 < $adb->num_rows($rsSequence)) {
    ...
    public function getSequence($crmId, $module, $primary_field, $primary_value)
    {
        ...
        $userModel = Users_Record_Model::getCurrentUserModel();
        $username = $userModel->get("user_name");
        $rsRecord = $adb->pquery("SELECT * FROM kanban_sequence WHERE crmid = " . $crmId . " AND username = ?", array($username));
        if (0 < $adb->num_rows($rsRecord)) {
            return $adb->query_result($rsRecord, 0, "sequence");
        }
        $seqNum = 1;
        $rsMaxSeq = $adb->pquery("SELECT MAX(sequence) as max_id FROM kanban_sequence WHERE module =? AND username = ?", array($module, $username));
        $maxSeq = $adb->query_result($rsMaxSeq, 0, "max_id");
        if ($maxSeq) {
            $seqNum = $maxSeq + 1;
        }
        $adb->pquery("INSERT INTO kanban_sequence(crmid,module,sequence,`primary_field_id`,`primary_field_value`,`username`) VALUES (?,?,?,?,?,?);", array($crmId, $module, $seqNum, $primary_field, $primary_value, $username));
        return $seqNum;
    }
    public function getCurrentSequence($crmId, $username)
    {
        global $adb;
        $seqNum = -1;
        $rs = $adb->pquery("SELECT * FROM kanban_sequence WHERE crmid = ? AND username = ?", array($crmId, $username));
        if (0 < $adb->num_rows($rs)) {
