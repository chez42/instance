...
class Accounts extends CRMEntity {
	...
	*/
	function get_tickets($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if(!empty ($entityIds)){
	                $query .= " OR vtiger_troubletickets.contact_id IN (".$entityIds.") OR vtiger_troubletickets.parent_id IN (".$entityIds.") )";
		} else {
			$query .= ")";
		}
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset);
	...
	*/
	...
	function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$entityIds = $this->getRelatedContactsIds();
		array_push($entityIds, $id);
		$ticket_ids = $this->getRelatedTicketIds($entityIds);
		$portfolio_ids = $this->getRelatedPortfolioIds($id);
		$entityIds = array_merge($entityIds, $ticket_ids, $portfolio_ids);
		$entityIds = implode(',', $entityIds);
	...
	function getRelatedTicketIds($id) {
		...
		$query = "SELECT DISTINCT vtiger_crmentity.crmid FROM vtiger_troubletickets INNER JOIN vtiger_crmentity ON 
					...
					vtiger_troubletickets.parent_id IN (".implode(',',$id).")";
	    ...
	    if($singlepane_view == 'true')
	        $returnset = '&return_module='.$this_module.'&return_action=DetailView&return_id='.$id;
	            ...
	            $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
	                'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
	            $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
        		vtiger_crmentity.*, vtiger_contactdetails.*, vtiger_task.subject, vtiger_task.date_start, vtiger_task.time_start,
        		...
        		inner join vtiger_account on vtiger_account.accountid = vtiger_task.parent_id
        		left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
        		left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
        		left join vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_task.contact_id
        		where vtiger_task.parent_id=".$id." and vtiger_crmentity.deleted=0";
	...
	function get_positions($id, $cur_tab_id, $rel_tab_id, $actions = false){
        ...
        if(!empty($ssn)){
            ...
            if(!empty($account_numbers)){
                ...
                foreach($account_numbers AS $key => $acc_num){
                    $account_numbers[$key] = str_replace('-', '', $acc_num);
                }
		...
		WHERE vtiger_crmentity.deleted = 0 AND vtiger_positioninformation.account_number IN (".$account_numbers.")
		AND vtiger_positioninformation.quantity != 0 GROUP BY vtiger_positioninformation.security_symbol";
        ...
        $query = "SET @global_total = (SELECT SUM(current_value) FROM vtiger_positioninformation WHERE account_number IN ($account_numbers))";
	...
	function get_portfolioinformation($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        ...
        LEFT JOIN vtiger_account ON vtiger_account.accountid = vtiger_portfolioinformation.household_account		
        LEFT JOIN vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
		LEFT JOIN vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
