...
class Trading_Ameritrade_Model extends Vtiger_Base_Model{
    ...
    public function __construct($userid="Omniscient", $password="test123456", $duration_minutes=1) {
        ...
        $this->notification_emails = "rsandnes@glenmerrybowl.com,felipe.luna@omnisrv.com";
    }
     ...
     * @param type $xml_data
     */
    private function GetCurlJsonData($url, $header, $xml_data, $assoc=false, $timeout=150)
    {
        $ch = curl_init($url);
...
/*                    echo $query . "<br />";
    ...
    public function GetAllAccountsForRepCode($url, $rep_codes=null, $accounts=null, $startIndex=null, $endIndex=null){
        if(!is_array($accounts))
            $account_numbers[] = $accounts;
        else
            $account_numbers = $accounts;
        ...
        if(sizeof($rep_codes) > 0){
            foreach($rep_codes AS $k => $v){
                $rep_code_text .= "<repCode>{$v}</repCode>";
            }
        }else{
            $rep_code_text .= "<repCode></repCode>";
        }
    ...
    public function GetBalances($url, $accounts, $date = null, $startIndex=null, $endIndex=null){
        ...
        $xml_data .="</accountNumbers>
            ...
            $result = $this->GetCurlJsonData($url, $header, $xml_data, true, 150);//Get balances with 150 second timeout
            if(strlen($result['error']) > 1){
                $msg = "There was an error.... " . $result['error'] . "....";
                $msg .= "Trying again with 300 second timeout: ";
                $result = $this->GetCurlJsonData($url, $header, $xml_data, true, 300);
                if(strlen($result['error']) > 1){
                    $msg .= $result['error'];
                    echo $msg;
                    mail($this->notification_emails, "TD API Balance Error Needs Attention", $msg);
                }else{
                    $msg .= " Successful Recovery";
                    echo $msg;
                    mail($this->notification_emails, "TD API Balance Error Recovered", $msg);
                }
            }
            return $result;
        }
...
//        echo "Logged out...";
    ...
    public function KillSession(){
        global $adb;
        $this->CloseSession("https://veoapi.advisorservices.com/InstitutionalAPIv2/api");
        $query = "DELETE FROM ameritrade_session WHERE user = ?";
        $adb->pquery($query, array($this->userid));
        return "Session Closed";
    }
