...
#            echo "RETURNING 0 -- {$symbol}, {$date}<br />";
	...
	function getWidgetTransactions($headerColumns, $pagingModel, $tradeDates, $transaction_activity){
		...
		$query .= " AND vtiger_transactionscf.transaction_type = 'Flow' AND transaction_activity IN (".generateQuestionMarks($transaction_activity).")  ";
		...
		$result = $db->pquery($query, array($transaction_activity));
