...
class ModSecurities_SecurityBridge_Model extends Vtiger_Module{
    ...
    private function GetTemporarySecurityList(){
        ...
        $query = "SELECT * FROM SecurityFinalList";
        $list = array();
    ...
    private function CreateTemporarySecurityList($symbol=null){
        ...
        $adb->pquery($query, array());
        $data = cPortfolioCenter::GetDatasets();
        $query = "CREATE TEMPORARY TABLE SecurityIDList 
                        ...
                        WHERE security_data_set_id IN ({$data})
                        {$and}
    ...
    public static function UpdateModSecuritiesSecurityIDWhenEmpty(){
        global $adb;
        $query = "UPDATE vtiger_modsecurities m
                  JOIN vtiger_securities s ON m.security_symbol = s.security_symbol
                  SET m.security_id = s.security_id
                  WHERE m.security_id = ''";
        $adb->pquery($query, array());
    }

    public static function UpdateEmptySecurityTypes(){
        global $adb;
        $query = "UPDATE vtiger_modsecurities ms
        JOIN vtiger_securities s ON ms.security_id = s.security_id
        JOIN vtiger_security_types t ON t.security_type_id = s.security_type_id
        SET ms.security_type = t.security_type_name
        WHERE ms.security_type = '' OR ms.security_type is null
        AND ms.security_id = s.security_id";
        $adb->pquery($query, array());
        ...
        SET cf.security_type = ms.security_type
        WHERE cf.security_type = '' or cf.security_type is null";
        $adb->pquery($query, array());
    }

    public static function UpdateAllSecurityTypes(){
        global $adb;
        $query = "UPDATE vtiger_modsecurities ms
                  (SELECT code_id FROM vtiger_pc_security_codes WHERE security_id = ms.security_id AND code_type_id = 10)";
    ...
    /**
     * Updates all securities and attach them to the proper households/contacts based on what their PortfolioInformation counterpart is connected to
     * @global type $adb
     */
    public static function ConnectSecuritiesToHHAndContact(){
        global $adb;
        $query = "UPDATE vtiger_positioninformation pin
                  JOIN vtiger_portfolioinformation por ON pin.account_number = por.account_number
                  SET pin.household_account = por.household_account, pin.contact_link = por.contact_link";
        $adb->pquery($query, array());
    }
...
//        $query = "SELECT * FROM SecurityPrices WHERE SecurityPriceID = {$price_id}";
    ...
    public static function GetCodeIDByAssetClassName($asset_class_name){
        ...
        $data = cPortfolioCenter::GetDatasets();
        $query = "SELECT code_id FROM vtiger_pc_codes WHERE code_description = ? AND code_type_id = 20 AND data_set_id IN ({$data}) ORDER BY data_set_id, code_order ASC";
    ...
    public static function GetCodeIDBySectorClassName($sector_name){
        ...
        $datasets = self::$datasets;
        $data = cPortfolioCenter::GetDatasets();
        $query = "SELECT code_id FROM vtiger_pc_codes WHERE code_description = ? AND code_type_id = 10 AND data_set_id IN ({$data}) ORDER BY data_set_id, code_order ASC";
    ...
    public static function GetSecurityTypeIDByTypeName($type_name){
        global $adb;
        $query = "SELECT security_type_id FROM vtiger_security_types WHERE security_type_name = ?";
        $result = $adb->pquery($query, array($type_name));
        if($adb->num_rows($result) > 0)
            return $adb->query_result($result, 0, 'security_type_id');
        return 0;
    }
    
    public static function UpdateSecurityTypeID($security_id, $type_id, $update_pc){
        global $adb;
        if($security_id != 0 && $type_id != 0){
            $query = "UPDATE vtiger_securities SET security_type_id = ? WHERE security_id = ?";
            $adb->pquery($query, array($type_id, $security_id));
            if($update_pc){
                self::UpdatePCSecurityTypeID($security_id, $type_id, $code_id);
            }
        }
    }
    ...
    public static function UpdatePayFrequencyCodeID($security_id, $frequency_id, $update_pc = 0){
        ...
        if($security_id != 0 && $frequency_id != 0){
            ...
            if($update_pc){
                self::UpdatePCPayFrequencyCodeID($security_id, $type_id);
            }
        }
    }
    ...
    public static function UpdatePCSecurityTypeID($security_id, $type_id){
        if($type_id != 0 && $security_id != 0){
            include_once('include/utils/cron/cPortfolioCenter.php');
            $pc = new cPortfolioCenter("pctestserver", "syncuser", "Concert222");        
            $query = "UPDATE Securities SET SecurityTypeID = {$type_id} WHERE SecurityID = {$security_id}";
            $r = $pc->CustomQuery($query);
            return $r;
        }
    }
