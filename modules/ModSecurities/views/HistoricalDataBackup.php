<?php
/**
 * This is a backup file from before the new vtiger_prices table was created
 *
 */
/* +***********************************************************************************
...
class ModSecurities_HistoricalData_View extends Vtiger_Detail_View {
    public function process(Vtiger_Request $request, $display=true) {
        ...
        if($calling_record){
            ...
            $prices = $request->get('advisor_prices') ? ModSecurities_HistoricalData_Model::GetHistoricalPricesWithVolumeAdvisor($record->get('security_id'), "ASC") :
                ModSecurities_SecurityBridge_Model::GetAllHistoricalPricesWithVolume($record->get('security_id'), "ASC");
        }else if($request->get('security_id')){
...
//        echo "SECURITY ID: {$request->get('security_id')}<br />";
        foreach($prices AS $k => $v){
            $price_data[] = array("date" => $v['stockFormat']. ' 00:00:00 GMT-700 (PDT)',
                "value" => $v['price'],
                "volume" => $v['volume']);
        }
