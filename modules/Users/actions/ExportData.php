...
class Users_ExportData_Action extends Vtiger_ExportData_Action{
    ...
    public function getHeaders() {
		...
		if(!empty($this->accessibleFields)) {
			...
			$skipUitypes = array(69,105);
			foreach($this->accessibleFields as $fieldName) {
				$fieldModel = $this->moduleFieldInstances[$fieldName];
				
				if(in_array($fieldModel->get('uitype'),$skipUitypes) || $fieldModel->getName() == 'signature')
				    continue;
    ...
    function getExportQuery(Vtiger_Request $request) {
        ...
        /*foreach($fieldInstances as $field) {
            ...
            if(in_array($field->get('uitype'),$skipUitypes) || $field->getName() == 'signature')
                continue;
                
            if(in_array($presence, $accessiblePresenceValue) && $field->get('displaytype') == '1') {
    ...
    function sanitizeValues($arr){
        ...
        foreach($arr as $fieldName=>&$value){
            ...
            if($uitype == 901){
                ...
                if($value)
                    $value = strip_tags($fieldInfo->getUITypeModel()->getDisplayValue($value));
                    
            } else if($uitype == 98){
                
                $value = getRoleName($value);
                
            } else if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
