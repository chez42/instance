...
class Users_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	function __construct() {
		...
		$this->exposeMethod('updateTurnOfConfirmation');
	}
	...
	public function checkPermission(Vtiger_Request $request) {
		...
		if(!$currentUserModel->isAdminUser()) {
			...
			if($mode == 'savePassword' && (isset($userId) && $currentUserModel->getId() != $userId)) {
				...
				throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
			} else if($mode != 'savePassword' && $mode != 'updateTurnOfConfirmation' && ($currentUserModel->getId() != $request->get('record'))) {
			    throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
			}else if($mode == 'updateTurnOfConfirmation' && (isset($userId) && $currentUserModel->getId() != $userId)){
			    throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
			}
		}
	}
	 ...
	 */
	public function updateTurnOfConfirmation(Vtiger_Request $request) {
	    $response = new Vtiger_Response();
	   
	    $userId = $request->get('userid');
	    
	    if($request->get("turn_of_confirmation")){
	        
	        global $adb;
	        
	        $adb->pquery("UPDATE vtiger_users SET turn_of_confirmation = 1 WHERE id = ?",array($userId));
