...
class Users extends CRMEntity {
   ...
   //END
	...
	function saveUserLogo($id,$module,$userLogo){
		...
		vtiger_salesmanattachmentsrel.smid=? and vtiger_crmentity.setype = ?", array($this->id, "User Logo")),0,'crmid');
	    
	    $file_saved = false;
	    
	    foreach($userLogo as $fileindex => $files){
	        if($files['name'] != '' && $files['size'] > 0){
	            $files['original_name'] = vtlib_purify($_REQUEST[$fileindex.'_hidden']);
	            $file_saved = $this->uploadAndSaveUserLogo($id,$module,$files);
	        }
	    }
	    
	    $imageNameSql = 'SELECT vtiger_attachments.name FROM vtiger_salesmanattachmentsrel
	    ...
	    $imageNameResult = $adb->pquery($imageNameSql,array($this->id, "User Logo"));
	    $imageName = decode_html($adb->query_result($imageNameResult, 0, "name"));
	    
	    //Inserting image information of record into base table
	    $adb->pquery('UPDATE vtiger_users SET user_logo = ? WHERE id = ?',array($imageName,$this->id));
	...
	function uploadAndSaveUserLogo($id,$module,$file_details) {
	    global $adb, $current_user;
	    global $upload_badext;
        ...
        if($width > 200 && $height > 200){
            ...
            if(($filetype == "image/jpeg") || ($filetype == "image/png") || ($filetype == "image/jpg")){
                ...
                if($filetype == "image/jpeg" || $filetype == "image/jpg"){
                    $logo_image = $this->resize_imagejpg($filetmp_name, 200, 200);
                    $upload_status = imagejpeg($logo_image, $upload_file_path . $current_id . "_" . $binFile, 100);
                } else if ($filetype == "image/png"){
                    $logo_image = $this->resize_imagepng($filetmp_name, 200, 200);
                    $upload_status = imagepng($logo_image, $upload_file_path . $current_id . "_" . $binFile, 8);
                }
            }
        } else {
            $upload_status = move_uploaded_file($filetmp_name, $upload_file_path . $current_id . "_" . $binFile);
        ...
        if ($save_file == 'true' && $upload_status == 'true') {
            ...
            $params1 = array($current_id, $current_user->id, $ownerid, "User Logo", $adb->formatDate($date_var, true), $adb->formatDate($date_var, true));
    		...
    		WHERE vtiger_crmentity.setype =  'User Logo' AND vtiger_salesmanattachmentsrel.smid =?";
