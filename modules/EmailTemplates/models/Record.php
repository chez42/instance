...
class EmailTemplates_Record_Model extends Vtiger_Record_Model {
	...
	public static function getInstanceById($templateId, $module=null) {
		...
		$sql = "SELECT CONVERT( GROUP_CONCAT( vtiger_emailtemplates_view_permission.view_permission_id SEPARATOR ' |##| ') USING  'utf8' ) AS template_view_permission,
		vtiger_emailtemplates.* FROM  vtiger_emailtemplates
		INNER JOIN vtiger_emailtemplates_view_permission ON vtiger_emailtemplates_view_permission.template_id = vtiger_emailtemplates.templateid
	...
	public function getTemplateViewPermissionRecordNameList(){
	    
	    $view_permission_ids = $this->get("template_view_permission");
	    
	    if($view_permission_ids){
	        
	        $view_permission_ids = explode(" |##| ", $view_permission_ids);
	        
	        $currentUser = Users_Record_Model::getCurrentUserModel();
	        
	        if(!$currentUser->isAdminUser()){
	            
	            $displayvalue = getOwnerNameList($view_permission_ids);
	            
	        } else {
	            
	            foreach($view_permission_ids as $view_permission_id){
	                
	                if(getUserFullName($view_permission_id)){
	                    $userModel = Users_Record_Model::getCleanInstance('Users');
	                    $userModel->set('id', $view_permission_id);
	                    $detailViewUrl = $userModel->getDetailViewUrl();
	                } else {
	                    $recordModel = new Settings_Groups_Record_Model();
	                    $recordModel->set('groupid',$view_permission_id);
	                    $detailViewUrl = $recordModel->getDetailViewUrl();
	                }
	                $displayvalue[] = "<a href=" .$detailViewUrl. ">" .getOwnerName($view_permission_id). "</a>&nbsp";
	            }
	        }
	...
	function checkEmailTemplateViewPermission(){
        ...
        $templateCreator = $this->get("creatorid");
        
        if($templateCreator == $currentUser->id)
            return true;
        ...
        if($view_permission_ids){
            ...
            if(in_array($currentUser->id, $view_permission_ids))
                return true;
                
            $groups = fetchUserGroupids($currentUser->id);
            
            if($groups){
               $groups = explode(",",$groups);
            }
            
            $is_permitted = array_intersect($groups, $view_permission_ids);
