...
class EmailTemplates_Field_Model extends Vtiger_Field_Model {
	...
	/*public static function getAllForModule($moduleModel){
		...
		if(empty(self::$allFields)) {
			...
			$secondBlockFields = parent::getAllForModule($moduleModel);
			...
			foreach($secondBlockFields as $block){
			    foreach($block as $fieldName=>$fieldLabel){
    				$fieldModel = new EmailTemplates_Field_Model();
    				$blockModel = $blocks['LBL_EMAIL_TEMPLATE'];
    				$fieldModel->set('name',$fieldLabel->name)->set('label',$fieldLabel->label)->set('block',$blockModel);
    				$fieldsList[$blockModel->get('id')][] = $fieldModel;
			    }
			}
