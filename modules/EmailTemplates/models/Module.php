	...
	*/
	public function saveRecord(EmailTemplates_Record_Model $recordModel) {
		...
		if(empty($templateid)){
			...
			$sql = "INSERT INTO vtiger_emailtemplates(templatename, subject, description, module, body, deleted, systemtemplate, creatorid, templateid) VALUES (?,?,?,?,?,?,?,?,?)";
		}else{
			if($systemtemplate) {
				$sql = "UPDATE vtiger_emailtemplates SET templatename=?, description=?, module=?, body=?, deleted=?, systemtemplate=?, creatorid=? WHERE templateid = ?";
			} else {
				$sql = "UPDATE vtiger_emailtemplates SET templatename=?, subject=?, description=?, module=?, body=?, deleted=?, systemtemplate=?, creatorid=? WHERE templateid = ?";
			}
		}
		if(!empty($recordId) && $systemtemplate) {
			$params = array(decode_html($recordModel->get('templatename')), decode_html($recordModel->get('description')),
			    $recordModel->get('module'),$recordModel->get('body'), 0, $systemtemplate,$recordModel->get('creator'), $templateid);
		} else {
			$params = array(decode_html($recordModel->get('templatename')), decode_html($recordModel->get('subject')),
			    ...
			    $systemtemplate, $recordModel->get('creator'), $templateid);
		}
	...
	public function deleteRecord(EmailTemplates_Record_Model $recordModel) {
		...
		$db->pquery('DELETE FROM vtiger_emailtemplates_view_permission WHERE template_id = ? ', array($recordId));
	}
	...
	public function deleteAllRecords() {
		...
		if($currentUser->isAdminUser()){
		  ...
		  $db->pquery('DELETE FROM vtiger_emailtemplates_view_permission', array());
		  
		}else {
		    
		    $temp_result = $db->pquery("select templateid from vtiger_emailtemplates where creatorid = ?", array($currentUser->id));
		    
		    if($db->num_rows($temp_result)){
		        
		        $template_ids = array();
		        
		        while($template = $db->fetchByAssoc($temp_result)){
		            $template_ids = array_merge($template_ids, array($template['templateid']));
		        }
		        
		        if(!empty($template_ids))
		            $db->pquery('DELETE FROM vtiger_emailtemplates_view_permission where template_ids in ('.generateQuestionMarks($template_ids).')', array($template_ids));
		    }
		    $db->pquery('DELETE FROM vtiger_emailtemplates where creatorid = ?', array($currentUser->id));
		}
