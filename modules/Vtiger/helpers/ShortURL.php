...
class Vtiger_ShortURL_Helper {
	...
	static function generate(array $options) {
		...
		$uid = '';
		if($options['handler_class'] == 'Documents_DocumentViewer_Handler'){
		    
		    $docData = json_encode($options['handler_data']);
		    
		    $document = $db->pquery("SELECT * FROM vtiger_shorturls WHERE handler_class = ? AND handler_data = ?",
		        array($options['handler_class'],$docData)); 
		    
		    if($db->num_rows($document))
		        $uid = $db->query_result($document, 0, 'uid');
