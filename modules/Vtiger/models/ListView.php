...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public static function getInstance($moduleName, $viewId='0', $listHeaders = array()) {
		...
		if(!empty($querySorting)){
		    ...
		    if($currentModule == $moduleName && isset($querySorting['columnname']) && $querySorting['columnname'] != 'none'){
		        $orderbyFieldname = explode(":",$querySorting['columnname']);
		        $fieldModel = Vtiger_Field_Model::getInstance($orderbyFieldname[2], $moduleModel);
		        if($moduleName != "Documents")//MANISH: PositionInformation is causing issues for us
	...
	public function getForSql($key, $skipEmtpy=true) {
	    $value = parent::getForSql($key);
	    if($key == 'orderby'){
	        if(!$value){
	            $value = $this->get("cv_orderby");
	            $queryGenerator =  $this->get('query_generator');
	            if(!empty($value)){
	                $orderbyFieldname = explode(".",$value);
	                if(!in_array($orderbyFieldname[1], $queryGenerator->getFields())){
	                    $headerFields = $queryGenerator->getFields();
	                    $headerFields[] = $orderbyFieldname[1];
	                    $queryGenerator->setFields($headerFields);
	                }
	                $value = $orderbyFieldname[1];
	            }
	        }
	    } else if($key == 'sortorder'){
	        if(!$value && $this->get("cv_orderby"))
	            $value = $this->get("cv_sortorder");
	    }
	    return $value;
	}
}
