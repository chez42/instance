...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	public static function getSearchResult($searchKey, $module=false) {
		...
		$query = 'SELECT label, crmid, setype, createdtime FROM vtiger_crmentity WHERE `label`
		LIKE ? AND vtiger_crmentity.deleted = 0';
		...
		/*if($module == 'Contacts') {
            $query = 'SELECT CONCAT(firstname, " ", lastname) as label, e.crmid, e.setype, e.createdtime 
			FROM vtiger_contactdetails cd JOIN vtiger_crmentity e ON e.crmid = cd.contactid 
			WHERE CONCAT(firstname, lastname) LIKE ? AND e.deleted = 0
			ORDER BY lastname';
            $params = array("%$searchKey%");
        }*/
	...
	function getRelatedPortfolioIds($module, $id) {
	    $relatedIds = array();
