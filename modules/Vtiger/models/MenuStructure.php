...
class Vtiger_MenuStructure_Model extends Vtiger_Base_Model {
	...
	public static function getAppMenuList(){
	    ...
	    $menu = $db->pquery("SELECT menuname FROM vtiger_menu ORDER BY sequence ASC");
	    $menus = array();
	    if($db->num_rows($menu)){
	        for($i=0;$i<$db->num_rows($menu);$i++){
	            $menus[] = $db->query_result($menu, $i, 'menuname');
	        }
	    }
	    return $menus;
	}
	...
	public static function getAppIcons() {
	    $db = PearDatabase::getInstance();
	    $menu = $db->pquery("SELECT menuname,icon FROM vtiger_menu ORDER BY sequence ASC");
	    $menuIcon = array();
	    if($db->num_rows($menu)){
	        for($i=0;$i<$db->num_rows($menu);$i++){
	            $menuIcon[$db->query_result($menu, $i, 'menuname')] = $db->query_result($menu, $i, 'icon');
	        }
	    }
		return $menuIcon;
	}
	
	public static function getAppSequence() {
	    $db = PearDatabase::getInstance();
	    $menu = $db->pquery("SELECT menuname,sequence FROM vtiger_menu ORDER BY sequence ASC");
	    $menuSeq = array();
	    if($db->num_rows($menu)){
	        for($i=0;$i<$db->num_rows($menu);$i++){
	            $menuSeq[$db->query_result($menu, $i, 'menuname')] = $db->query_result($menu, $i, 'sequence');
	        }
	    }
	    return $menuSeq;
	}
	...
	public static function getAppColor() {
	    $db = PearDatabase::getInstance();
	    $menu = $db->pquery("SELECT menuname,color FROM vtiger_menu ORDER BY sequence ASC");
	    $menuSeq = array();
	    if($db->num_rows($menu)){
	        for($i=0;$i<$db->num_rows($menu);$i++){
	            $menuSeq[$db->query_result($menu, $i, 'menuname')] = $db->query_result($menu, $i, 'color');
	        }
	    }
	    return $menuSeq;
	}
