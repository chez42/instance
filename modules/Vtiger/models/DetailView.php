...
class Vtiger_DetailView_Model extends Vtiger_Base_Model {
    ...
    public function getDetailViewRelatedLinks() {
        ...
        if($moduleName != 'Documents'){
            ...
            $moduleQuery = $adb->pquery("SELECT DISTINCT vtiger_tab.name, vtiger_tab.tabid FROM vtiger_relatedlists
                INNER JOIN vtiger_tab ON vtiger_tab.tabid = vtiger_relatedlists.tabid
                INNER JOIN vtiger_tab relatedtab ON relatedtab.tabid = vtiger_relatedlists.related_tabid
                WHERE relatedtab.name IN ('Task', 'ModComments', 'Emails', 'Calendar' )");
            
            if($adb->num_rows($moduleQuery)){
                for($m=0;$m<$adb->num_rows($moduleQuery);$m++){
                    $modules[] = $adb->query_result($moduleQuery,$m,'name');
                }
            }
            ...
            if(in_array($moduleName,$modules)){
                ...
                $relatedLinks[] = array(
                    'linktype' => 'DETAILVIEWTAB',
                    'linklabel' => 'LBL_JOURNAL',
                    'linkurl' => 'index.php?module='.$moduleName.'&view=Journal&record='.$recordModel->getId().'&mode=recentJournals&page=1',
                    'linkicon' => ''
                );
