...
class Vtiger_Tag_Model extends Vtiger_Base_Model {
	...
	public static function getAllUserTags($userId) {
		...
		$result = $db->pquery("SELECT * FROM vtiger_freetags WHERE owner=? ", array($userId));
...
//        }
	...
	public static function getTaggedRecords($tagId) {
		...
		if(!empty($tagId)) {
			...
			for($i=0; $i<$rows; $i++) {
				$row = $db->query_result_rowdata($result, $i);
				$recordPermission = Users_Privileges_Model::isPermitted(getSalesEntityType($row['crmid']), 'DetailView', $row['crmid']);
				if($recordPermission) {
