...
/****************************** 09-Aug-2018 Changes *************************************/
		...
		$relatedListViewFieldsList = $relatedModuleModel->getRelatedListViewFieldsList();
		if(count($relatedListViewFieldsList) > 0) {
		    foreach($relatedListViewFieldsList as $fieldName => $fieldModel) {
		        $headerFields[$fieldName] = $fieldModel;
		    }
		} elseif(count($summaryFieldsList) > 0) {
...
/****************************** 09-Aug-2018 Changes End *************************************/
	...
	/**
	 * Function to get the relatedlist of Mass actions for the module
	...
	public function getRelatedListViewMassActions($linkParams = array()) {
	    ...
	    if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'EditView')) {
	        $massActionLinks[] = array(
	            'linktype' => 'RELATEDLISTVIEWMASSACTION',
	            'linklabel' => 'LBL_EDIT',
	            'linkurl' => "index.php?module=".$moduleModel->get('name')."&view=RelatedMassActionAjax&mode=showMassEditForm",
	            'linkicon' => ''
	        );
	    }
	    if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'Delete')) {
	        $massActionLinks[] = array(
	            ...
	            'linkurl' => "index.php?module=".$moduleModel->get('name')."&action=RelatedMassDelete",
	            'linkicon' => ''
	        );
	    }
	    ...
	    if($moduleModel->isCommentEnabled() && $modCommentsModel->isPermitted('CreateView')) {
	        $massActionLinks[] = array(
	            ...
	            'linkurl' => 'index.php?module='.$moduleModel->get('name').'&view=RelatedMassActionAjax&mode=showAddCommentForm',
	            'linkicon' => ''
	        );
	    }
	    ...
	    if($exportPermission) {
	        $massActionLinks[] = array(
	            ...
	            'linkurl' => $moduleModel->getRelatedExportUrl(),
	            'linkicon' => ''
	        );
	    }
	    
	    foreach($massActionLinks as $massActionLink) {
	        $links['RELATEDLISTVIEWMASSACTION'][] = Vtiger_Link_Model::getInstanceFromValues($massActionLink);
	    }
