...
class Vtiger_Module_Model extends Vtiger_Module {
	...
	public function getBlocks() {
		if(empty($this->blocks)) {
			...
			global $adb;
			$isTab = '';
			$tabView = $adb->pquery("SELECT * FROM vtiger_module_tab_view WHERE vtiger_module_tab_view.module_name=?",
			    array($this->getName()));
			if($adb->num_rows($tabView)){
			    $isTab = $adb->query_result($tabView,0,'is_tab');
			}
			if($isTab){
			    $moduleBlocks = Vtiger_Block_Model::getAllForModuleTabView($this);
			}else{
...
/****************************** 09-Aug-2018 Changes *************************************/
		...
		$relatedListViewFields = $relatedModule->getRelatedListViewFieldsList();
		...
		if(!empty($relationFieldMapping)) {
			...
			//for ($i=0; $i<count($relatedModules); $i++) {
				...
				for($j=0; $j<$num_rows; $j++){
				    $deleted = $db->query_result($result1, $j, 'deleted');
				    if($deleted == 1)
				        continue;
...
/****************************** 09-Aug-2018 Changes End *************************************/
    ...
    /**
     * Function to get the url for the Related List Export action of the module
     * @return <String> - url
     */
