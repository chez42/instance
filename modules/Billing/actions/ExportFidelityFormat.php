...
class Billing_ExportFidelityFormat_Action extends Vtiger_Mass_Action {
    ...
    public function process(Vtiger_Request $request) {
		...
		header('Content-Disposition: attachment; filename=' . $request->get('filename'). '.txt');
        ...
        if($adb->num_rows($result)){
            ...
            for($i = 0; $i < $adb->num_rows($result); $i++){
				fwrite($output, $adb->query_result($result, $i, 'account_number') . '              ' . CurrencyField::convertToUserFormat($adb->query_result($result, $i, 'portfolio_amount')) . PHP_EOL);
			}
        }
