...
class Contacts_Module_Model extends Vtiger_Module_Model {
	...
	public function getRelationQuery($recordId, $functionName, $relatedModule, $relationId) {
		if ($functionName === 'get_activities') {
		...
		}elseif ($functionName === 'get_connection') {
		    
		    $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
		        'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
		    $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
    		vtiger_crmentity.*,vtiger_contactdetails.*,vtiger_contactscf.*,vtiger_contactaddress.*,
            vtiger_contactsubdetails.*,vtiger_connection.*
    		...
    		INNER JOIN vtiger_contactdetails on (vtiger_contactdetails.contactid = vtiger_connection.child_contact_id 
            OR vtiger_contactdetails.contactid = vtiger_connection.parent_contact_id )
            INNER JOIN vtiger_crmentity crm ON crm.crmid = vtiger_contactdetails.contactid AND crm.deleted = 0            
            ...
            LEFT JOIN vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
    		LEFT JOIN vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
    		WHERE (vtiger_connection.parent_contact_id =".$recordId." OR  vtiger_connection.child_contact_id =".$recordId." )
            and vtiger_contactdetails.contactid !=".$recordId." and vtiger_crmentity.deleted=0";
    ...
    public function getRecentTasks($mode, $pagingModel, $user, $recordId = false){
        ...
        if ($recordId) {
            $query .= " AND vtiger_task.contact_id = ?";
            array_push($params, $recordId);
        }
