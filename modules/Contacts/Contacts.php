  ...
  $log->debug("Exiting get_contactsforol method ...");
	...
	*/
    ...
    function get_positions($id, $cur_tab_id, $rel_tab_id, $actions = false){
        ...
        $comma_separated = SeparateArrayToQuoteAndCommas($account_numbers);
		...
		WHERE vtiger_crmentity.deleted = 0 AND vtiger_positioninformation.account_number IN ({$comma_separated})
    ...
    function get_portfolios($id, $cur_tab_id, $rel_tab_id, $actions = false){
        include_once('libraries/reports/new/nCommon.php');
        ...
        $query = "SELECT vtiger_portfolioinformation.*, vtiger_crmentity.*, vtiger_users.*,
                  case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name
                  ...
                  LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
                  LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id
                  WHERE vtiger_crmentity.deleted = 0 AND vtiger_portfolioinformation.account_number IN ({$comma_separated})
                  AND vtiger_portfolioinformation.accountclosed != 1 ";
    ...
    /*16-Oct-2018*/
    function get_tasks($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        global $log, $singlepane_view,$currentModule,$current_user;
        $log->debug("Entering get_tasks(".$id.") method ...");
        $this_module = $currentModule;
        ...
        if($singlepane_view == 'true')
            ...
            $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
                'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
            $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
    		vtiger_crmentity.*, vtiger_task.subject, vtiger_task.date_start, vtiger_task.time_start,
    		vtiger_task.due_date, vtiger_task.parent_id, vtiger_task.task_status, vtiger_task.taskid
    		FROM vtiger_task INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_task.taskid
    		inner join vtiger_taskcf ON vtiger_taskcf.taskid = vtiger_task.taskid
    		inner join vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_task.contact_id
    		left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
    		left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
    		where vtiger_task.contact_id=".$id." and vtiger_crmentity.deleted=0";
    ...
    function get_portfolioinformation($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        global $log, $singlepane_view,$currentModule,$current_user;
        $log->debug("Entering get_portfolioinformation(".$id.") method ...");
        $this_module = $currentModule;
        ...
        $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
            'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
        $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
		vtiger_crmentity.*, vtiger_portfolioinformation.*
		FROM vtiger_portfolioinformation
        INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_portfolioinformation.portfolioinformationid
		INNER JOIN vtiger_portfolioinformationcf ON vtiger_portfolioinformationcf.portfolioinformationid = vtiger_portfolioinformation.portfolioinformationid
        INNER JOIN vtiger_pc_account_custom ON vtiger_pc_account_custom.account_number = vtiger_portfolioinformation.account_number
		LEFT JOIN vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_portfolioinformation.contact_link		
        LEFT JOIN vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
		LEFT JOIN vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
		WHERE vtiger_portfolioinformation.account_number IN (".$questions.") and vtiger_crmentity.deleted=0
        ...
        $log->debug("Exiting get_portfolioinformation method ...");
        return $return_value;
    }
    
    function get_transactions($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        global $log, $singlepane_view,$currentModule,$current_user;
        $log->debug("Entering get_transactions(".$id.") method ...");
        $this_module = $currentModule;
        ...
        $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
            'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
        $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
		vtiger_crmentity.*, vtiger_transactions.*
        ...
        $log->debug("Exiting get_transactions method ...");
        return $return_value;
    }
    
    
    /* Function to get attachments in the related list of contacts module */
    function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        ...
        $ticket_ids = $this->getRelatedTicketIds($id);
        $portfolio_ids = $this->getRelatedPortfolioIds($id);
        $entityIds = array_merge( $ticket_ids, $portfolio_ids);
    ...
    function getRelatedPortfolioIds($id) {
        $relatedIds = array();
        require_once("libraries/reports/cReports.php");
        include_once('libraries/reports/new/nCommon.php');
        ...
        $db = PearDatabase::getInstance();
        $query = "SELECT DISTINCT vtiger_crmentity.crmid FROM vtiger_portfolioinformation
        INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_portfolioinformation.portfolioinformationid
		INNER JOIN vtiger_portfolioinformationcf ON vtiger_portfolioinformationcf.portfolioinformationid = vtiger_portfolioinformation.portfolioinformationid
        INNER JOIN vtiger_pc_account_custom ON vtiger_pc_account_custom.account_number = vtiger_portfolioinformation.account_number
		LEFT JOIN vtiger_contactdetails on vtiger_contactdetails.contactid = vtiger_portfolioinformation.contact_link		
    ...
    public static function getPortalResetPasswordEmailContents($entityData, $password){
        ...
        //here templateid is hardcoded with 25. it is for portal reset password EmailTemplate Content
        $query = 'SELECT vtiger_emailtemplates.subject,vtiger_emailtemplates.body FROM vtiger_emailtemplates WHERE templateid = 20';
        ...
        $contents = getMergedDescription($contents, $entityData->getId(), $moduleName);
        
        $subject = getMergedDescription($subject,$entityData->getId(), $moduleName);
    ...
    function getUpdatePortalDetails(){
        ...
        if ($isPortalEnabled) {
            ...
            $params = array($password, $entityId);
            $adb->pquery($sql, $params);
            $sendEmail = true;
    ...
    function get_connection($id, $cur_tab_id, $rel_tab_id, $actions=false) {
