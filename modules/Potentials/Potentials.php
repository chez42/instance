	...
	function get_tasks($id, $cur_tab_id, $rel_tab_id, $actions=false) {
	    ...
	    if($singlepane_view == 'true')
            ...
            $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
                'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
            $query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
    		vtiger_crmentity.*, vtiger_leaddetails.*, vtiger_task.subject, vtiger_task.date_start, vtiger_task.time_start,
    		...
    		inner join vtiger_potential on vtiger_potential.potentialid = vtiger_task.parent_id
    		left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
    		left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
