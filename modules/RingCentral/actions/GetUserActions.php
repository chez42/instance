...
class RingCentral_GetUserActions_Action extends Vtiger_Mass_Action {
    ...
    function __construct() {
        parent::__construct();
        $this->exposeMethod('getdetail');
        $this->exposeMethod('getDisconnect');
    }
    ...
    public function getdetail(Vtiger_Request $request){
    ...
    public function getDisconnect(Vtiger_Request $request){
        ...
        if(!$platform->auth()->tokenType()){
            $adb->pquery("DELETE FROM vtiger_ringcentral_oauth WHERE userid = ?",array($userId));
            $result = array('success'=> true);
        }
