	...
	function get_tickets($id, $cur_tab_id, $rel_tab_id, $actions=false) {
	    ...
	    if($singlepane_view == 'true')
	        $returnset = '&return_module='.$this_module.'&return_action=DetailView&return_id='.$id;
				...
				INNER JOIN vtiger_project ON vtiger_project.projectid = vtiger_troubletickets.project_id
				LEFT JOIN vtiger_ticketcf ON vtiger_troubletickets.ticketid = vtiger_ticketcf.ticketid
				LEFT JOIN vtiger_users ON vtiger_users.id=vtiger_crmentity.smownerid
				LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
				WHERE  vtiger_crmentity.deleted = 0 and vtiger_troubletickets.project_id = $id";
