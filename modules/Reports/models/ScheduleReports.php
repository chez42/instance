...
class Reports_ScheduleReports_Model extends Vtiger_Base_Model {
	...
	public function getRecipientEmails($userId) {
...
// 		}
	...
	public function sendEmail($userId) {
		...
		$recipientEmails = $this->getRecipientEmails($userId);
		...
		if($reportType != 'chart') {
			if ($reportFormat == 'CSV') {
				...
				$oReportRun->writeScheduleReportToCSVFile($filePath);
			} else if($reportFormat == 'XLS') {
				$fileName = $baseFileName . '.xls';
				$filePath = 'storage/' . $fileName;
				$attachments[$fileName] = $filePath;
				$oReportRun->writeScheduleReportToExcelFile($filePath);
			}
...
// 		$util->adminUser();
		...
		foreach ($scheduledReports as $reportId => $scheduledReport) {
		    foreach($scheduledReport->getRecipientIds() as $userId){
    		    $user = new Users();
    			...
    			$reportType = $reportRecordModel->get('reporttype');
    			if($reportType == 'chart') {
    			    $status = $scheduledReport->sendEmail($userId);
    			} else {
    			    $query = $reportRecordModel->getScheduleReportSQL();
