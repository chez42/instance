...
class Reports_Folder_Model extends Vtiger_Base_Model {
	...
	public function getReportsCount() {
		...
		/*************************************  02 Aug 2018 change  for reports for shared  ****************************/
		...
		if (!$currentUserModel->isAdminUser()) {
			...
			$user_group_query = " (shareid IN (".$groupId.") AND setype='groups') OR";
			$non_admin_query = " vtiger_report.reportid IN (SELECT reportid FROM vtiger_reportsharing WHERE $user_group_query (shareid=".$currentUserId." AND setype='users'))";
			...
			$sql .= " AND ( ($non_admin_query  "
			. "vtiger_report.owner = ".$currentUserId." OR vtiger_report.owner IN (SELECT vtiger_user2role.userid "
			    . "FROM vtiger_user2role INNER JOIN vtiger_users ON vtiger_users.id=vtiger_user2role.userid "
			        . "INNER JOIN vtiger_role ON vtiger_role.roleid=vtiger_user2role.roleid "
			            . "WHERE vtiger_role.parentrole LIKE '".$parentRoleSeq."'))";
			            
			
			$sql .= " OR (  $currentUserId IN (
								SELECT vtiger_user2role.userid FROM vtiger_user2role
									INNER JOIN vtiger_users ON vtiger_users.id = vtiger_user2role.userid
									INNER JOIN vtiger_role ON vtiger_role.roleid = vtiger_user2role.roleid
								WHERE vtiger_role.parentrole LIKE '".$parentRoleSeq."')
                            OR vtiger_report.reportid IN (SELECT vtiger_report_shareusers.reportid FROM vtiger_report_shareusers WHERE vtiger_report_shareusers.userid=".$currentUserId.")";
			
			if(!empty($groupId)){
			    $sql .= " OR vtiger_report.reportid IN (SELECT vtiger_report_sharegroups.reportid FROM vtiger_report_sharegroups WHERE vtiger_report_sharegroups.groupid IN (".$groupId."))";
			}
			
			$sql.= " OR vtiger_report.reportid IN (SELECT vtiger_report_sharerole.reportid FROM vtiger_report_sharerole WHERE vtiger_report_sharerole.roleid ='".fetchUserRole($currentUserId)."')";
			
			
			$parentRoles=getParentRole(fetchUserRole($currentUserId));
			$parentRolelist= array();
			foreach($parentRoles as $par_rol_id)
			{
			    array_push($parentRolelist, $par_rol_id);
			}
			array_push($parentRolelist, fetchUserRole($currentUserId));
		...
		/*************************************  02 Aug 2018 change  for reports for shared End  ****************************/
