...
/************************* 27 Jul 2018 change for shared id field  ******************/
    
    public static function getAllForSharedid($onlyActive=true) {
        $members = array();
        $user_Model = Users_Record_Model::getCurrentUserModel();
        
        $allUsers = $user_Model->getAccessibleUsers();
        
        foreach($allUsers as $userId => $userModel) {
            $qualifiedId = 'Users:'.$userId;
            $member = new Settings_Groups_Member_Model();
            $members['Users'][$qualifiedId] = $member->set('id', $qualifiedId)->set('name', $userModel);
        }
        
        $allGroups =  $user_Model->getAccessibleGroups();
        foreach($allGroups as $groupId => $groupModel) {
            $qualifiedId = 'Groups:'.$groupId;
            $member = new Settings_Groups_Member_Model();
            $members['Groups'][$qualifiedId] = $member->set('id', $qualifiedId)->set('name', $groupModel);
        }
       
        $allRoles = Settings_Roles_Record_Model::getAll();
        foreach($allRoles as $roleId => $roleModel) {
            $qualifiedId = 'Roles:'.$roleId;
            $member = new Settings_Groups_Member_Model();
            $members['Roles'][$qualifiedId] = $member->set('id', $qualifiedId)->set('name', $roleModel->getName());
            
            $qualifiedId = 'RoleAndSubordinates:'.$roleId;
            $member = new Settings_Groups_Member_Model();
            $members['RoleAndSubordinates'][$qualifiedId] = $member->set('id', $qualifiedId)->set('name', $roleModel->getName());
        }
        
        return $members;
    }
    
/************************* 27 Jul 2018 change for shared id field End  ******************/
