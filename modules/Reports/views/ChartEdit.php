...
Class Reports_ChartEdit_View extends Vtiger_Edit_View {
	...
	function step1(Vtiger_Request $request) {
		...
		$viewer->assign('MEMBER_GROUPS',  $reportModuleModel::getAllForSharedid());
	...
	public function getPageTitle(Vtiger_Request $request) {
	    $name = vtranslate($request->getModule(), $request->getModule());
	    if($request->get('record')){
	        global $adb;
	        $nameQuery = $adb->pquery("SELECT vtiger_report.reportname FROM vtiger_report WHERE vtiger_report.reportid = ?",
	            array($request->get('record')));
	        if($adb->num_rows($nameQuery))
	            $name = vtranslate($request->getModule(), $request->getModule()).' - '.$adb->query_result($nameQuery, 0,'reportname');
	    }
	    return $name;
	}
}