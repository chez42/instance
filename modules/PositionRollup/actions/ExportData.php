...
class PositionRollup_ExportData_Action extends Vtiger_Mass_Action {
    ...
    //        $query = str_replace("vtiger_positioninformation.quantity", "SUM(vtiger_positioninformation.quantity) AS quantity", $query);
        ...
        /**
         * Replaces all rollup with positioninformation so the query doesn't plop
         * @param type $query
         * @return type
         */
        function InjectGrouping($query){
            $query .= " GROUP BY security_symbol";
            $query = preg_replace("/positionrollupid/", "positioninformationid",$query);
            $query = preg_replace("/positionrollup/", "positioninformation",$query);
            return $query;
        }
        
        function IDListToSymbol($idlist){
            global $adb;
            $symbols = array();
            $query = "SELECT security_symbol FROM vtiger_positioninformation WHERE positioninformationid = ?";
            foreach($idlist AS $k => $v){
                $result = $adb->pquery($query, array($v));
                if($adb->num_rows($result) > 0){
                    $symbols[] = $adb->query_result($result, 0, 'security_symbol');
                }
            }

            return SeparateArrayWithCommasAndSingleQuotes($symbols);
        }
	...
	function getExportQuery(Vtiger_Request $request) {
		...
		$query = $queryGenerator->getQuery();
                $query = $this->InjectQuery($query);

		$this->accessibleFields = $queryGenerator->getFields();

		switch($mode) {
			case 'ExportAllData' :	return $this->InjectGrouping($query);
									break;
			...
			case 'ExportCurrentPage' :	$pagingModel = new Vtiger_Paging_Model();
										...
										return $this->InjectGrouping($query);
										break;
...
//													$idList = implode(',' , $idList);
													$query .= ' AND '.$baseTable.'.'.'security_symbol'.' IN ('.$this->IDListToSymbol($idList).')';
												}
											} else {
			...
			default :	return $this->InjectGrouping($query);
						break;
		}
	}
