...
class VTEEmailMarketing_ActionAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function saveRelatedRecord(Vtiger_Request $request)
    {
        ...
        for ($i = 0; $i < count($arrayId); $i++) {
            $checkRecord = $adb->pquery("SELECT 1 FROM vtiger_vteemailmarketingrel WHERE vteemailmarketingid=? AND crmid = ?", array($recordId, $arrayId[$i]));
            $numrows = $adb->num_rows($checkRecord);
            if ($numrows == 0) {
                $params = array($recordId, $arrayId[$i], $relModule);
