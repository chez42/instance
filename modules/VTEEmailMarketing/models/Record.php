...
class VTEEmailMarketing_Record_Model extends Vtiger_Record_Model
{
    ...
    public function getVTEEmailMarketing($recordId)
    {
        ...
        $data["countPotentials"] = self::getCountRelated($recordId, "Potentials");
        return $data;
    }
    ...
    public function getRelationId()
    {
        ...
        $modulePotentials = Vtiger_Record_Model::getCleanInstance("Potentials")->getModule();
        ...
        $relationIdPotentials = self::getRelationIdModule($moduleVTEEmailMarketing->getId(), $modulePotentials->getId());
        $data = array("relationIdAccounts" => $relationIdAccounts, "relationIdLeads" => $relationIdLeads, "relationIdContacts" => $relationIdContacts, "relationIdPotentials" => $relationIdPotentials);
        return $data;
    }
    ...
    {
        global $adb;
        $query = "SELECT `cvid`,`viewname`,`entitytype` ,`first_name`,`last_name`\n              FROM vtiger_customview JOIN vtiger_users ON vtiger_customview.userid = vtiger_users.id\n              WHERE entitytype = ? OR entitytype = ? OR entitytype= ? ORDER BY( CASE entitytype WHEN 'Contacts' THEN 1 WHEN 'Leads' THEN 2 WHEN 'Accounts' THEN 3 WHEN 'Potentials' THEN 4 ELSE 100 END), viewname ASC";
        $params = array("Leads", "Accounts", "Contacts", "Potentials");
        $result = $adb->pquery($query, $params);
        ...
        for ($i = 0; $i < $numrow; $i++) {
            ...
            if ($relatedModule == "Contacts") {
                ...
                if ($relatedModule == "Leads") {
                    ...
                    $getRecordPotentials = $adb->pquery("SELECT * FROM vtiger_potentialscf WHERE potentialid = ?", array($relatedId));
                    $data[$i]["record_type"] = $relatedModule;
                    $data[$i]["name"] = $label;
                    $data[$i]["email"] = $adb->query_result($getRecordPotentials, 0, "cf_869");
                    $data[$i]["record_id"] = $relatedId;
                    $data[$i]["data_url"] = "index.php?module=" . $relatedModule . "&view=Detail&record=" . $relatedId;
                } else {
                ...
                if ($dispayType == "failed_to_send") {
                    ...
                    if ($dispayType == "unique_open") {
                        ...
                        if ($dispayType == "unopened") {
                            ...
                            if ($dispayType == "unsubcribes") {
                                $query = "SELECT DISTINCT c.label , rel.* \n                    FROM vtiger_vteemailmarketing_unsubcribes un\n                    INNER JOIN vtiger_vteemailmarketing mk ON un.vteemailmarketingid = mk.vteemailmarketingid\n                    INNER JOIN vtiger_crmentity c ON mk.vteemailmarketingid = c.crmid\n                    INNER JOIN vtiger_vteemailmarketingrel rel ON mk.vteemailmarketingid = rel.vteemailmarketingid AND rel.crmid = un.crmid\n                    WHERE c.deleted = 0 AND c.crmid = ?  AND un.status = 1\n                    LIMIT " . $startIndex . "," . $pageLimit;
                            } else {
