...
class VTEEmailMarketing extends Vtiger_CRMEntity
{
    ...
    public function init($moduleName)
    {
        ...
        $modulePotentials->setRelatedList($moduleInstance, "VTEEmailMarketing", "", "get_related_list");
        $moduleRelatedId = array($moduleContacts->id, $moduleLeads->id, $moduleAccounts->id, $modulePotentials->id);
...
//         $adb->pquery("DELETE FROM `vte_modules` WHERE module=?;", array("VTEEmailMarketing"));
    ...
    public function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array())
    {
        ...
        foreach ($with_crmids as $with_crmid) {
            if ($with_module == "Leads" || $with_module == "Contacts" || $with_module == "Accounts" || $with_module == "Potentials") {
