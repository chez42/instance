...
class Office365_SaveSyncSettings_Action extends Vtiger_BasicAjax_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		$googleModuleModel = Vtiger_Module_Model::getInstance('Office365');

		$returnUrl = $googleModuleModel->getBaseExtensionUrl($sourceModule);
		
		if($request->has('parent') && $request->get('parent') === 'Settings') {
			$returnUrl = 'index.php?module=' . $sourceModule . '&parent=Settings&view=Extension&extensionModule=Office365&extensionView=Index&mode=settings';
		}
