...
Class Office365_Vtiger_Connector extends Office365_Base_Connector{
	...
	public function pull($sourceModule) {
		...
		$lastSync = Office365_Utils_Helper::getLastVtigerSyncTime($sourceModule);
		
		if ($sourceModule == 'Calendar' && !$lastSync) {
            $lastSync = Office365_Utils_Helper::getCalendarSyncStartDate();
		...
		foreach ($createdRecords as $record) {
			...
			$recordModels[] = $model->setMode(Office365_SyncRecord_Model::CREATE_MODE);
		}
		...
		foreach ($updatedRecords as $record) {
			...
			$model = $this->getRecordModelFromData($record);
			$recordModels[] = $model->setMode(Office365_SyncRecord_Model::UPDATE_MODE);
		}

		foreach ($deletedRecords as $record) {
			$model = $this->getRecordModelFromData($record);
			$recordModels[] = $model->setMode(Office365_SyncRecord_Model::DELETE_MODE);
		}
		...
		*/
        ...
        foreach ($result['updated'] as $u) {
            ...
            if(in_array($u['id'],$updatedIds)){
            	...
            	if ( 
					...
					$u['_id'] = $updatedLookupIds[$u['id']]['officeid']; // Add officeid
                    $u['_modifiedtime'] = $updatedLookupIds[$u['id']]['office365modifiedtime'];
		...
		foreach ($deletedIds as $d) {
          	if (isset($deletedLookupIds[$d])) {
				$filteredDeletes[] = array(
					'_id' => $deletedLookupIds[$d]['officeid'], /* Add officeid;*/
	...
	function vtigerpull($lastSync,$elementType,$syncType,$user){
		...
		foreach($accessableModules as $elementType){
			...
			$q = $selectClause." ".$fromClause ;//." AND ".$deletedQueryCondition;
	...
	public function push($recordList) {
		...
		foreach ($pushResult as $mode => $records) {
			if ($mode == 'created') {
				$recordMode = Office365_SyncRecord_Model::CREATE_MODE;
			} else if ($mode == 'updated') {
				$recordMode = Office365_SyncRecord_Model::UPDATE_MODE;
			} else if ($mode == 'deleted') {
    		    $recordMode = Office365_SyncRecord_Model::DELETE_MODE;
    		} else {
    		    $recordMode = 'skipped';
    		}
	...
	public function put($records, $user) {
        ...
        foreach ($records as $record) {
			...
			$lookupResult = $db->pquery("SELECT * FROM vtiger_office365_recordmapping 
			WHERE BINARY officeid = ?", array($clientRecordId));
			        ...
			        if($record['module'] == 'Events'){
			            ...
			            if ($db->num_rows($lookupResult)){
			                ...
			                while($childRow = $db->fetchByAssoc($lookupResult)){
			                    $series[$childRow['serverid']] = $childRow['officeid'];
			                }
    			...
    			if ( empty($lookupRecordId) && !empty($record['values']) ) {
                	...
                	if( !(empty($lookupRecordId)) ){
    	            	...
    	            	if(!empty($record['values'])) {	
    	            		...
    	            		$clientLastModifiedTime = $db->query_result($lookupResult,0,'office365modifiedtime');
        ...
        foreach($vtigerSaveResult['created'] as $clientRecordId => $record){
            ...
            $office365Data = $createRecords[$clientRecordId];
            
            if($office365Data['module'] == "Events")
                $parent_id = $office365Data['parent_id'];
            else 
                $parent_id = false;
       	...
       	foreach($vtigerSaveResult['updated'] as $clientRecordId => $record){
       	    ...
       	    $office365Data = $updateRecords[$clientRecordId];
	...
	public function fillMandatoryEmptyFields($moduleName, $recordLists, $user) {
		...
		foreach ($recordLists as $record) {
			foreach ($mandatoryFields as $fieldName) {
				...
				*/
				if($record->getMode() == Office365_SyncRecord_Model::UPDATE_MODE) {
					continue;
				}
	...
	public function postEvent($type, $synchronizedRecords) {
		if ($type == 'pull') {
			...
			if( $this->nextSyncTime != '' )
				Office365_Utils_Helper::updateLastVtigerSyncTime($this->sourceModule, $this->nextSyncTime);
		}
	}
	...
	public function map($synchronizedRecords) {
		...
		/* now save mapping in office365_recordmapping table */
