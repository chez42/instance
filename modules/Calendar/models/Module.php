...
class Calendar_Module_Model extends Vtiger_Module_Model {
	...
	public function getCreateEventRecordUrl() {
		return 'index.php?module=Calendar&view='.$this->getEditViewName().'&mode=Events';
	}
	...
	public function getCreateTaskRecordUrl() {
	    ...
	    $taskModuleModel = Vtiger_Module_Model::getInstance('Task');
	    return $taskModuleModel->getCreateRecordUrl().'&parent_id='.$this->getId();
	}
	...
	*/
	public static function getCalendarViewTypes($id){
		...
		$query = "SELECT * FROM vtiger_calendar_user_activitytypes 
			...
			WHERE vtiger_calendar_user_activitytypes.userid=? and vtiger_calendar_default_activitytypes.fieldname <> 'Tasks'";
		$result = $db->pquery($query, array($id));
		$rows = $db->num_rows($result);
	...
	public function checkDuplicateView(Vtiger_Request $request) {
		...
		$query = 'SELECT id FROM vtiger_calendar_default_activitytypes WHERE (module=? AND fieldname=? AND conditions=?) ';
		if($typeId)
		    $query .= ' AND id!='.$typeId.' ';
		
		$queryResult = $db->pquery($query, array($viewmodule, $viewfieldname,$conditions));
	...
	public function addCalendarView(Vtiger_Request $request) {
		...
		$db = PearDatabase::getInstance();
		$queryResult = $db->pquery('SELECT id,isdefault FROM vtiger_calendar_default_activitytypes WHERE id=?', array($typeId));
		$type = '';
		if($db->num_rows($queryResult) > 0) {
			$defaultId = $db->query_result($queryResult, 0, 'id');
			...
			if($db->num_rows($query) > 0) {
				$db->pquery('UPDATE vtiger_calendar_user_activitytypes 
							INNER JOIN vtiger_calendar_default_activitytypes ON vtiger_calendar_default_activitytypes.id = vtiger_calendar_user_activitytypes.defaultid
							SET vtiger_calendar_user_activitytypes.color=?, vtiger_calendar_user_activitytypes.visible=? ,vtiger_calendar_default_activitytypes.conditions=?
							WHERE vtiger_calendar_user_activitytypes.userid=? AND vtiger_calendar_default_activitytypes.id=?  ',
				    array($viewcolor, '1', $viewconditions, $userId, $typeId));
			} else {
	...
	public function getRecentRecords($limit=10) {
		...
		$query .= ' WHERE setype=? AND  vtiger_activity.activitytype != "Task" AND '.$deletedCondition.' AND modifiedby = ? ORDER BY modifiedtime DESC LIMIT ?';
	...
	public static function getCalendarReminder() {
	    ...
	    if($activityReminder != '' ) {
	        ...
	        //$date_time = date('Y-m-d H:i:s', strtotime("+$activityReminder seconds", $currentTime));
	        ...
	        $date_time = date('Y-m-d H:i:s', strtotime("+$activityReminder seconds", strtotime($now->format('Y-m-d H:s:i'))));
	        ...
	        $reminderActivitiesResult = "SELECT  reminderid, recordid FROM vtiger_activity_reminder_popup
			        ...
			        AND vtiger_activity.eventstatus <> 'Held' AND (vtiger_activity.status <> 'Completed'
			            OR vtiger_activity.status IS NULL) and
			        DATE_FORMAT(
			            CONVERT_TZ(
			                CONCAT(vtiger_activity_reminder_popup.date_start, ' ', vtiger_activity_reminder_popup.time_start)
			                , '+0:00', ?) ,'%Y-%m-%d %H:%i:%s') < ? LIMIT 20";
	        ...
	        /*$reminderActivitiesResult = "SELECT reminderid, recordid FROM vtiger_activity_reminder_popup
	         ...
	         AND vtiger_activity.eventstatus <> 'Held' AND (vtiger_activity.status <> 'Completed'
	         OR vtiger_activity.status IS NULL) LIMIT 20";*/
	        
	        //$result = $db->pquery($reminderActivitiesResult, array($currentUserModel->getId(), $date, $time));
	        
	        $result = $db->pquery($reminderActivitiesResult, array($currentUserModel->getId(), $format, $date_time));
	...
	public function getSettingLinks() {
		...
		if($currentUserModel->isAdminUser()) {
			$settingLinks[] = array(
					...
					'linkurl' => 'index.php?parent=Settings&module=LayoutEditor&sourceModule=Events',
			...
			$settingLinks[] = array(
					...
					'linkurl' => 'index.php?parent=Settings&module=Picklist&view=Index&sourceModule=Events',
					'linkicon' => ''
			);
		}
		return $settingLinks;
