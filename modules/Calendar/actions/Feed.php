	...
	protected function pullDetails($start, $end, &$result, $type, $fieldName, $color = null, $textColor = 'white', $conditions = '') {
		...
		foreach ($records as $record) {
			...
			if($type == 'Task'){
			    ...
			    $taskRecords = $adb->pquery("SELECT * FROM vtiger_task 
                ...
                WHERE vtiger_crmentity.deleted = 0 AND vtiger_task.taskid = ?",array($crmid));
			    
			    if($adb->num_rows($taskRecords)){
			        $contact = $adb->query_result($taskRecords, 0, 'contact_id');
			        $parent = $adb->query_result($taskRecords, 0, 'parent_id');
			        $status = $adb->query_result($taskRecords, 0, 'task_status');
			        $openDays = $adb->query_result($taskRecords, 0, 'cf_3415');
			        $contactLink = '';
			        $parentLink = '';
			        if($contact)
			            $contactLink = '<a target="_blank" style="color:#0000cd !important;" href="index.php?module=Contacts&view=Detail&record='.$contact.'">'.getContactName($contact).'</a>, ';
			        if($parent)
			            $parentLink = '<a target="_blank" style="color: #0000cd !important;" href="index.php?module='.getSalesEntityType($parent).'&view=Detail&record='.$parent.'">'.Vtiger_Functions::getCRMRecordLabel($parent).'</a>';
	...
	protected function pullEvents($start, $end, &$result, $userid = false, $color = null, $textColor = 'white', $isGroupId = false, $conditions = '') {
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			if(!$currentUser->isAdminUser() && $visibility == 'Private' && $userid && $userid != $currentUser->getId() && $recordBusy) {
			    ...
			    $relQuery = $db->pquery("SELECT vtiger_crmentity.label, vtiger_crmentity.crmid FROM vtiger_seactivityrel
                INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_seactivityrel.crmid
                WHERE vtiger_seactivityrel.activityid = ?",array($crmid));
			    $relLabel = '';
			    $relLink = '';
			    if($db->num_rows($relQuery)){
			        $relLabel = $db->query_result($relQuery, 0, 'label').', ';
			        $relId = $db->query_result($relQuery, 0, 'crmid');
			        $relLink = '<a target="_blank" style="color:#0000cd !important;" href="index.php?module='.getSalesEntityType($relId).'&view=Detail&record='.$relId.'">'.$db->query_result($relQuery, 0, 'label').'</a>, ';
			    }
                ...
                WHERE vtiger_cntactivityrel.activityid = ?",array($crmid));
			    $cntLabels = '';
			    $contactLinks = ''; 
			    if($db->num_rows($cntQuery)){
			        for($con=0;$con<$db->num_rows($cntQuery);$con++){
			            if($con == 1){
			                $cntLabels.=', ';
			                $contactLinks.=', ';
			            }
		                ...
		                $contactLinks .= '<a target="_blank" style="color:#0000cd !important;" href="index.php?module=Contacts&view=Detail&record='.$cntId.'">'.$db->query_result($cntQuery, $con, 'contactname').'</a>';
			        }
			    }
				...
				$item['relatedToLinks'] = decode_html($record['subject']).' - ('.decode_html(vtranslate($record['eventstatus'],'Calendar')).') '.$relLink.$contactLinks;
			}
