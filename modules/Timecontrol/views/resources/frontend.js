...
jQuery(function() {
    ...
    Timecontrol.helpdeskModuelFunction();
    ...
    if(window.location.href.indexOf('tcaccountingids') != -1) {
        ...
        if(accountingids.length > 0) {
            ...
            jQuery.post('index.php', {module:'Timecontrol', action:'GetAccountingData', records:accountingids}, function(response) {
                ...
                if(response.preriodOfPerformance && jQuery('[name="period_of_performance"]').length > 0){
                	jQuery('[name="period_of_performance"]').val(response.preriodOfPerformance);
                }
                if(response.invoicetitle && jQuery('[name="subject"]').length > 0){
                	jQuery('[name="subject"]').val(response.invoicetitle);
                }	
    ...
    if(window.location.href.indexOf('recallid') != -1) {
        ...
        if(accountingids.length > 0) {
            ...
            jQuery.post('index.php', {module:'Timecontrol', action:'GetAccountingData', records:accountingids, invoiceid:invoceId}, function(response) {
                var data = response.data;
                console.log(data);
                var rowIndex = parseInt(data['rowcount']) + 1;
        ...
        jQuery.get('index.php', {
            ...
            jQuery.each(response.timer, function(index, value) {
                html += '<div style="border-bottom:1px solid #ccc;padding: 5px;margin:0;">';
                    ...
                    html += '<span style="text-align:left;font-size:16px;font-weight: bold;" class="startCounter" data-staredts="' + value.timestamp + '" id="counter_' + index + '" data-index="' + index + '" >&nbsp;</span>';   
                    html += '<span class="pull-right"><a style="padding:0;"href="' + value.relatedurl + '"><strong>' + value.relatedname + '</strong></a> <a id="finish" value="'+ value.timecontrolid +'" class="fa fa-check finish" style="padding:0;">&nbsp;</a></span>';
                    html += '</div>';

               // html += '<div style="float:right;margin-bottom:1%;"><a id="finish" value="'+ value.timecontrolid +'" class="fa fa-check finish" style="padding:0;">&nbsp;</a></div>';
            ...
            htmlBtn += '<style> #timeControlBtns > div a {padding: 0px !important;} #timeControlBtns .select2-container {width :50%!important;}</style>';
            htmlBtn += '<div class="referencefield-wrapper pull-left">';
        	htmlBtn += '<div class="input-group">';
        	htmlBtn += '<input type="hidden" name="ticket_id" value="" class="sourceField"/>';
        	htmlBtn += '<input id="ticket_id_display" name="ticket_id_display" data-fieldname="ticket_id" data-fieldtype="reference" type="text" ';
        	htmlBtn += ' class="marginLeftZero autoComplete inputElement" ';
        	htmlBtn += ' value="" placeholder="Type to search" data-module="HelpDesk" />';
            htmlBtn +='</div></div>';
            
            htmlBtn += '<input type="button" style="margin-left:10px;" class="pull-right btn btn-primary" value="' + response.label['create timer']+ '" id="btnQuickTimer" />';

            jQuery('#timeControlBtns').html(htmlBtn);
            jQuery('#timeControlPopUpContent').html(html);
            
            vtUtils.applyFieldElementsView(jQuery('#timeControlBtns'));
            ...
            jQuery('input.autoComplete').autocomplete({
    			...
    			'source' : function(request, response){
    				...
    				var params = {};
    				params.search_module = 'HelpDesk' 
    				params.module = 'Timecontrol';
    				params.search_value = searchValue;
    				params.action = "TimePopup";
    				params.mode = "getAllTickets";
    				
    				if(params.search_module && params.search_module!= 'undefined') {
    					var indexInstance = Vtiger_Index_Js.getInstance();
    					indexInstance.searchModuleNames(params).then(function(data){
    			...
    			'select' : function(event, ui ){
    				...
    				element.attr("value",selectedItemData.selectedName);
            ...
            jQuery('#btnQuickTimer').on('click', function(e) {
            	if(jQuery('[name="ticket_id"]').val()){
                    jQuery.post('index.php', {
                        module:'Timecontrol',
                        action:'QuickCreate',
                        related: jQuery('[name="ticket_id"]').length > 0 ? jQuery('[name="ticket_id"]').val() : 0
                    }, function(data) {
                    	if(data.result){
	                        Timecontrol.loadTimerPopup();
	                        ...
	                        if(app.getModuleName() == 'HelpDesk' && app.getViewName() == 'Detail'){
	                			...
	                			if(form.length){
	                				if(jQuery('[name="ticket_id"]').val() == app.getRecordId()){
		                				form.find("[name='record']").val(data.result);
		                                form.find("#clock_counter").val('');
		                                form.find("#clock_display_hours").html('00');
		                                form.find("#clock_display_minutes").html('00');
		                      			form.find('.ticketHalted').addClass('hide');
		                      			form.find('.ticketStart').removeClass('hide');
		            	           		app.helper.showSuccessNotification({ message: 'Timer Start Successfully.'});
	                				}
	                			}
	                        }
                    	}else{
                    		app.helper.showErrorNotification({title: 'Error', message: 'Timer is already running.'});
                    	}
                    });
            	}else{
            		app.helper.showErrorNotification({title: 'Error', message: 'Please select ticket.'});
            	}
            });
            ...
            jQuery('.finish').on('click', function(e){
            	e.preventDefault();
...
//
    ...
    SelectInvoice : function(module,view) {
        ...
        var postPopupHandler = function(e, data){
            ...
            if(!$.isArray(data)){
                ...
                window.location.href = "index.php?module=Invoice&view=Edit&record="+invoiceId+"&recallid="+selectedIds.join(',');
    ...
    helpdeskModuelFunction :function(){
         ...
         $('.stopTimer').on('click',function(e){
     	   ...
     	   app.helper.showProgress();
	           	jQuery.post('index.php', {
	           		...
	           		form.find('.ticketHalted').removeClass('hide');
	           		form.trigger("reset");
	           		app.helper.showSuccessNotification({ message: 'Timer Stopped Successfully.'});
            });
        });
