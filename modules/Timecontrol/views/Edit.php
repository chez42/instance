...
// 			$request->set('tcunits', 1);
		...
		if(!$request->has('record') && $request->get('mode') != 'edit') {
			...
			$request->set('date_end', $vtnow->getDisplayDate($current_user));
// 			$request->set('tcunits', 1);
// 			$rshd=$adb->pquery('select tcproduct from vtiger_users where id=?',array($current_user->id), false);
// 			if ($rshd) {
...
// 			}
		...
		if ($request->has('relatedto') and getSalesEntityType($request->get('relatedto'))=='HelpDesk') {
			$rshd=$adb->pquery('select ticket_no from vtiger_troubletickets where ticketid=?',array($request->get('relatedto')));
			if ($rshd) {
...
// 				}
        ...
        if($request->has('isDuplicate') && $request->get('isDuplicate') == 'true' && $request->get('modeTC') == 'continueTimetracker') {
//             $request->set('invoiced_on', '');
            $request->set('timecontrolstatus', 'run');
        }
