    ...
    */
    ...
    public static function getForId($id, $module_name = false, $user = false) {
        ...
        if(strpos($id, "x") !== false) {
            ...
            if(empty($module_name)) {
                global $adb;
                $sql = "SELECT name FROM vtiger_ws_entity WHERE id = ?";
                $result = $adb->pquery($sql, array(intval($idParts[0])));
        ...
        if($this->_data === false) {
            ...
            try {
				...
				if($exp->getCode() == "DATABASE_QUERY_ERROR") {
                    ...
                    $sql = "SELECT setype FROM vtiger_crmentity WHERE crmid = ?";
                    $checkTMP = $adb->pquery($sql, array($this->_id));
    ...
    public function getWsId() {
        ...
        if($parts[0] == "9") {
            ...
            $result = $adb->pquery($sql, array(intval($parts[1])));
