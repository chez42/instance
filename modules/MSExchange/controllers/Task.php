...
class MSExchange_Task_Controller extends MSExchange_Base_Controller{
    ...
    function __construct($user = false){
        ...
        $syncUserCredx = MSExchange_Utils_Helper::getCredentialsForUser($user, 'Task');
        ...
        if(!$this->api){
            ...
            if($this->db->num_rows($result)){
                ...
                if($globalSettings['impersonate_user_account'] == 1){
                    ...
                    $this->api = new MSExchange_ExchangeTask_Model($globalSettings['url'], $globalSettings['username'], $globalSettings['password'], $globalSettings['exchange_version']);
                    $this->api->setImpersonation($globalSettings['impersonation_type'], $userSettings['impersonation_identifier']);
                } else {
                    $this->api = MSExchange_ExchangeTask_Model::getInstance($globalSettings['url'], $userSettings['username'], $userSettings['password'], $globalSettings['exchange_version']);
                }
            }
        }
    ...
    public function getTargetConnector() {
        $connector =  new MSExchange_Task_Connector($this->user);
    ...
    public function synchronizePull() {
        ...
        $sourceRecords = $this->sourceConnector->pull('Task');
