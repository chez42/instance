...
abstract class MSExchange_Base_Controller {
    ...
    public function getSourceConnector() {
        $connector = new MSExchange_Vtiger_Connector($this->user);
	...
	function getTargetRecordModel($data) {
	    return new MSExchange_Calendar_Model($data);
	}

	function getSourceRecordModel($data) {
	    return new MSExchange_Vtiger_Model($data);
	}
    
	function getMSExchangeModel(){
	    return $this->api;
	}
}
