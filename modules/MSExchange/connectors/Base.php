...
class MSExchange_Base_Connector {
    ...
    public function exchangeFormat($date) {
     ...
     *
     */
    public function performBasicTransformations(MSExchange_SyncRecord_Model $sourceRecord, MSExchange_SyncRecord_Model $targetRecord){
    ...
    public function performBasicTransformationsToSourceRecords(MSExchange_SyncRecord_Model $sourceRecord, MSExchange_SyncRecord_Model $targetRecord){
        $sourceRecord->setId($targetRecord->getId())
        ->setModifiedTime($targetRecord->getModifiedTime());
        return $sourceRecord;
    }
    
    public function performBasicTransformationsToTargetRecords(MSExchange_SyncRecord_Model $sourceRecord, MSExchange_SyncRecord_Model $targetRecord){
        ...
        if($mode == $this->create)
            $this->idmap_create($syncid, $serverid, $clientid,$clientModifiedTime,$serverModifiedTime,$parentId);
                ...
                $this->idmap_update($syncid, $serverid, $clientid, $clientModifiedTime,$serverModifiedTime,$parentId);
                    ...
                    $result = $db->pquery("SELECT * FROM vtiger_msexchange_recordmapping WHERE serverid=?
			and BINARY exchangeid=?",array($serverid,$clientid));
    ...
    function idmap_create($syncid, $serverid, $clientid, $clientModifiedTime, $serverModifiedTime, $parentId = 'NULL'){
        $db = PearDatabase::getInstance();
        $db->pquery("INSERT INTO vtiger_msexchange_recordmapping (sync_id, serverid, exchangeid, exchangemodifiedtime,
    ...
    function idmap_update($syncid, $serverid, $clientid,$clientModifiedTime,$serverModifiedTime, $parentId = 'NULL'){
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE vtiger_msexchange_recordmapping SET exchangemodifiedtime=?,servermodifiedtime=?,parent_exchangeid=?
        WHERE sync_id = ? and serverid = ? and BINARY exchangeid = ? ",
    ...
    function idmap_delete($syncid, $serverid, $clientid){
        $db = PearDatabase::getInstance();
        $db->pquery("DELETE FROM vtiger_msexchange_recordmapping WHERE sync_id = ? and serverid=?
		and BINARY exchangeid=?", array($syncid, $serverid, $clientid));
    }
    
    
    function idmap_updateMapDetails($syncid, $clientid,$clientModifiedTime,$serverModifiedTime){
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE vtiger_msexchange_recordmapping SET exchangemodifiedtime=?, servermodifiedtime=?
    ...
    function idmap_get_clientmap($syncid, $serverids) {
        ...
        $result = $db->pquery(sprintf(
            "SELECT serverid, exchangeid, exchangemodifiedtime, servermodifiedtime,id
			FROM vtiger_msexchange_recordmapping WHERE sync_id = ? and serverid IN ('%s')",
            implode("','", $serverids)), array($syncid));
        
        $mapping = array();
        if ($db->num_rows($result)) {
            while ($row = $db->fetch_array($result)) {
                $mapping[$row['serverid']] = array("exchangeid"=>$row['exchangeid'],"exchangemodifiedtime"=>$row['exchangemodifiedtime'],
    ...
    function idmap_get_clientservermap($syncid, $clientids){
        ...
        $result = $db->pquery(sprintf("SELECT serverid, exchangeid FROM vtiger_msexchange_recordmapping
		WHERE sync_id = ? and BINARY exchangeid IN ('%s')",implode("','", $clientids)), array($syncid));
        
        $mapping = array();
        if($db->num_rows($result)){
            while($row = $db->fetch_array($result)){
                $mapping[$row['exchangeid']] = $row['serverid'];
            }
        }
    ...
    function getQueueDeleteRecord($appId){
		...
		INNER JOIN vtiger_msexchange_recordmapping ON (vtiger_msexchange_recordmapping.id=vtiger_msexchange_queuerecords.syncserverid)
		WHERE vtiger_msexchange_recordmapping.sync_id=? ",array($appId));
        $serverIds = array();
        $num_rows = $db->num_rows($result);
        for($i=0;$i<$num_rows;$i++){
    ...
    function getSyncServerId($clientId,$serverId,$clientAppId){
        $db = PearDatabase::getInstance();
        $syncServerId = NULL;
        $query = "SELECT id FROM vtiger_msexchange_recordmapping WHERE exchangeid=? and serverid=? and sync_id=?";
    ...
    function deleteQueueMasterRecords($syncServerIdList){
        $db= PearDatabase::getInstance();
        $deleteQuery = "DELETE FROM vtiger_msexchange_recordmapping WHERE parent_exchangeid IN (".generateQuestionMarks($syncServerIdList).")";
        $result = $db->pquery($deleteQuery,$syncServerIdList);
    }
