...
class MSExchange_Contacts_Connector extends MSExchange_Base_Connector{
    ...
    public function __construct($user) {
    	...
    	$mappingFields = MSExchange_Utils_Helper::getFieldMappingDetails($user, 'Contacts');
    	
    	$this->mappingFields = $mappingFields = array_reduce($mappingFields, function ($result, $item) {
    	    $result[$item['CRM']] = $item['MSExchange'];
    	    return $result;
    	}, array());
    }
    
	public function getName() {
		return 'MSExchangeContacts';
	}
    ...
    public function getContacts($user = false) {
    	...
    	if (MSExchange_Utils_Helper::getSyncTime('Contacts')) {
    	    $lastUpdatedTime = MSExchange_Utils_Helper::getSyncTime('Contacts');
		...
		foreach ($contacts as $exchangeContact) {
		    ...
		    $recordModel = MSExchange_Contacts_Model::getInstanceFromValues(array('entity' => $exchangeContact));
            
		    $recordModel->setType($this->getSynchronizeController()->getSourceType())->setMode(MSExchange_SyncRecord_Model::UPDATE_MODE);
		    
		    $itemId = $exchangeContact->getItemId();
		    
		    $exchangeRecords[$itemId->getId()] = $recordModel;
            
		    $exchange_modified_time[] = $exchangeContact->getLastModifiedTime();
        }
        ...
        if (isset($last_modified_time)&& !empty($exchange_modified_time)){
            MSExchange_Utils_Helper::updateSyncTime('Contacts', $last_modified_time, $user);
        } else {
            MSExchange_Utils_Helper::updateSyncTime('Contacts', false, $user);
        }
    ...
    public function transformToTargetRecord($vtRecords) {
        ...
        foreach ($vtRecords as $vtRecord) {
            ...
            if($vtRecord->get('_id') != '' && $vtRecord->getMode() == MSExchange_SyncRecord_Model::UPDATE_MODE){
            ...
            if ($exchangeRecord->getMode() != MSExchange_SyncRecord_Model::DELETE_MODE) {
                ...
                if (empty($entity['lastname'])) {
                    if (!empty($entity['firstname'])) {
                        ...
                        $entity['lastname'] = 'MS Exchange Contact'; 
                    }
               }
    ...
    function getLastModifiedExchangeContactItems($lastUpdatedTime){
    
        $syncController = $this->getSynchronizeController();
        
        $MSExchangeModel = $syncController->getMSExchangeModel();
        
        $folder = $MSExchangeModel->getFolder('contacts');
        
        $response = $MSExchangeModel->getContacts($folder, $lastUpdatedTime, $this->indexedPagingOffset);
        ...
        if($last_modified_time == $lastUpdatedTime && $totalRecords >= 100){
            ...
            $response = $this->getLastModifiedExchangeContactItems($lastUpdatedTime);
    ...
    public function getExchangeRecords($lastUpdatedTime){
        ...
        $exchangeRecords = $this->getLastModifiedExchangeContactItems($lastUpdatedTime);
        
        $exchangeContactDetails = array();
        
        if(!empty($exchangeRecords)){
            
            $exchange_contact_ids = array_chunk($exchangeRecords, 100);
            
            if(!empty($exchange_contact_ids)){
                
                foreach($exchange_contact_ids as $exchange_contact_chunk_ids){
                    
                    $exchangeContactChunkDetails = $MSExchangeModel->getItems($exchange_contact_chunk_ids);
                    
                    if(is_object($exchangeContactChunkDetails)){
                        $exchangeContactChunkDetails = array($exchangeContactChunkDetails);
                    }
                    
                    $exchangeContactDetails = array_merge($exchangeContactDetails, array_filter($exchangeContactChunkDetails));
                }
            }
        }
        
        return $exchangeContactDetails;
    }
	...
	public function addExchangeRecord($data){
	    ...
	    $response = $MSExchangeModel->createContact($folder, $data);
    ...
    function getExchangeContactData($exchangeRecord){
        ...
        $entity = new MSExchange_MSExchangeContacts_Model();
    ...
    function getMSExchangeContactData($vtData, $mode, $exchangeObject = false ){
        ...
        $newRecord = new MSExchange_MSExchangeContacts_Model();
        ...
        if(isset($vtData['_id']) && !empty($vtData['_id'])){
            ...
            if($mode == MSExchange_SyncRecord_Model::UPDATE_MODE) {
                ...
                $exchangeContactdata = $this->getContactEntityData(new MSExchange_Contacts_Model(array("entity" => $allData)), $user, true);
