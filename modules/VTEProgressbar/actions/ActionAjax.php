...
class VTEProgressbar_ActionAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->exposeMethod("ChangeProgressBar");
    ...
    public function vteLicense()
    {
        ...
        if (!$vTELicense->validate()) {
            header("Location: index.php?module=VTEProgressbar&parent=Settings&view=Settings&mode=step2");
        }
    }
    ...
    public function UpdateStatus(Vtiger_Request $request)
    {
        ...
        $progressbar_module = $request->get("progressbar_module");
        if ($status == "on") {
            $status = 1;
        } else {
            $status = 0;
        }
        $sql = "UPDATE `vte_progressbar_settings` SET active=? WHERE id=?";
        $adb->pquery($sql, array($status, $record));
        if ($status == 1) {
            $sql = "UPDATE `vte_progressbar_settings` SET active= 0 WHERE module= ? AND id <> ?";
            $adb->pquery($sql, array($progressbar_module, $record));
        }
        $response = new Vtiger_Response();
        $response->setResult("success");
        $response->emit();
    }
    public function ChangeProgressBar(Vtiger_Request $request)
    {
        global $adb;
        $record = $request->get("record");
        $moduleSelected = $request->get("moduleSelected");
    ...
    public function DeleteRecord(Vtiger_Request $request)
    {
        global $adb;
        $record = $request->get("record");
        $sql = "DELETE FROM `vte_progressbar_settings`  WHERE id=?";
