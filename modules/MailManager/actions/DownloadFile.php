...
class MailManager_DownloadFile_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
	    ...
	    if (MailManager_Utils_Helper::allowedFileExtension($attachmentName)) {
	        ...
	        $mail->readFromDB($request->get('attachment_id'));
    ...
    public function saveAsDocument(Vtiger_Request $request){
        ...
        $attachmentName = urldecode($request->get('attname'));
        ...
        if (MailManager_Utils_Helper::allowedFileExtension($attachmentName)) {
            ...
            $mail = new MailManager_Message_Model(false, false);
            $mail->readFromDB($attachmentId);
            $attachment = $mail->attachments(true, $attachmentName);
            
            $attachmentId = $attachment[0]['attachid'];
    ...
    public function previewDocument(Vtiger_Request $request){
        ...
        if (MailManager_Utils_Helper::allowedFileExtension($attachmentName)) {
            ...
            $recordId = $attachment[0]['attachid'];
