...
class QuotingTool_Record_Model extends Vtiger_Record_Model
{
    ...
    public function save($id, $data)
    {
        ...
        $columnNames = array("filename", "module", "body", "header", "content", "footer", "description", "deleted", "email_subject", "email_content", "mapping_fields", "attachments", "created", "updated", "anwidget", "anblock", "is_active", "createnewrecords", "linkproposal", "owner", "share_to", "share_status", "settings_layout", "custom_function", "file_name");
        if ($id) {
    ...
    public static function findAll()
    {
        ...
        $db = PearDatabase::getInstance();
        $instances = array();
        $rsQuotingTool = $db->pquery("SELECT * FROM `vtiger_quotingtool` WHERE `deleted` != 1 order by updated desc");
            ...
            while ($row = $db->fetchByAssoc($rs)) {
                ...
                if ($numRow == 0 && in_array($moduleName, $allRelatedModule) == false) {
                    ...
                    if (0 < $db->num_rows($checkLink)) {
                        ...
                        $moduleInstance->deleteLink("DETAILVIEW", $js_widgetLabel, $js_link);
                    }
                }
            }
        }
                ...
                foreach ($arrModule as $val) {
                    ...
                    if ($db->num_rows($listRecord) == 0 && in_array($val, $allRelatedModule) == false) {
                        ...
                        if (0 < $db->num_rows($checkLink)) {
                            ...
                            $parentModule->deleteLink("DETAILVIEW", $js_widgetLabel, $js_link);
                        }
                    }
                }
            }
