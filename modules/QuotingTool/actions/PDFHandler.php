...
class QuotingTool_PDFHandler_Action extends Vtiger_Action_Controller
{
    ...
    public function duplicate(Vtiger_Request $request)
    {
        ...
        $data = $record->getData();
        $allow = array("filename", "module", "body", "header", "content", "footer", "description", "deleted", "email_subject", "email_content", "mapping_fields", "attachments");
        if ($data && !empty($data)) {
            foreach ($data as $key => $value) {
                ...
                if ($key == "filename") {
                    ...
                    if (($key == "mapping_fields" || $key == "attachments") && $value) {
