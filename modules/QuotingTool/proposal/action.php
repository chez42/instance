...
function submit()
{
    ...
    if ($sign_to == "PRIMARY") {
        $newSignedRecord = array("signature" => $signature, "signature_name" => $signatureName, "signature_date" => date("Y-m-d", $timestamp), "cf_signature_time" => date("H:i:s", $timestamp), "filename" => $pdf, "signedrecord_status" => $status_text, "related_to" => $refId);
    } else {
        $newSignedRecord = array("secondary_signature" => $signature, "secondary_signature_name" => $signatureName, "secondary_signature_date" => date("Y-m-d", $timestamp), "cf_secondary_signature_time" => date("H:i:s", $timestamp), "secondary_filename" => $pdf, "secondary_signedrecord_status" => $status_text, "related_to" => $refId);
    }
...
function mappingData($refId, $tempMappingFields, $status, $isCreateNewRecord = false, $childModule, $primaryModule)
{
    ...
    foreach ($tempMappingFields as $k => $field) {
        $vField = Vtiger_Field_Model::getInstance($field->obfuscated_73656C65637465642D6669656C64_);
        if ($field->type == $status && $vField) {
            $mappingFields[$vField->get("name")] = $field->obfuscated_73656C65637465642D76616C7565_;
        }
    }
