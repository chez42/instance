...
#WHERE advisor_control_number RLIKE (CONCAT('[[:<:]]',p.master_production_number,'[[:>:]]'))
    ...
    static public function GetContactIDsFromOmniscientControlNumber($ccn, $limit = null)
    {
        global $adb;
        if (strlen($limit) > 0)
            $limit = " LIMIT {$limit} ";
        ...
        $query = "SELECT contact_link FROM vtiger_portfolioinformation p 
                  JOIN vtiger_portfolioinformationcf cf ON p.portfolioinformationid = cf.portfolioinformationid 
                  WHERE omniscient_control_number IN ({$questions}) {$limit}";
    ...
    static public function GetAdvisorIDFromAccountNumber($account_number)
    {
        global $adb;
        $query = "SELECT advisor_id FROM vtiger_portfolioinformation WHERE account_number = ?";
        $result = $adb->pquery($query, array($account_number));
        if ($adb->num_rows($result) > 0) {
            return $adb->query_result($result, 0, 'advisor_id');
        }
        return 0;
    }
...
#            CALL CALCULATE_MONTHLY_INTERVALS_LOOP("34300882", "1900-01-01", "2017-10-12", "schwab", "live_omniscient");
    ...
    static public function UpdatePortfolioTypeTDOnly()
    {
        ...
        $query = "UPDATE vtiger_portfolioinformation p
	             JOIN vtiger_portfolioinformationcf cf USING (portfolioinformationid)
	             JOIN portfolios_mapping_td map ON cf.account_type_code = map.td_code 
	             SET cf.cf_2549 = map.omniscient_type WHERE origination = 'TD'";
        $adb->pquery($query, array());
    }
