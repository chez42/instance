...
#            $f = self::GetFieldsFromRow($rules);//Get the field names only
            $element_rules = self::GetValueMap($f);//Determine the rules for each individual field
            while($row = $adb->fetchByAssoc($result)){
                $tmp = array("");
                foreach($row AS $k => $v){
                        ...
                        foreach ($category_list AS $k => $heading) {
                            if ($row[$heading] == $category[$heading]) {
                                ...
                                if ($counter == $expected_count) {
                                    ...
                                    #                                echo "WE HAVE OUR CATEGORY FOR {$row['security_symbol']} and it is {$category['category_id']}!<br />";
                                }
                            }
                        }
