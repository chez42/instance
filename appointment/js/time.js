...
function getConvertedTimeFromEpoch(epoch)
{
	...
	var time = date.tz(SELECTED_TIMEZONE).format('HH:mm');
	return time;
}
...
function getTimeInVisitorTimezone(time,timezone,firstTimeLoading)
{
	...
	return toTimeZone(m.valueOf(),timezone);
}
 ...
 * @param timezone  we are change  user 09:00 to Visitor time @Us/eastrn 
 * @returns {String}
 */
...
/**
 * converts the user time to vistor time
 * @param time
 * @param zone
 * @returns
 */
