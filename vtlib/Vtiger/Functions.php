...
class Vtiger_Functions {
	...
	static function validateImage($file_details) {
		...
		if ($saveimage == 'true') {
			...
			if (stripos($imageContents, "<?php") !== false) { // suspicious dynamic content.
				$saveimage = 'false';
			}
		}
		return $saveimage;
		...
		foreach ($fields['custom'] as $columnname) {
			...
			switch ($columnname) {
				...
				case 'crmdetailviewurl'	:	if($module !== 'Users') {
				                                $token_value = $site_URL.'/modules/Emails/actions/EmailRedirect.php?applicationKey='.$application_unique_key.'&redirectUrl='.$site_URL."/index.php?module=$module&view=Detail&record=$recordId";
											} else {
