...
class Vtiger_Block {
	...
	/**
	 * Get all block instances associated for tab view with the module
	 * @param Vtiger_Module Instance of the module
	 */
	static function getAllForModuleTabView($moduleInstance) {
	    global $adb;
	    $instances = false;
	    
	    $query = "SELECT vtiger_blocks.* FROM vtiger_module_tab
        ...
        WHERE vtiger_module_tab.module_name=? ORDER BY vtiger_module_tab.sequence,vtiger_module_tab_blocks.blocksequence ASC";
	    $queryParams = Array($moduleInstance->getName());
